 ...
 *
 * This algorithm can compute the AUC in 1-pass with good resolution. During the
 * pass, it builds an online histogram of the probabilities up to the resolution
 * (number of bins) asked-for. It also computes the true-positive and
 * false-positive counts for the histogramed thresholds. With these in hand, we
 * can compute the TPR (True Positive Rate) and the FPR for the given
...
public class AUC2 extends Iced {
	 ...
	 *
	 * This is an Enum class, with an exec() function to compute the criteria from
	 * the basic parts, and from an AUC2 at a given threshold index.
	 */
	public enum ThresholdCriterion {
		...
		ThresholdCriterion(boolean isInt) {
			_isInt = isInt;
		}

		/**
		 * @param tp
		 *            True Positives (predicted true, actual true )
		 * @param fp
		...
		public double exec(AUC2 auc, int idx) {
			return exec(auc.tp(idx), auc.fp(idx), auc.fn(idx), auc.tn(idx));
		}
	...
	/**
	 * Default bins, good answers on a highly unbalanced sorted (and reverse sorted)
	 * datasets
	 */
	public AUC2(Vec probs, Vec actls) {
		this(NBINS, probs, actls);
	}

	/**
	 * User-specified bin limits. Time taken is product of nBins and rows; large
	 * nBins can be very slow.
	 */
	AUC2(int nBins, Vec probs, Vec actls) {
		this(new AUC_Impl(nBins).doAll(probs, actls)._bldr);
	}
	...
	// Build a CM for a threshold index. - typed as doubles because of double
	// observation weights
	...
	public void perRow(double pred, int act, double w ) {
      ...
      if( idx >= 0 ) {          // Found already in histogram; merge results
        if( act==0 ) {
        	_fps[idx]+=w; else _tps[idx]+=w; // One more count; no change in squared error
        }
