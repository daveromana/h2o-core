...
public class CreateFrame extends Iced {
  ...
  public Job<Frame> execImpl() {
    ...
    if (seed_for_column_types == -1) {
    	seed_for_column_types = seed;
    ...
    if (missing_fraction < 0 || missing_fraction > 1) {
    	throw new IllegalArgumentException("Missing fraction must be between 0 and 1.");
    if (integer_fraction < 0 || integer_fraction > 1){
    ...
    if (response_factors > Model.Parameters.MAX_SUPPORTED_LEVELS) {
    	throw new IllegalArgumentException("Response factors must be <= " + Model.Parameters.MAX_SUPPORTED_LEVELS + ".");
    if (factors > 1000000){
