...
public class Distribution extends Iced<Distribution> {
	...
	public Distribution(Model.Parameters params) {
		...
		huberDelta = 1; // should be updated to huber_alpha quantile of absolute error of predictions
						// via setter
	...
	public static double log(double x) {
		...
		// if (val == MIN_LOG) Log.warn("Log underflow: log(" + x + ") truncated to " +
		// MIN_LOG);
		return val;
	}
	...
	public double deviance(double w, double y, double f) {
		...
		case tweedie:
			assert (tweediePower > 1 && tweediePower < 2);
			return 2 * w
					* (Math.pow(y, 2 - tweediePower) / ((1 - tweediePower) * (2 - tweediePower))
							- y * exp(f * (1 - tweediePower)) / (1 - tweediePower)
							+ exp(f * (2 - tweediePower)) / (2 - tweediePower));
		case modified_huber:
			double yf = (2 * y - 1) * f;
			if (yf < -1) {
				return -w * 4 * yf;
	...
	/**
	 * (Negative half) Gradient of deviance function at predicted value f, for
	 * actual response y This assumes that the deviance(w,y,f) is w*deviance(y,f),
	 * so the gradient is w * d/df deviance(y,f)
	 * 
	 * @param y
	 *            (actual) response
	 * @param f
	 *            (predicted) response in link space (including offset)
	 ...
	 * 
	 * @param f
	 *            value in original space, to be transformed to link space
	 * @return link(f)
	 */
	 ...
	 * 
	 * @param f
	 *            value in link space, to be transformed back to original space
	 * @return linkInv(f)
	 */
	 ...
	 * 
	 * @param f
	 *            value to be transformed by link inverse
