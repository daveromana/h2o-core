...
/**
 * A Model models reality (hopefully). A model can be used to 'score' a row
 * (make a prediction), or a collection of rows on any compatible dataset -
 * meaning the row has all the columns with the same names as used to build the
 * mode and any categorical columns can be adapted.
 */
public abstract class Model<M extends Model<M, P, O>, P extends Model.Parameters, O extends Model.Output>
	...
	/**
	 * Default threshold for assigning class labels to the target class (for
	 * binomial models)
	 * 
	...
	/**
	 * Model-specific parameter class. Each model sub-class contains instance of one
	 * of these containing its builder parameters, with model-specific parameters.
	 * E.g. KMeansModel extends Model and has a KMeansParameters extending
	 * Model.Parameters; sample parameters include K, whether or not to normalize,
	 * max iterations and the initial random seed.
	 *
	 * <p>
	 * The non-transient fields are input parameters to the model-building process,
	 * and are considered "first class citizens" by the front-end - the front-end
	 * will cache Parameters (in the browser, in JavaScript, on disk) and rebuild
	 * Parameter instances from those caches.
	 *
	...
	public abstract static class Parameters extends Iced<Parameters> {
		...
		/**
		 * The pretty algo name for this Model (e.g., Gradient Boosting Machine, rather
		 * than GBM).
		 */
		...
		public enum CategoricalEncodingScheme {
			AUTO(false), OneHotInternal(false), OneHotExplicit(false), Enum(false), Binary(false), Eigen(
					false), LabelEncoder(false), SortByResponse(true), EnumLimited(false);
			CategoricalEncodingScheme(boolean needResponse) {
		...
		public long getOrMakeRealSeed() {
			while (_seed == -1) {
				...
				Log.debug(
						"Auto-generated time-based seed for pseudo-random number generator (because it was set to -1): "
								+ _seed);
			}
			return _seed;
		}
		 ...
		 * average of the stopping_metric does not improve by stopping_tolerance for k
		 * scoring events. Can only trigger after at least 2k scoring events. Use 0 to
		 * disable.
		 */
		...
		/**
		 * When classes are being balanced, limit the resulting dataset size to the
		 * specified multiple of the original dataset size. Maximum relative size of the
		 * training data after balancing class counts (can be less than 1.0)
		 */
		public float _max_after_balance_size = 5.0f;

		/**
		 * Desired over/under-sampling ratios per class (lexicographic order). Only when
		 * balance_classes is enabled. If not specified, they will be automatically
		...
		/**
		 * A model key associated with a previously trained Deep Learning model. This
		 * option allows users to build a new model as a continuation of a previously
		 * generated model.
		 */
		public Key<? extends Model> _checkpoint;

		/**
		 * A pretrained Autoencoder DL model with matching inputs and hidden layers can
		 * be used to initialize the weights and biases (excluding the output layer).
		 */
		...
		public final Frame train() {
			return _train == null ? null : _train.get();
		}

		/**
		 * @return the validation frame instance, or null if a validation frame was not
		 *         specified
		 */
		public final Frame valid() {
			return _valid == null ? null : _valid.get();
		}
		...
		/**
		 * Read-UnLock both training and validation User frames. This method is called
		 * on crashing cleanup pathes, so handles the case where the frames are not
		 * actually locked.
		 */
		 ...
		 * Overload this method for models that have sparse data handling - a zero will
		 * preserve the sparseness. Otherwise, NaN is used.
		 * 
		...
		// FIXME: this is really horrible hack, Model.Parameters has method
		// checksum_impl,
		...
		/**
		 * Compute a checksum based on all non-transient non-static ice-able assignable
		 * fields (incl. inherited ones) which have @API annotations. Sort the fields
		 * first, since reflection gives us the fields in random order and we don't want
		 * the checksum to be affected by the field order. NOTE: if a field is added to
		 * a Parameters class the checksum will differ even when all the previous
		 * parameters have the same value. If a client wants backward compatibility they
		 * will need to compare parameter values explicitly.
		 *
		 * The method is motivated by standard hash implementation `hash = hash * P +
		 * value` but we use high prime numbers in random order.
		 * 
		 * @return checksum
		 */
	...
	/**
	 * Model-specific output class. Each model sub-class contains an instance of one
	 * of these containing its "output": the pieces of the model needed for scoring.
	 * E.g. KMeansModel has a KMeansOutput extending Model.Output which contains the
	 * cluster centers. The output also includes the names, domains and other fields
	 * which are determined at training time.
	 */
	public abstract static class Output extends Iced {
		 ...
		 * The last name is the response column name (if any).
		 */
		...
		/**
		 * List of Keys to cross-validation models (non-null iff _parms._nfolds > 1 or
		...
		// Each individual model's start/end/run time is reported here, not the total
		// time to build N+1 cross-validation models, or all grid models
		...
		/**
		 * Any final prep-work just before model-building starts, but after the user has
		 * clicked "go". E.g., converting a response column to an categorical touches
		 * the entire column (can be expensive), makes a parallel vec (Key/Data leak
		 * management issues), and might throw IAE if there are too many classes.
		 */
		...
		/**
		 * Returns number of input features (OK for most supervised methods, need to
		 * override for unsupervised!)
		 */
		public int nfeatures() {
			return _names.length - (_hasOffset ? 1 : 0) - (_hasWeights ? 1 : 0) - (_hasFold ? 1 : 0)
					- (isSupervised() ? 1 : 0);
		}

		/**
		 * List of all the associated ModelMetrics objects, so we can delete them when
		 * we delete this model.
		 */
		...
		/**
		 * Validation set metrics obtained during model training (if a validation data
		 * set was specified)
		 */
		...
		/**
		 * User-facing model summary - Display model type, complexity, size and other
		 * useful stats
		 */
		public TwoDimTable _model_summary;

		/**
		 * User-facing model scoring history - 2D table with modeling accuracy as a
		 * function of time/trees/epochs/iterations, etc.
		 */
		...
		public String responseName() {
			return isSupervised() ? _names[responseIdx()] : null;
		}

		public String weightsName() {
			return _hasWeights ? _names[weightsIdx()] : null;
		}

		public String offsetName() {
			return _hasOffset ? _names[offsetIdx()] : null;
		}

		public String foldName() {
			return _hasFold ? _names[foldIdx()] : null;
		}
		...
		// Vec layout is [c1,c2,...,cn,w?,o?,r], cn are predictor cols, r is response, w
		// and o are weights and offset, both are optional
		...
		/**
		 * Is this model a binomial classification model? (v. a regression or clustering
		 * model)
		 */
		...
		/**
		 * Is this model a multinomial classification model (supervised and nclasses() >
		 * 2
		 */
		...
		/**
		 * Number of classes in the response column if it is categorical and the model
		 * is supervised.
		 */
		public int nclasses() {
		...
		// Note: some algorithms MUST redefine this method to return other model
		// categories
		public ModelCategory getModelCategory() {
			if (isSupervised()) {
				return (isClassifier() ? (nclasses() > 2 ? ModelCategory.Multinomial : ModelCategory.Binomial)
		...
		public boolean isAutoencoder() {
			return false;
		} // Override in DeepLearning and so on.
		...
		public synchronized Key<ModelMetrics>[] getModelMetrics() {
			return Arrays.copyOf(_model_metrics, _model_metrics.length);
		}

		protected long checksum_impl() {
			return (null == _names ? 13 : Arrays.hashCode(_names))
					* (null == _domains ? 17 : Arrays.deepHashCode(_domains)) * getModelCategory().ordinal();
		}
	...
	protected String[][] scoringDomains() {
		return _output._domains;
	}

	public ModelMetrics addMetrics(ModelMetrics mm) {
		return addModelMetrics(mm);
	}
	 ...
	 *            (actual) response
	 * @param f
	 *            (predicted) response in original response space
	...
	public void fillScoringInfo(ScoringInfo scoringInfo) {
		...
		if (this._output.isBinomialClassifier()) {
			scoringInfo.training_AUC = this._output._training_metrics == null ? null
					: ((ModelMetricsBinomial) this._output._training_metrics)._auc;
			scoringInfo.validation_AUC = this._output._validation_metrics == null ? null
					: ((ModelMetricsBinomial) this._output._validation_metrics)._auc;
		}
	}
	...
	public double classification_error() {
		if (scoringInfo != null) {
			return last_scored().cross_validation ? last_scored().scored_xval._classError
					: last_scored().validation ? last_scored().scored_valid._classError
							: last_scored().scored_train._classError;
		}
		ModelMetrics mm = _output._cross_validation_metrics != null ? _output._cross_validation_metrics
				: _output._validation_metrics != null ? _output._validation_metrics : _output._training_metrics;
		if (mm == null) {
			return Double.NaN;
		}
	...
	public double mse() {
		if (scoringInfo != null) {
			return last_scored().cross_validation ? last_scored().scored_xval._mse
					: last_scored().validation ? last_scored().scored_valid._mse : last_scored().scored_train._mse;
		}
		ModelMetrics mm = _output._cross_validation_metrics != null ? _output._cross_validation_metrics
				: _output._validation_metrics != null ? _output._validation_metrics : _output._training_metrics;
		if (mm == null) {
	...
	public double mae() {
		if (scoringInfo != null) {
			return last_scored().cross_validation ? last_scored().scored_xval._mae
					: last_scored().validation ? last_scored().scored_valid._mae : last_scored().scored_train._mae;
		}
		ModelMetrics mm = _output._cross_validation_metrics != null ? _output._cross_validation_metrics
				: _output._validation_metrics != null ? _output._validation_metrics : _output._training_metrics;
		if (mm == null) {
	...
	public double rmsle() {
		if (scoringInfo != null) {
			return last_scored().cross_validation ? last_scored().scored_xval._rmsle
					: last_scored().validation ? last_scored().scored_valid._rmsle : last_scored().scored_train._rmsle;
		}
		ModelMetrics mm = _output._cross_validation_metrics != null ? _output._cross_validation_metrics
				: _output._validation_metrics != null ? _output._validation_metrics : _output._training_metrics;
		if (mm == null) {
	...
	public double auc() {
		if (scoringInfo != null) {
			return last_scored().cross_validation ? last_scored().scored_xval._AUC
					: last_scored().validation ? last_scored().scored_valid._AUC : last_scored().scored_train._AUC;
		}
		ModelMetrics mm = _output._cross_validation_metrics != null ? _output._cross_validation_metrics
				: _output._validation_metrics != null ? _output._validation_metrics : _output._training_metrics;
		if (mm == null) {
	...
	public double deviance() {
		if (scoringInfo != null) {
			return last_scored().cross_validation ? last_scored().scored_xval._mean_residual_deviance
					: last_scored().validation ? last_scored().scored_valid._mean_residual_deviance
							: last_scored().scored_train._mean_residual_deviance;
		}
		ModelMetrics mm = _output._cross_validation_metrics != null ? _output._cross_validation_metrics
				: _output._validation_metrics != null ? _output._validation_metrics : _output._training_metrics;
		if (mm == null) {
	...
	public double logloss() {
		if (scoringInfo != null) {
			return last_scored().cross_validation ? last_scored().scored_xval._logloss
					: last_scored().validation ? last_scored().scored_valid._logloss
							: last_scored().scored_train._logloss;
		}
		ModelMetrics mm = _output._cross_validation_metrics != null ? _output._cross_validation_metrics
				: _output._validation_metrics != null ? _output._validation_metrics : _output._training_metrics;
		if (mm == null) {
	...
	public double mean_per_class_error() {
		if (scoringInfo != null) {
			return last_scored().cross_validation ? last_scored().scored_xval._mean_per_class_error
					: last_scored().validation ? last_scored().scored_valid._mean_per_class_error
							: last_scored().scored_train._mean_per_class_error;
		}
		ModelMetrics mm = _output._cross_validation_metrics != null ? _output._cross_validation_metrics
				: _output._validation_metrics != null ? _output._validation_metrics : _output._training_metrics;
		if (mm == null) {
	...
	public double lift_top_group() {
		if (scoringInfo != null) {
			return last_scored().cross_validation ? last_scored().scored_xval._lift
					: last_scored().validation ? last_scored().scored_valid._lift : last_scored().scored_train._lift;
		}
		ModelMetrics mm = _output._cross_validation_metrics != null ? _output._cross_validation_metrics
				: _output._validation_metrics != null ? _output._validation_metrics : _output._training_metrics;
		if (mm == null) {
	 ...
	 * count of columns, and within each factor column the same set of same-numbered
	 * levels. Extra levels are renumbered past those in the Train set but will
	 * still be present in the Test set, thus requiring range-checking.
	 *
	 * This routine is used before model building (with no Model made yet) to check
	 * for compatible datasets, and also used to prepare a large dataset for scoring
	 * (with a Model).
	 *
	 * Adaption does the following things: - Remove any "extra" Vecs appearing only
	 * in the test and not the train - Insert any "missing" Vecs appearing only in
	 * the train and not the test with all NAs ({@see missingColumnsType}). This
	 * will issue a warning, and if the "expensive" flag is false won't actually
	 * make the column replacement column but instead will bail-out on the whole
	 * adaption (but will continue looking for more warnings). - If all columns are
	 * missing, issue an error. - Renumber matching cat levels to match the Train
	 * levels; this might make "holes" in the Test set cat levels, if some are not
	 * in the Test set. - Extra Test levels are renumbered past the end of the Train
	 * set, hence the train and test levels match up to all the train levels; there
	 * might be extra Test levels past that. - For all mis-matched levels, issue a
	 * warning.
	 *
	 * The {@code test} frame is updated in-place to be compatible, by altering the
	 * names and Vecs; make a defensive copy if you do not want it modified. There
	 * is a fast-path cutout if the test set is already compatible. Since the
	 * test-set is conditionally modifed with extra CategoricalWrappedVec optionally
	 * added it is recommended to use a Scope enter/exit to track Vec lifetimes.
	 *
	 * @param test
	 *            Testing Frame, updated in-place
	 * @param expensive
	 *            Try hard to adapt; this might involve the creation of whole Vecs
	 *            and thus get expensive. If {@code false}, then only adapt if no
	 *            warnings and errors; otherwise just the messages are produced.
	 *            Created Vecs have to be deleted by the caller (e.g.
	 *            Scope.enter/exit).
	 * @return Array of warnings; zero length (never null) for no warnings. Throws
	 *         {@code IllegalArgumentException} if no columns are in common, or if
	 *         any factor column has no levels in common.
	 */
	public String[] adaptTestForTrain(Frame test, boolean expensive, boolean computeMetrics) {
		return adaptTestForTrain(test, _output._origNames, _output._origDomains, _output._names, _output._domains,
	 ...
	 *            Frame to be adapted
	 * @param origNames
	 *            Training column names before categorical column encoding - can be
	 *            the same as names
	 * @param origDomains
	 *            Training column levels before categorical column encoding - can be
	 *            the same as domains
	 * @param names
	 ...
	 *            Model parameters
	 * @param expensive
	 *            Whether to actually do the hard work
	 * @param computeMetrics
	 *            Whether metrics can be (and should be) computed
	 * @param interactions
	 *            Column names to create pairwise interactions with
	 * @param catEncoded
	 *            Whether the categorical columns of the test frame were already
	 *            transformed via categorical_encoding
	 */
	public static String[] adaptTestForTrain(Frame test, String[] origNames, String[][] origDomains, String[] names,
			String[][] domains, Parameters parms, boolean expensive, boolean computeMetrics, String[] interactions,
			ToEigenVec tev, IcedHashMap<Key, String> toDelete, boolean catEncoded) throws IllegalArgumentException {
		...
		// test frame matches the training frame (after categorical encoding, if
		// applicable)
		...
		// whether we need to be careful with categorical encoding - the test frame
		// could be either in original state or in encoded state
		...
		// test frame matches the user-given frame (before categorical encoding, if
		// applicable)
		if (checkCategoricals && origNames != null) {
			boolean match = Arrays.equals(origNames, test._names);
			if (!match) {
				match = true;
				// In case the test set has extra columns not in the training set - check that
				// all original pre-encoding columns are available in the test set
				// We could be lenient here and fill missing columns with NA, but then it gets
				// difficult to decide whether this frame is pre/post encoding, if a certain
				// fraction of columns mismatch...
			...
			// still have work to do below, make sure we set the names/domains to the
			// original user-given values such that we can do the int->enum mapping and cat.
			// encoding below (from scratch)
			if (match) {
				names = origNames;
				domains = origDomains;
			}
		...
		if (null != interactions) {
			...
			test.add(makeInteractions(test, false,
					InteractionPair.generatePairwiseInteractionsFromList(interactionIndexes), true, true, false));
		}
		...
		for (int i = 0; i < names.length; i++) {
			...
			// If a training set column is missing in the test set, complain (if it's ok,
			// fill in with NAs (or 0s if it's a fold-column))
			if (vec == null) {
				if (isResponse && computeMetrics) {
					...
					throw new IllegalArgumentException(
							"Test/Validation dataset is missing offset column '" + offset + "'");
					...
					String str = "Test/Validation dataset is missing column '" + names[i]
							+ "': substituting in a column of " + (isFold ? 0 : missing);
			...
			if (vec != null) { // I have a column with a matching name
				if (domains[i] != null) { // Model expects an categorical
					if (vec.isString()) {
						vec = VecUtils.stringToCategorical(vec); // turn a String column into a categorical column (we
																	// don't delete the original vec here)
					}
					if (expensive && vec.domain() != domains[i] && !Arrays.equals(vec.domain(), domains[i])) { // Result
																												// needs
																												// to be
																												// the
																												// same
						...
						try {
							...
							throw new IllegalArgumentException("Test/Validation dataset has a non-categorical column '"
									+ names[i] + "' which is categorical in the training data");
						}
					...
					if (parms._categorical_encoding == Parameters.CategoricalEncodingScheme.LabelEncoder) {
						...
						throw new IllegalArgumentException("Test/Validation dataset has categorical column '" + names[i]
		...
		if (good == names.length || (response != null && test.find(response) == -1 && good == names.length - 1)) { // Only
																													// update
																													// if
																													// got
																													// something
		...
		if (expensive && checkCategoricals && !catEncoded) {
			for (int i = 0; i < test.numCols(); ++i) {
				...
				// either the column of the test set is categorical (could be a numeric col
				// that's already turned into a factor)
				if (test.vec(i).cardinality() > 0) {
					haveCategoricalPredictors = true;
					break;
				}
				// or a equally named column of the training set is categorical, but the test
				// column isn't (e.g., numeric column provided to be converted to a factor)
		...
		// check if we first need to expand categoricals before calling this method
		// again
	 ...
	 * @param fr
	 *            frame which should be scored
	 * @return A new frame containing a predicted values. For classification it
	 *         contains a column with prediction and distribution for all response
	 *         classes. For regression it contains only one column with predicted
	 *         values.
	...
	/**
	 * Bulk score the frame {@code fr}, producing a Frame result; the 1st Vec is the
	 * predicted class, the remaining Vecs are the probability distributions. For
	 * Regression (single-class) models, the 1st and only Vec is the prediction
	 * value. The result is in the DKV; caller is responsible for deleting.
	 *
	 * @param fr
	 *            frame which should be scored
	 * @return A new frame containing a predicted values. For classification it
	...
	public Frame score(Frame fr, String destination_key, Job j, boolean computeMetrics)
			throws IllegalArgumentException {
		Frame adaptFr = new Frame(fr);
		computeMetrics = computeMetrics && (!isSupervised()
				|| (adaptFr.vec(_output.responseName()) != null && !adaptFr.vec(_output.responseName()).isBad()));
		...
		if (msg.length > 0) {
			for (String s : msg) {
				if ((_output.responseName() == null) || !containsResponse(s, _output.responseName())) { // response
																										// column
																										// missing will
																										// not generate
																										// warning for
		...
		if (_output.isClassifier() && computeMetrics) {
			/*
			 * if (false) { assert(mdomain != null); // label must be categorical
			 * ModelMetrics mm = ModelMetrics.getFromDKV(this,fr); ConfusionMatrix cm =
			 * mm.cm(); if (cm != null && cm._domain != null) //don't print table for
			 * regression if( cm._cm.length < _parms._max_confusion_matrix_size ) { // Print
			 * size limitation Log.info(cm.table().toString(1)); } if (mm.hr() != null) {
			...
			if (actual != null) { // Predict does not have an actual, scoring does
				...
				if (sdomain != null && mdomain != sdomain && !Arrays.equals(mdomain, sdomain)) {
					output.replace(0, new CategoricalWrappedVec(actual.group().addVec(), actual._rowLayout, sdomain,
							predicted._key));
				}
			}
		}
	 ...
	 * 
	 * @param valid
	 *            Validation Frame (must contain the response)
	 * @param predictions
	...
	public Frame computeDeviances(Frame valid, Frame predictions, String outputName) {
		...
		if (myDist != null && myDist.distribution == DistributionFamily.huber) {
			myDist.setHuberDelta(hex.ModelMetricsRegression.computeHuberDelta(valid.vec(_parms._response_column), // actual
	...
	protected String[] makeScoringNames() {
		...
		final int ncols = nc == 1 ? 1 : nc + 1; // Regression has 1 predict col; classification also has class
												// distribution
	...
	protected BigScore makeBigScoreTask(String[][] domains, String[] names, Frame adaptFrm, boolean computeMetrics,
			boolean makePrediction, Job j) {
		return new BigScore(domains[0], names != null ? names.length : 0, adaptFrm.means(),
				_output.hasWeights() && adaptFrm.find(_output.weightsName()) >= 0, computeMetrics, makePrediction, j);
		// .doAll(names.length, Vec.T_NUM, adaptFrm);
	}

	/**
	 * Score an already adapted frame. Returns a new Frame with new result vectors,
	 * all in the DKV. Caller responsible for deleting. Input is already adapted to
	 * the Model's domain, so the output is also. Also computes the metrics for this
	 * frame.
	 *
	 * @param adaptFrm
	 *            Already adapted frame
	 * @param computeMetrics
	...
	protected Frame predictScoreImpl(Frame fr, Frame adaptFrm, String destination_key, Job j, boolean computeMetrics) {
		...
		domains[0] = names.length == 1 ? null
				: !computeMetrics ? _output._domains[_output._domains.length - 1] : adaptFrm.lastVec().domain();

		// Score the dataset, building the class distribution & predictions
		BigScore bs = makeBigScoreTask(domains, names, adaptFrm, computeMetrics, true, j).doAll(names.length, Vec.T_NUM,
				adaptFrm);
	...
	/**
	 * Score an already adapted frame. Returns a MetricBuilder that can be used to
	 * make a model metrics.
	 * 
	 * @param adaptFrm
	 *            Already adapted frame
	...
	protected ModelMetrics.MetricBuilder scoreMetrics(Frame adaptFrm) {
		final boolean computeMetrics = (!isSupervised()
				|| (adaptFrm.vec(_output.responseName()) != null && !adaptFrm.vec(_output.responseName()).isBad()));
		// Build up the names & domains.
		// String[] names = makeScoringNames();
		String[][] domains = new String[1][];
		domains[0] = _output.nclasses() == 1 ? null
				: !computeMetrics ? _output._domains[_output._domains.length - 1] : adaptFrm.lastVec().domain();
	...
	protected class BigScore extends MRTask<BigScore> {
		...
		final protected int _npredcols; // Number of columns in prediction; nclasses+1 - can be less than the prediction
										// domain
		...
		public BigScore(String[] domain, int ncols, double[] mean, boolean testHasWeights, boolean computeMetrics,
				boolean makePreds, Job j) {
			_j = j;
			_domain = domain;
			_npredcols = ncols;
			_mean = mean;
		...
		public void map(Chunk chks[], NewChunk cpreds[]) {
			...
			try {
				...
				for (int row = 0; row < len; row++) {
					...
					if (weight == 0) {
						if (_makePreds) {
							for (int c = 0; c < _npredcols; c++) // Output predictions; sized for train only (excludes
																	// extra test classes)
								cpreds[c].addNum(0);
						}
						continue;
					...
					if (_makePreds) {
						for (int c = 0; c < _npredcols; c++) // Output predictions; sized for train only (excludes extra
																// test classes)
							cpreds[c].addNum(p[c]);
					}
				}
	...
	// OVerride this if your model needs data preprocessing (on the fly
	// standardization, NA handling)
	protected double data(Chunk[] chks, int row, int col) {
		return chks[col].atd(row);
	}

	/**
	 * Bulk scoring API for one row. Chunks are all compatible with the model, and
	 * expect the last Chunks are for the final distribution and prediction. Default
	 * method is to just load the data into the tmp array, then call subclass
	 * scoring logic.
	 */
	...
	public double[] score0(Chunk chks[], double offset, int row_in_chunk, double[] tmp, double[] preds) {
		...
		if (isSupervised()) {
			// Correct probabilities obtained from training on oversampled data back to
			...
			if (_output.isClassifier()) {
				...
				scored[0] = hex.genmodel.GenModel.getPrediction(scored, _output._priorClassDist, tmp,
						defaultThreshold());
			}
		}
		return scored;
	...
	/**
	 * Subclasses implement the scoring logic. The data is pre-loaded into a re-used
	 * temp array, in the order the model expects. The predictions are loaded into
	 * the re-used temp array, which is also returned.
	 */
	...
	public double score(double[] data) {
		return ArrayUtils.maxIndex(score0(data, new double[_output.nclasses()]));
	}
	...
	protected SBPrintStream toJava(SBPrintStream sb, boolean isGeneratingPreview, boolean verboseCode) {
		...
		sb.p("  ").nl();
		sb.p("  Standalone prediction code with sample test data for ").p(this.getClass().getSimpleName()).p(" named ")
				.p(modelName).nl();
		sb.nl();
		...
		sb.p("      curl http:/").p(H2O.SELF.toString()).p("/3/Models.java/").pobj(_key).p(" > ").p(modelName)
				.p(".java").nl();
		...
		// sb.p("// java -cp h2o-genmodel.jar:. -Xmx2g -XX:MaxPermSize=256m
		// -XX:ReservedCodeCacheSize=256m ").p(modelName).nl();
		sb.nl();
		...
		sb.ip("public hex.ModelCategory getModelCategory() { return hex.ModelCategory." + _output.getModelCategory()
				+ "; }").nl();
	...
	protected SBPrintStream toJavaNCLASSES(SBPrintStream sb) {
		return _output.isClassifier() ? JCodeGen.toStaticVar(sb, "NCLASSES", _output.nclasses(),
				"Number of output classes included in training data response column.") : sb;
	}
	...
	private SBPrintStream toJavaDOMAINS(SBPrintStream sb, CodeGeneratorPipeline fileCtx) {
		...
		for (int i = 0; i < domains.length; i++) {
			...
			// since it does not hold any interesting information except String array
			// holding domain
			if (dom != null) {
				fileCtx.add(new CodeGenerator() {
					@Override
	...
	protected SBPrintStream toJavaPROB(SBPrintStream sb) {
		if (isSupervised()) {
			...
			JCodeGen.toStaticVar(sb, "MODEL_CLASS_DISTRIB", _output._modelClassDist,
	...
	// Method returns code which should be appended into generated top level class
	// after
	// predict method.
	protected void toJavaPredictBody(SBPrintStream body, CodeGeneratorPipeline classCtx, CodeGeneratorPipeline fileCtx,
			boolean verboseCode) {
		throw new IllegalArgumentException("This model type does not support conversion to Java");
	}

	// Wrapper around the main predict call, including the signature and return
	// value
	private SBPrintStream toJavaPredict(SBPrintStream ccsb, CodeGeneratorPipeline fileCtx, boolean verboseCode) { // ccsb
																													// =
																													// classContext
		ccsb.nl();
		...
		try {
			...
			if (_output.isClassifier()) {
				...
				if (sdomain != null && !Arrays.equals(mdomain, sdomain)) {
					omap = CategoricalWrappedVec.computeMap(mdomain, sdomain); // Map from model-domain to
																				// scoring-domain
				}
			}
				...
				for (int row = 0; row < fr.numRows(); row++) { // For all rows, single-threaded
					...
					for (int col = _output.isClassifier() ? 1 : 0; col < pvecs.length; col++) { // Compare predictions
						...
						if (!MathUtils.compare(predictions[col], d, abs_epsilon, rel_epsilon)) {
							if (num_errors++ < 10) {
								System.err.println("Predictions mismatch, row " + row + ", col "
										+ model_predictions._names[col] + ", internal prediction=" + d
										+ ", POJO prediction=" + predictions[col]);
							}
							break;
						}
					}
			...
			for (int i = 0; i < 2; ++i) {
				...
				EasyPredictModelWrapper epmw = new EasyPredictModelWrapper(new EasyPredictModelWrapper.Config()
						.setModel(genmodel).setConvertUnknownCategoricalLevelsToNa(true));
				...
				for (int row = 0; row < fr.numRows(); row++) { // For all rows, single-threaded
					...
					for (int col = 0; col < features.length; col++) {
						if (dvecs[col].isString()) {
							...
							rowData.put(genmodel._names[col],
									genmodel._domains[col] == null ? (Double) val
											: Double.isNaN(val) ? val // missing categorical values are kept as NaN, the
																		// score0 logic passes it on to bitSetContains()
													: (int) val < genmodel._domains[col].length
															? genmodel._domains[col][(int) val]
															: "UnknownLevel"); // unseen levels are treated as such
						}
					}
					...
					for (int col = genmodel.isClassifier() ? 1 : 0; col < pvecs.length; col++) {
						if (!MathUtils.compare(actual_preds[col], expected_preds[col], abs_epsilon, rel_epsilon)) {
							num_errors++;
							if (num_errors < 20) {
								System.err.println((i == 0 ? "POJO" : "MOJO")
										+ " EasyPredict Predictions mismatch for row " + rowData);
			...
			if (num_errors != 0) {
				System.err.println(
						"Number of errors: " + num_errors + (num_errors > 20 ? " (only first 20 are shown)" : "")
	...
	public static Frame makeInteractions(Frame fr, boolean valid, InteractionPair[] interactions,
			boolean useAllFactorLevels, boolean skipMissing, boolean standardize) {
		...
		for (InteractionPair ip : interactions) {
			...
			InteractionWrappedVec iwv = new InteractionWrappedVec(anyTrainVec.group().addVec(), anyTrainVec._rowLayout,
					ip._v1Enums, ip._v2Enums, useAllFactorLevels, skipMissing, standardize, fr.vec(ip._v1)._key,
					fr.vec(ip._v2)._key);
	...
	public static InteractionWrappedVec[] makeInteractions(Frame fr, InteractionPair[] interactions,
			boolean useAllFactorLevels, boolean skipMissing, boolean standardize) {
		Vec anyTrainVec = fr.anyVec();
		InteractionWrappedVec[] interactionVecs = new InteractionWrappedVec[interactions.length];
		int idx = 0;
		for (InteractionPair ip : interactions)
			interactionVecs[idx++] = new InteractionWrappedVec(anyTrainVec.group().addVec(), anyTrainVec._rowLayout,
					ip._v1Enums, ip._v2Enums, useAllFactorLevels, skipMissing, standardize, fr.vec(ip._v1)._key,
					fr.vec(ip._v2)._key);
		return interactionVecs;
	}

	public static InteractionWrappedVec makeInteraction(Frame fr, InteractionPair ip, boolean useAllFactorLevels,
			boolean skipMissing, boolean standardize) {
		Vec anyVec = fr.anyVec();
		return new InteractionWrappedVec(anyVec.group().addVec(), anyVec._rowLayout, ip._v1Enums, ip._v2Enums,
				useAllFactorLevels, skipMissing, standardize, fr.vec(ip._v1)._key, fr.vec(ip._v2)._key);
	}

	/**
	 * This class represents a pair of interacting columns plus some additional data
	 * about specific enums to be interacted when the vecs are categorical. The
	 * question naturally arises why not just use something like an ArrayList of
	 * int[2] (as is done, for example, in the Interaction/CreateInteraction
	 * classes) and the answer essentially boils down a desire to specify these
	 * specific levels.
	 *
	 * Another difference with the CreateInteractions class: 1. do not interact on
	 * NA (someLvl_NA and NA_somLvl are actual NAs) this does not appear here, but
	 * in the InteractionWrappedVec class TODO: refactor the CreateInteractions to
	 * be useful here and in InteractionWrappedVec
	 */
	public static class InteractionPair extends Iced {
		...
		private InteractionPair(int v1, int v2, String[] v1Enums, String[] v2Enums) {
			...
			if (_v1Enums == null) {
				...
				for (String s : _v1Enums)
					_hash = 31 * _hash + s.hashCode();
			if (_v2Enums == null) {
				_hash = 31 * _hash;
			} else {
				for (String s : _v2Enums)
					_hash = 31 * _hash + s.hashCode();
			}
		}
		...
		public static InteractionPair[] generatePairwiseInteractions(int from, int to) {
			if (1 == (to - from)) {
				throw new IllegalArgumentException(
						"Illegal range of values, must be greater than a single value. Got: " + from + "<" + to);
			}
		...
		/**
		 * Set the domain; computed in an MRTask over the two categorical vectors that
		 * make up this interaction pair
		 * 
		 * @param dom
		 *            The domain retrieved by the CombineDomainTask in
		...
		private int parseNum() {
			int start = _p++;
			while (_p < _str.length() && '0' <= _str.charAt(_p) && _str.charAt(_p) <= '9')
				_p++;
			try {
		...
		@Override
		public String toString() {
			return _v1 + (_v1Enums == null ? "" : Arrays.toString(_v1Enums)) + ":" + _v2
					+ (_v2Enums == null ? "" : Arrays.toString(_v2Enums));
		}
		...
		public boolean equals(Object o) {
			...
			if (res) {
				...
				return (_v1 == ip._v1) && (_v2 == ip._v2) && Arrays.equals(_v1Enums, ip._v1Enums)
						&& Arrays.equals(_v2Enums, ip._v2Enums);
			}
			return false;
		}
	}
