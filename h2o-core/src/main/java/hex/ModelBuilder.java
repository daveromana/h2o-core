...
/**
 * Model builder parent class. Contains the common interfaces and fields across
 * all model builders.
 */
abstract public class ModelBuilder<M extends Model<M, P, O>, P extends Model.Parameters, O extends Model.Output>
		extends Iced {
	...
	public boolean shouldReorder(Vec v) {
		return _parms._categorical_encoding.needsResponse() && isSupervised();
	}
	 ...
	 * funny assert: the Job does NOT have to be controlling this model build, but
	 * might, e.g. be controlling a Grid search for which this is just one of many
	 * results. Calling 'get' means that we are blocking on the Job which is
	 * controlling ONLY this ModelBuilder, and when the Job completes we can return
	 * built Model.
	 */
	public final M get() {
	...
	protected boolean timeout() {
		...
		return _parms._max_runtime_secs > 0
				&& System.currentTimeMillis() - _start_time > (long) (_parms._max_runtime_secs * 1e3);
	}
	...
	/**
	 * List of known ModelBuilders with all default args; endlessly cloned by the
	 * GUI for new private instances, then the GUI overrides some of the defaults
	 * with user args.
	 */
	...
	public static String algoName(String urlName) {
		return BUILDERS[ArrayUtils.find(ALGOBASES, urlName)]._parms.algoName();
	}

	/** gbm -> hex.tree.gbm.GBM, deeplearning -> hex.deeplearning.DeepLearning */
	public static String javaName(String urlName) {
		return BUILDERS[ArrayUtils.find(ALGOBASES, urlName)]._parms.javaName();
	}

	/** gbm -> GBMParameters */
	public static String paramName(String urlName) {
		return algoName(urlName) + "Parameters";
	}
	...
	/**
	 * Training frame: derived from the parameter's training frame, excluding all
	 * ignored columns, all constant and bad columns, perhaps flipping the response
	 * column to an Categorical, etc.
	 */
	...
	// Map the algo name (e.g., "deeplearning") to the builder class (e.g.,
	// DeepLearning.class) :
	private static final Map<String, Class<? extends ModelBuilder>> _builders = new HashMap<>();

	// Map the Model class (e.g., DeepLearningModel.class) to the algo name (e.g.,
	// "deeplearning"):
	private static final Map<Class<? extends Model>, String> _model_class_to_algo = new HashMap<>();

	// Map the simple algo name (e.g., deeplearning) to the full algo name (e.g.,
	// "Deep Learning"):
	private static final Map<String, String> _algo_to_algo_full_name = new HashMap<>();

	// Map the algo name (e.g., "deeplearning") to the Model class (e.g.,
	...
	public Vec vresponse() {
		return _vresponse == null ? _response : _vresponse;
	}
	...
	abstract protected class Driver extends H2O.H2OCountedCompleter<Driver> {
		...
		// Pull the boilerplate out of the computeImpl(), so the algo writer doesn't
		// need to worry about the following:
		// 1) Scope (unless they want to keep data, then they must call
		// Scope.untrack(Key<Vec>[]))
	...
	final public Job<M> trainModel() {
		...
		// cross-validation needs to be forked off to allow continuous (non-blocking)
		// progress bar
	 ...
	 *
	 * @param fr:
	 *            Input frame override, ignored if null. In some cases, algos do not
	 *            work directly with the original frame in the K/V store. Instead
	 *            they run on a private anonymous copy (eg: reblanced dataset). Use
	 *            this argument if you want nested job to work on the actual working
	 *            copy rather than the original Frame in the K/V. Example: Outer job
	 *            rebalances dataset and then calls nested job. To avoid needless
	 *            second reblance, pass in the (already rebalanced) working copy.
	 */
	final public M trainModelNested(Frame fr) {
		if (fr != null) {// Use the working copy (e.g. rebalanced) instead of the original K/V store
							// version
			setTrain(fr);
		}
		if (error_count() > 0) {
	...
	/**
	 * How many should be trained in parallel during N-fold cross-validation? Train
	 * all CV models in parallel when parallelism is enabled, otherwise train one at
	 * a time Each model can override this logic, based on parameters, dataset size,
	 * etc.
	 * 
	...
	protected int nModelsInParallel() {
		if (!_parms._parallelize_cross_validation || _parms._max_runtime_secs != 0) {
			return 1; // user demands serial building (or we need to honor the time constraints for
						// all CV models equally)
		}
	...
	/**
	 * Default naive (serial) implementation of N-fold cross-validation (builds N+1
	 * models, all have train+validation metrics, the main model has N-fold
	 * cross-validated validation metrics)
	 */
	public void computeCrossValidation() {
		...
		try {
			...
			// Step 3: Build N train & validation frames; build N ModelBuilders; error check
			// them all
	...
	public Vec cv_AssignFold(int N) {
		...
		if (fold != null) {
			if (!fold.isInt() || (!(fold.min() == 0 && fold.max() == N - 1) && !(fold.min() == 1 && fold.max() == N))) { // Allow
																															// 0
																															// to
																															// N-1,
																															// or
				...
				throw new H2OIllegalArgumentException(
						"Fold column must be either categorical or contiguous integers from 0..N-1 or 1..N");
			}
			return fold;
		}
	...
	public Vec[] cv_makeWeights(final int N, Vec foldAssignment) {
		...
		if (_parms._keep_cross_validation_fold_assignment) {
			DKV.put(new Frame(Key.<Frame>make("cv_fold_assignment_" + _result.toString()),
					new String[] { "fold_assignment" }, new Vec[] { foldAssignment.makeCopy() }));
		}
		...
		for (Vec weight : weights)
			if (weight.isConst()) {
				throw new H2OIllegalArgumentException("Not enough data to create " + N
						+ " random cross-validation splits. Either reduce nfolds, specify a larger dataset (or specify another random number seed, if applicable).");
			}
		return weights;
	}
	...
	public ModelBuilder<M, P, O>[] cv_makeFramesAndBuilders(int N, Vec[] weights) {
		...
		for (int i = 0; i < N; i++) {
			...
			cv_mb._parms._weights_column = weightName;// All submodels have a weight column, which the main model does
														// not
	 ...
	 *
	 * @param modelType
	 *            text description of group of models being built (for logging
	 *            purposes)
	 * @param job
	 *            parent job (processing will be stopped if stop of a parent job was
	 *            requested)
	 * @param modelBuilders
	 *            list of model builders to run in bulk
	 * @param parallelization
	 *            level of parallelization (how many models can be built at the same
	 *            time)
	 * @param updateInc
	 *            update increment (0 = disable updates)
	 */
	...
	public ModelMetrics.MetricBuilder[] cv_scoreCVModels(int N, Vec[] weights,
		...
		for (int i = 0; i < N; ++i) {
			...
			if (nclasses() == 2
					/* need holdout predictions for gains/lift table */ || _parms._keep_cross_validation_predictions
					|| (_parms._distribution == DistributionFamily.huber /*
																			 * need to compute quantiles on abs error of
																			 * holdout predictions
																			 */)) {
	...
	// Step 6: Combine cross-validation scores; compute main model x-val scores;
	// compute gains/lifts
	public void cv_mainModelScores(int N, ModelMetrics.MetricBuilder mbs[], ModelBuilder<M, P, O> cvModelBuilders[]) {
		...
		for (int i = 0; i < N; ++i) {
			...
			predKeys[i] = Key.make("prediction_" + cvModelKey.toString()); // must be the same as in cv_scoreCVModels
																			// above
		}
		Frame holdoutPreds = null;
		if (_parms._keep_cross_validation_predictions
				|| (nclasses() == 2 /* GainsLift needs this */ || _parms._distribution == DistributionFamily.huber)) {
		...
		if (_parms._keep_cross_validation_fold_assignment) {
			mainModel._output._cross_validation_fold_assignment_frame_id = Key
					.make("cv_fold_assignment_" + _result.toString());
			Frame xvalidation_fold_assignment_frame = mainModel._output._cross_validation_fold_assignment_frame_id
					.get();
		...
		mainModel._output._cross_validation_metrics = mbs[0].makeModelMetrics(mainModel, _parms.train(), null,
				holdoutPreds);
		...
		mainModel._output._cross_validation_metrics._description = N
				+ "-fold cross-validation on training data (Metrics computed for combined holdout predictions)";
		Log.info(mainModel._output._cross_validation_metrics.toString());

		mainModel._output._cross_validation_metrics_summary = makeCrossValidationSummaryTable(
				mainModel._output._cross_validation_models);
	...
	/**
	 * Override for model-specific checks / modifications to _parms for the main
	 * model during N-fold cross-validation. Also allow the cv models to be modified
	 * after all of them have been built. For example, the model might need to be
	 * told to not do early stopping. CV models might have their lambda value
	 * modified, etc.
	 */
	...
	/**
	 * List containing the categories of models that this builder can build. Each
	 * ModelBuilder must have one of these.
	 */
	abstract public ModelCategory[] can_build();

	/**
	 * Visibility for this algo: is it always visible, is it beta (always visible
	 * but with a note in the UI) or is it experimental (hidden by default, visible
	 * in the UI if the user gives an "experimental" flag at startup); test-only
	...
	public boolean hasOffsetCol() {
		return _parms._offset_column != null;
	} // don't look at transient Vec

	public boolean hasWeightCol() {
		return _parms._weights_column != null;
	} // don't look at transient Vec

	public boolean hasFoldCol() {
		return _parms._fold_column != null;
	} // don't look at transient Vec
	...
	// no hasResponse, call isSupervised instead (response is mandatory if
	// isSupervised is true)
	...
	public int separateFeatureVecs() {
		...
		if (_parms._weights_column != null) {
			...
			if (w == null) {
				error("_weights_column",
						"Weights column '" + _parms._weights_column + "' not found in the training frame");
			} else {
				if (!w.isNumeric()) {
					error("_weights_column",
							"Invalid weights column '" + _parms._weights_column + "', weights must be numeric");
				}
				_weights = w;
				if (w.naCnt() > 0) {
		...
		if (_parms._offset_column != null) {
			...
			if (o == null) {
				error("_offset_column",
						"Offset column '" + _parms._offset_column + "' not found in the training frame");
			} else {
				if (!o.isNumeric()) {
					error("_offset_column",
							"Invalid offset column '" + _parms._offset_column + "', offset must be numeric");
				}
				_offset = o;
				if (o.naCnt() > 0) {
		...
		if (_parms._fold_column != null) {
			...
			if (f == null) {
				...
				if (!f.isInt() && !f.isCategorical()) {
					error("_fold_column",
							"Invalid fold column '" + _parms._fold_column + "', fold must be integer or categorical");
				}
				if (f.min() < 0) {
					error("_fold_column",
							"Invalid fold column '" + _parms._fold_column + "', fold must be non-negative");
				}
				if (f.isConst()) {
			...
			if (_response == null) {
				if (isSupervised()) {
					error("_response_column",
							"Response column '" + _parms._response_column + "' not found in the training frame");
				}
			} else {
	...
	/**
	 * Ignore invalid columns (columns that have a very high max value, which can
	 * cause issues in DHistogram)
	 * 
	 * @param npredictors
	 * @param expensive
	 */
	 ...
	 *
	 * Note: This method should not be overridden (override
	 * checkMemoryFootPrint_impl instead). It is not declared 'final' to not to
	 * break 3rd party implementations. It might be declared final in the future if
	 * necessary.
	 */
	...
	/**
	 * Override this method to call error() if the model is expected to not fit in
	 * memory, and say why
	 */
	...
	private int _error_count = -1; // -1 ==> init not run yet, for those Jobs that have an init, like ModelBuilder.
									// Note, this counts ONLY errors, not WARNs and etc.

	public int error_count() {
		assert _error_count >= 0 : "init() not run yet";
		return _error_count;
	}

	public void hide(String field_name, String message) {
		message(Log.TRACE, field_name, message);
	}

	public void info(String field_name, String message) {
		message(Log.INFO, field_name, message);
	}

	public void warn(String field_name, String message) {
		message(Log.WARN, field_name, message);
	}

	public void error(String field_name, String message) {
		message(Log.ERRR, field_name, message);
		_error_count++;
	}
	...
	/**
	 * Get a string representation of only the ERROR ValidationMessages (e.g., to
	 * use in an exception throw).
	 */
	...
	/**
	 * Can be an ERROR, meaning the parameters can't be used as-is, a TRACE, which
	 * means the specified field should be hidden given the values of other fields,
	 * or a WARN or INFO for informative messages to the user.
	 */
	public static final class ValidationMessage extends Iced {
		...
		public String toString() {
			return Log.LVLS[_log_level] + " on field: " + _field_name + ": " + _message;
		}
	}
	 ...
	 * training frame. This call is expected to be overridden in the subclasses and
	 * each subclass will start with "super.init();". This call is made by the
	 * front-end whenever the GUI is clicked, and needs to be fast whenever
	 ...
	 * <p>
	 * The incoming training frame (and validation frame) will have ignored columns
	 * dropped out, plus whatever work the parent init did.
	 * <p>
	 * NOTE: The front end initially calls this through the parameters validation
	 * endpoint with no training_frame, so each subclass's {@code init()} method has
	 * to work correctly with the training_frame missing.
	 * <p>
	 */
	public void init(boolean expensive) {
		...
		if (isSupervised()) {
			...
			if (!_parms._balance_classes) {
				...
				error("_balance_classes",
						"Balance classes and observation weights are not currently supported together.");
			}
			...
			if (_train != null) {
				...
				if (_response != null && computePriorClassDistribution()) {
					if (isClassifier() && isSupervised()) {
						MRUtils.ClassDist cdmt = _weights != null
								? new MRUtils.ClassDist(nclasses()).doAll(_response, _weights)
								: new MRUtils.ClassDist(nclasses()).doAll(_response);
			...
			if (!isClassifier()) {
				...
				hide("_class_sampling_factors",
						"Class sampling factors is only applicable to classification problems.");
				hide("_max_after_balance_size",
						"Max after balance size is only applicable to classification problems.");
				hide("_max_confusion_matrix_size",
						"Max confusion matrix size is only applicable to classification problems.");
			}
			if (_nclass <= 2) {
				hide("_max_hit_ratio_k",
						"Max K-value for hit ratio is only applicable to multi-class classification problems.");
		...
		if (_nclass > Model.Parameters.MAX_SUPPORTED_LEVELS) {
			error("_nclass", "Too many levels in response column: " + _nclass
					+ ", maximum supported number of classes is " + Model.Parameters.MAX_SUPPORTED_LEVELS + ".");
		}
		...
		if (expensive) {
			...
			if (_valid != null) {
				_valid = encodeFrameCategoricals(_valid,
						!_parms._is_cv_model /* for CV, need to score one more time in outer loop */);
				_vresponse = _valid.vec(_parms._response_column);
			}
			...
			for (int j = 0; j < vecs.length; ++j) {
				...
				if (v.isCategorical() && shouldReorder(v)) {
					...
					Log.info("Reordering categorical column " + _train.name(j) + " (" + len
							+ " levels) based on the mean (weighted) response per level.");
					...
					for (int i = 0; i < len; ++i)
						invIdx[idx[i]] = i;
					Vec vNew = new VecUtils.ReorderTask(invIdx).doAll(1, Vec.T_NUM, new Frame(v)).outputFrame()
							.anyVec();
		...
		assert (!expensive || _valid == null || Arrays.equals(_train._names, _valid._names)
				|| _parms._categorical_encoding == Model.Parameters.CategoricalEncodingScheme.Binary);
		if (_valid != null && !Arrays.equals(_train._names, _valid._names)
				&& _parms._categorical_encoding == Model.Parameters.CategoricalEncodingScheme.Binary) {
			for (String name : _train._names)
				assert (ArrayUtils.contains(_valid._names,
						name)) : "Internal error during categorical encoding: training column " + name
								+ " not in validation frame with columns " + Arrays.toString(_valid._names);
		}
		...
		if (_parms._stopping_rounds == 0) {
			...
			if (isClassifier()) {
				if (_parms._stopping_metric == ScoreKeeper.StoppingMetric.deviance
						&& !getClass().getSimpleName().contains("GLM")) {
				...
				if (_parms._stopping_metric == ScoreKeeper.StoppingMetric.misclassification
					...
					error("_stopping_metric",
							"Stopping metric cannot be " + _parms._stopping_metric.toString() + " for regression.");
				}
			}
		}
	...
	/**
	 * Adapts a given frame to the same schema as the training frame. This includes
	 * encoding of categorical variables (if expensive is enabled).
	 *
	 * Note: This method should only be used during ModelBuilder initialization - it
	 * should be called in init(..) method.
	 *
	 * @param fr
	 *            input frame
	 * @param frDesc
	 *            frame description, eg. "Validation Frame" - will be shown in
	 *            validation error messages
	 * @param field
	 *            name of a field for validation errors
	 * @param expensive
	 *            indicates full ("expensive") processing
	...
	private Frame adaptFrameToTrain(Frame fr, String frDesc, String field, boolean expensive) {
		...
		try {
			String[] msgs = Model.adaptTestForTrain(adapted, null, null, _train._names, _train.domains(), _parms,
					expensive, true, null, getToEigenVec(), _toDelete, false);
	...
	private Frame encodeFrameCategoricals(Frame fr, boolean scopeTrack) {
		String[] skipCols = new String[] { _parms._weights_column, _parms._offset_column, _parms._fold_column,
	 ...
	 *            Input frame
	 * @param local
	 *            Whether to only create enough chunks to max out all cores on one
	 *            node only
	 * @param name
	 *            Name of rebalanced frame
	...
	protected Frame rebalance(final Frame original_fr, boolean local, final String name) {
		...
		if (original_fr.anyVec().nChunks() >= chunks) {
			if (chunks > 1) {
				Log.info(name.substring(name.length() - 5) + " dataset already contains "
						+ original_fr.anyVec().nChunks() + " chunks. No need to rebalance.");
			}
			return original_fr;
		}
	...
	private static Frame combineHoldoutPredictions(Key<Frame>[] predKeys, Key key) {
		...
		return new HoldoutPredictionCombiner(N, template.numCols()).doAll(template.types(), new Frame(vecs))
				.outputFrame(key, template.names(), template.domains());
	}

	// helper to combine multiple holdout prediction Vecs (each only has 1/N-th
	// filled with non-zeros) into 1 Vec
	...
	private TwoDimTable makeCrossValidationSummaryTable(Key[] cvmodels) {
		...
		TwoDimTable table = new TwoDimTable("Cross-Validation Metrics Summary", null, rowNames.toArray(new String[0]),
				colNames, colTypes, colFormats, "");
