...
abstract public class ModelBuilder<M extends Model<M,P,O>, P extends Model.Parameters, O extends Model.Output> extends Iced {
  ...
  final public M trainModelNested(Frame fr) {
    if(fr != null) {
    	setTrain(fr);// Use the working copy (e.g. rebalanced) instead of the original K/V store version
    }
    ...
    if( fold != null ) {
      if( !fold.isInt() ||
          (!(fold.min() == 0 && fold.max() == N-1) &&
           !(fold.min() == 1 && fold.max() == N  ) )) {
    	  throw new H2OIllegalArgumentException("Fold column must be either categorical or contiguous integers from 0..N-1 or 1..N");// Allow 0 to N-1, or 1 to N
      }
