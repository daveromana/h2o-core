...
public class ModelMetricsClustering extends ModelMetricsUnsupervised {
	...
	public TwoDimTable createCentroidStatsTable() {
		...
		TwoDimTable table = new TwoDimTable("Centroid Statistics", null, new String[K],
	...
	public static class MetricBuilderClustering extends MetricBuilderUnsupervised<MetricBuilderClustering> {
		...
		public double[] perRow(double[] preds, float[] dataRow, Model m) {
			...
			double[][] centers = standardize ? ((ClusteringOutput) clm._output)._centers_std_raw
					: ((ClusteringOutput) clm._output)._centers_raw;
			...
			double sqr = hex.genmodel.GenModel.KMeans_distance(centers[clus], dataRow,
					((ClusteringOutput) clm._output)._mode, colSum, colSumSq);
			...
			if (Double.isNaN(_sumsqe)) {
				throw new H2OIllegalArgumentException(
						"Sum of Squares is invalid (Double.NaN) - Check for missing values in the dataset.");
			}
