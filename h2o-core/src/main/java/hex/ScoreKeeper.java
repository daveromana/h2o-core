...
/**
 * Low-weight keeper of scores solely intended for display (either direct or as
 * helper to create scoring history TwoDimTable). Not intended to store large
 * AUC object or ConfusionMatrices, etc.
 */
public class ScoreKeeper extends Iced {
	...
	/**
	 * Keep score for a model using its validation_metrics if available and
	 * training_metrics if not.
	 * 
	 * @param m
	 *            model for which we should keep score
	 */
	...
	public enum StoppingMetric {
		AUTO, deviance, logloss, MSE, RMSE, MAE, RMSLE, AUC, lift_top_group, misclassification, mean_per_class_error
	}
	...
	/**
	 * Based on the given array of ScoreKeeper and stopping criteria should we stop
	 * early?
	 */
	public static boolean stopEarly(ScoreKeeper[] sk, int k, boolean classification, StoppingMetric criterion,
			double rel_improvement, String what, boolean verbose) {
		if (k == 0) {
			return false;
		}
		int len = sk.length - 1; // how many "full"/"conservative" scoring events we have (skip the first)
		if (len < 2 * k) {
			return false; // need at least k for SMA and another k to tell whether the model got better or
							// not
		}
		...
		for (int i = 0; i < movingAvg.length; ++i) {
			...
			// movingAvg[1] is based on scoring events indices 15,16,17 <- first "new"
			// smooth score
			// movingAvg[2] is based on scoring events indices 16,17,18 <- second "new"
			// smooth score
			// movingAvg[3] is based on scoring events indices 17,18,19 <- third "new"
			// smooth score
			...
			// movingAvg[1] is based on scoring events indices 15,16 <- first "new" smooth
			// score
			// movingAvg[2] is based on scoring events indices 16,17 <- second "new" smooth
			// score
		...
		// zero-crossing could be for residual deviance or r^2 -> mark it not yet
		...
		if (verbose) {
			Log.info("Windowed averages (window size " + k + ") of " + what + " " + (k + 1) + " " + criterion.toString()
					+ " metrics: " + Arrays.toString(movingAvg));
		}
		...
		if (verbose) {
			Log.info("Checking convergence with " + criterion.toString() + " metric: " + lastBeforeK + " --> "
					+ bestInLastK + (improved ? " (still improving)." : " (converged)."));
		}
		return !improved;
	} // stopEarly
