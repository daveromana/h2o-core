...
/**
 * A ByteBuffer backed mixed Input/Output streaming class, using Iced
 * serialization.
 *
 * Reads/writes empty/fill the ByteBuffer as needed. When it is empty/full it we
 * go to the ByteChannel for more/less. Because DirectByteBuffers are expensive
 * to make, we keep a few pooled.
 *
 * When talking to a remote H2O node, switches between UDP and TCP transport
 * protocols depending on the message size. The TypeMap is not included, and is
 * assumed to exist on the remote H2O node.
 *
 ...
 * current map on read.
 *
 * Support read/write from byte[] - and this defeats the purpose of a Streaming
 * protocol, but is frequently handy for small structures. The TypeMap is not
 * included, and is assumed to be the current map on read.
 *
...
public final class AutoBuffer {
	...
	// TRUE for read-mode. FALSE for write-mode. Can be flipped for rapid
	// turnaround.
	...
	// I/O persistence flavor: Value.ICE, NFS, HDFS, S3, TCP. Used to record I/O
	// time.
	final byte _persist;
	 ...
	 * figure the originating H2ONode from the first few bytes read.
	 *
	 * remoteAddress set to null means that the communication is originating from
	 * non-h2o node, non-null value represents the case where the communication is
	 * coming from h2o node.
	 */
	AutoBuffer(ByteChannel sock, InetAddress remoteAddress) throws IOException {
		...
		if (remoteAddress != null) {
			...
			// In case the communication originates from non-h2o node, we set _h2o node to
			// null.
			...
			// - H2OIntern also reads port (getPort()) and additional information which we
			// do not send
	...
	/**
	 * Make an AutoBuffer to write to an H2ONode. Requests for full buffer will open
	 * a TCP socket and roll through writing to the target. Smaller requests will
	 * send via UDP. Small requests get ordered by priority, so that e.g. NACK and
	 * ACKACK messages have priority over most anything else. This helps in UDP
	 * floods to shut down flooding senders.
	 */
	...
	/**
	 * Read from UDP multicast. Same as the byte[]-read variant, except there is an
	 * H2O.
	 */
	...
	/**
	 * Write to an ever-expanding byte[]. Instead of calling {@link #close()}, call
	 * {@link #buf()} to retrieve the final byte[].
	 */
	public AutoBuffer() {
	...
	/**
	 * Read from a persistent Stream (including all TypeMap info) into same exact
	 * rev of H2O).
	 */
	public AutoBuffer(InputStream is) {
		...
		if (!version.equals(H2O.ABV.projectVersion())) {
			throw new IllegalArgumentException(
					"Found version " + version + ", but running version " + H2O.ABV.projectVersion());
		}
	...
	static class BBPool {
		...
		private ByteBuffer stats(ByteBuffer bb) {
			...
			sb.p("BB").p(this == BBP_BIG ? 1 : 0).p(" made=").p(_made).p(" -freed=").p(_freed).p(", cache hit=")
					.p(_cached).p(" ratio=").p(_numer / _denom).p(", goal=").p(_goal).p(" cache size=").p(_bbs.size())
					.nl();
		...
		void free(ByteBuffer bb) {
			...
			synchronized (_bbs) {
				if (ratio < 100 || _bbs.size() < _goal) { // low hit/miss ratio or below goal
					...
					_freed++; // Toss the extras (above goal & ratio)
				...
				if (now - _lastGoal > 1000000000L) { // Once/sec, drop goal by 10%
					...
					// Once/sec, lower numer/denom... means more recent activity outweighs really
					// old stuff
			...
			if (hasTCP()) { // TCP connection?
				try {
					if (_read) { // Reader?
						...
						// either TCP con was dropped or other side closed connection without
						// reading/confirming (e.g. task was cancelled).
						if (x == -1) {
					...
					if (!_read) {
						_h2o.freeTCPSocket((ByteChannel) _chan);
					} // Recycle writable TCP channel
	...
	boolean hasTCP() {
		assert !isClosed();
		return SocketChannelUtils.isSocketChannel(_chan) || (_h2o != null && _bb.position() >= MTU);
	}
	 ...
	 * reading off end of the bytebuffer; generally this is easy for array-backed
	 * autobuffers and difficult for i/o-backed bytebuffers.
	 */
	...
	private ByteBuffer getSp(int sz) {
		return sz > _bb.remaining() ? getImpl(sz) : _bb;
	}

	/**
	 * Ensure buffer has at least sz bytes in it. - Also, set position just past
	 * this limit for future reading.
	 */
	...
	private ByteBuffer getImpl(int sz) {
		...
		while (_bb.position() < sz) { // Read until we got enuf
			try {
				...
				if (e.getMessage()
						.equals("An established connection was aborted by the software in your host machine")) {
					throw new AutoBufferException(e);
				}
	...
	private int readAnInt() throws IOException {
		...
		try {
			...
			throw new IOException("Failed reading " + remaining + " bytes into buffer[" + array.length + "] at "
					+ position + " from " + sourceName + " " + _is, ioe);
		}
	}
	...
	private ByteBuffer sendPartial() {
		...
		try {
			...
			while (_bb.hasRemaining()) {
				...
				if (RANDOM_TCP_DROP != null && SocketChannelUtils.isSocketChannel(_chan)
						&& RANDOM_TCP_DROP.nextInt(100) == 0) {
	...
	private ByteBuffer expandByteBuffer(int sizeHint) {
		...
		if ((_h2o == null && _chan == null) || (_bb.hasArray() && needed < MTU)) {
			if (needed > MAX_ARRAY_SIZE) {
				throw new IllegalArgumentException(
						"Cannot allocate more than 2GB array: sizeHint=" + sizeHint + ", " + "needed=" + needed
	...
	int get1U(int off) {
		return _bb.get(off) & 0xFF;
	}

	int get4(int off) {
		return _bb.getInt(off);
	}

	long get8(int off) {
		return _bb.getLong(off);
	}
	...
	public <T extends Freezable> T get(Class<T> tc) {
		...
		assert tc.isInstance(TypeMap.theFreezable(id)) : tc.getName() + " != "
				+ TypeMap.theFreezable(id).getClass().getName() + ", id = " + id;
		return (T) TypeMap.newFreezable(id).read(this);
	}
	...
	// Get a (compressed) integer. See above for the compression strategy and
	// reasoning.
	int getInt() {
		int x = get1U();
		if (x <= 253) {
	...
	int getPort() {
		return getSz(1 + 2).getChar(1);
	}

	// Get the task# in the next 4 bytes
	int getTask() {
		return getSz(1 + 2 + 4).getInt(1 + 2);
	}

	// Get the flag in the next 1 byte
	int getFlag() {
		return getSz(1 + 2 + 4 + 1).get(1 + 2 + 4);
	}
	...
	public AutoBuffer putA4(int[] ary) {
		...
		putInt(ary.length);
		// Note: based on Brandon commit this should improve performance during parse
		// (7d950d622ee3037555ecbab0e39404f8f0917652)
	...
	public AutoBuffer putJNULL() {
		return put1('n').put1('u').put1('l').put1('l');
	}
	...
	private AutoBuffer putJStr(String s) {
		...
		for (int i = 0; i < b.length; i++) {
			...
			// if( b[i] == '/' ) { putA1(b,off,i); put1('\\'); put1('/'); off=i+1;
			// continue;}
			if (b[i] == '\b') {
				putA1(b, off, i);
				put1('\\');
			...
			if (b[i] >= 0 && b[i] < 32) {
				...
				for (int j = 0; j < hexStr.length(); j++)
					put1(hexStr.charAt(hexStr.length() - j - 1));
				off = i + 1;
			}
		}
	...
	public AutoBuffer putJSONStrUnquoted(String s) {
		return s == null ? putJNULL() : putJStr(s);
	}

	public AutoBuffer putJSONStrUnquoted(String name, String s) {
		return s == null ? putJSONStr(name).put1(':').putJNULL() : putJSONStr(name).put1(':').putJStr(s);
	}

	public AutoBuffer putJSONName(String s) {
		return put1('"').putJStr(s).put1('"');
	}

	public AutoBuffer putJSONStr(String s) {
		return s == null ? putJNULL() : putJSONName(s);
	}
	...
	public AutoBuffer putJSONStr(String name, String s) {
		return putJSONStr(name).put1(':').putJSONStr(s);
	}

	@SuppressWarnings("unused")
	public AutoBuffer putJSONAStr(String name, String[] ss) {
		return putJSONStr(name).put1(':').putJSONAStr(ss);
	}

	@SuppressWarnings("unused")
	public AutoBuffer putJSONAAStr(String name, String[][] sss) {
		return putJSONStr(name).put1(':').putJSONAAStr(sss);
	}

	@SuppressWarnings("unused")
	public AutoBuffer putJSONSer(String name, Object o) {
		return putJSONStr(name).put1(':').putJNULL();
	}
	...
	public AutoBuffer putJSONAZ(String name, boolean[] f) {
		return putJSONStr(name).put1(':').putJSONAZ(f);
	}

	public AutoBuffer putJSON(Freezable ice) {
		return ice == null ? putJNULL() : ice.writeJSON(this);
	}
	...
	public AutoBuffer putJSON1(String name, byte b) {
		return putJSONStr(name).put1(':').putJSON1(b);
	}

	@SuppressWarnings("unused")
	public AutoBuffer putJSONA1(String name, byte b[]) {
		return putJSONStr(name).put1(':').putJSONA1(b);
	}

	@SuppressWarnings("unused")
	public AutoBuffer putJSONAA1(String name, byte b[][]) {
		return putJSONStr(name).put1(':').putJSONAA1(b);
	}
	...
	AutoBuffer putJSON2(String name, char c) {
		return putJSONStr(name).put1(':').putJSON2(c);
	}
	...
	public AutoBuffer putJSONA2(String name, short ary[]) {
		return putJSONStr(name).put1(':').putJSONA2(ary);
	}
	...
	AutoBuffer putJSON8(long l) {
		return putJStr(Long.toString(l));
	}
	...
	public AutoBuffer putJSON8(String name, long l) {
		return putJSONStr(name).put1(':').putJSON8(l);
	}

	public AutoBuffer putJSONEnum(String name, Enum e) {
		return putJSONStr(name).put1(':').putJSONEnum(e);
	}

	public AutoBuffer putJSONA8(String name, long ary[]) {
		return putJSONStr(name).put1(':').putJSONA8(ary);
	}

	public AutoBuffer putJSONAA8(String name, long ary[][]) {
		return putJSONStr(name).put1(':').putJSONAA8(ary);
	}

	public AutoBuffer putJSONAAA8(String name, long ary[][][]) {
		return putJSONStr(name).put1(':').putJSONAAA8(ary);
	}
	...
	public AutoBuffer putJSON4(String name, int i) {
		return putJSONStr(name).put1(':').putJSON4(i);
	}

	public AutoBuffer putJSONA4(String name, int[] a) {
		return putJSONStr(name).put1(':').putJSONA4(a);
	}

	public AutoBuffer putJSONAA4(String name, int[][] a) {
		return putJSONStr(name).put1(':').putJSONAA4(a);
	}

	public AutoBuffer putJSONAAA4(String name, int[][][] a) {
		return putJSONStr(name).put1(':').putJSONAAA4(a);
	}

	AutoBuffer putJSON4f(float f) {
		return f == Float.POSITIVE_INFINITY ? putJSONStr(JSON_POS_INF)
				: (f == Float.NEGATIVE_INFINITY ? putJSONStr(JSON_NEG_INF)
						: (Float.isNaN(f) ? putJSONStr(JSON_NAN) : putJStr(Float.toString(f))));
	}

	public AutoBuffer putJSON4f(String name, float f) {
		return putJSONStr(name).put1(':').putJSON4f(f);
	}
	...
	AutoBuffer putJSON8d(double d) {
		...
		return d == Double.POSITIVE_INFINITY ? putJSONStr(JSON_POS_INF)
				: (d == Double.NEGATIVE_INFINITY ? putJSONStr(JSON_NEG_INF)
						: (Double.isNaN(d) ? putJSONStr(JSON_NAN) : putJStr(Double.toString(d))));
	}

	public AutoBuffer putJSON8d(String name, double d) {
		return putJSONStr(name).put1(':').putJSON8d(d);
	}
	...
	public AutoBuffer putJSONAAA8d(String name, double[][][] a) {
		return putJSONStr(name).put1(':').putJSONAAA8d(a);
	}
