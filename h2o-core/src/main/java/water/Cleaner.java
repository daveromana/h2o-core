...
class Cleaner extends Thread {
	...
	/* synchronized */ public void run() {
		...
		while (true) {
			...
			if (force && diskFull) { // Try to clean the diskFull flag
				...
				String s = h + " DESIRED=" + (DESIRED >> 20) + "M dirtysince=" + (now - dirty) + " force=" + force
						+ " clean2age=" + (now - clean_to_age);
			...
			for (int i = 2; i < kvs.length; i += 2) {
				...
				if (isChunk && !val.isPersisted() && !diskFull && ((Key) ok).home()) { // && (force || (lazyPersist() &&
																						// lazy_clean(key)))) {
					...
					try {
						...
						Log.warn(isDiskFull()
								? "Disk full! Disabling swapping to disk."
										+ (force ? " Memory low! Please free some space in " + H2O.ICE_ROOT + "!" : "")
				...
				if (isChunk && force && (val.isPersisted() || !((Key) ok).home())) {
					...
					if (isChunk) {
						freed -= val._max; // Double-counted freed mem for Chunks since val._pojo._mem & val._mem are
											// the same.
					}
				}
			...
			String s1 = "Cleaner pass took: " + PrettyPrint.msecs(System.currentTimeMillis() - now, true) + ", spilled "
					+ PrettyPrint.bytes(cleaned) + " in " + PrettyPrint.usecs(io_ns >> 10);
			h = Histo.current(true); // Force a new histogram
			MemoryManager.set_goals("postclean", false);
			// No logging if under memory pressure: can deadlock the cleaner thread
			String s2 = h + " diski_o=" + PrettyPrint.bytes(cleaned) + ", freed=" + (freed >> 20) + "M, DESIRED="
					+ (DESIRED >> 20) + "M";
			if (MemoryManager.canAlloc()) {
				Log.debug(s1, s2);
			} else
	...
	static class Histo {
		...
		public String toString() {
			long x = _eldest;
			long now = System.currentTimeMillis();
			return "H(cached:" + (_cached >> 20) + "M, eldest:" + x + "L < +" + (_oldest - x) + "ms <...{" + _hStep
					+ "ms}...< +" + (_hStep * _hs.length) + "ms < +" + (now - x) + ")";
		}
	}
}
