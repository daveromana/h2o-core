...
final public class H2O {
	...
	public static class BaseArgs {
		...
		/**
		 * -internal_security_conf path (absolute or relative) to a file containing all
		 * internal security related configurations
		 */
		public String internal_security_conf = null;

		/**
		 * -internal_security_enabled is a boolean that indicates if internal
		 * communication paths are secured
		 */
		...
		/**
		 * -web_ip=ip4_or_ip6; IP used for web server. By default it listen to all
		 * interfaces.
		 */
	...
	public static class OptArgs extends BaseArgs {
		...
		/**
		 * -client, -client=true; Client-only; no work; no homing of Keys (but can
		 * cache)
		 */
		...
		/**
		 * -hdfs_skip=hdfs_skip; used by Hadoop driver to not unpack and load any HDFS
		 * jar file at runtime.
		 */
		...
		/**
		 * -random_udp_drop, -random_udp_drop=true; test only, randomly drop udp
		 * incoming
		 */
		public boolean random_udp_drop;

		/**
		 * -md5skip, -md5skip=true; test-only; Skip the MD5 Jar checksum; allows jars
		 * from different builds to mingle in the same cloud
		 */
		public boolean md5skip = false;

		/**
		 * -quiet Enable quiet mode and avoid any prints to console, useful for client
		 * embedding
		 */
		...
		/**
		 * -no_latest_check Do not attempt to retrieve latest H2O version from S3 on
		 * startup
		 */
		...
		/**
		 * Whether this H2O instance was launched on hadoop (using 'hadoop jar
		 * h2odriver.jar') or not.
		 */
	...
	private static void validateArguments() {
		...
		if (ARGS.hash_login || ARGS.ldap_login || ARGS.kerberos_login || ARGS.pam_login) {
			...
			if (H2O.ARGS.form_auth) {
				parseFailed(
						"No login method was specified. Form-based authentication can only be used in conjunction with of a LoginService.\n"
	...
	/**
	 * Tell the embedding software that this H2O instance belongs to a cloud of a
	 * certain size. This may be non-blocking.
	 *
	 * @param ip
	 *            IP address this H2O can be reached at.
	 * @param port
	 *            Port this H2O can be reached at (for REST API and browser).
	 * @param size
	...
	/**
	 * Notify embedding software instance H2O wants to exit. Shuts down a single
	 * Node.
	 * 
	 * @param status
	...
	/**
	 * Orderly shutdown with infinite timeout for confirmations from the nodes in
	 * the cluster
	 */
	...
	public static int orderlyShutdown(int timeout) {
		...
		if (timeout > 0) {
			...
			fs.blockForPending(); // todo, should really have block for pending with a timeout
	...
	public static void configureLogging() {
		...
		//
		// The full logger initialization is done by setLog4jProperties() in class
		// water.util.Log.
		// The trick is the output path / file isn't known until the H2O API PORT is
		// chosen,
		// so real logger initialization has to happen somewhat late in the startup
		// lifecycle.
	...
	/**
	 * Calculate a unique model id that includes User-Agent info (if it can be
	 * discovered). For the user agent info to be discovered, this needs to be
	 * called from a Jetty thread.
	 *
	 * This lets us distinguish models created from R vs. other front-ends, for
	 * example. At some future point, it could make sense to include a sessionId
	 * here.
	 *
	 * The algorithm is:
	 * descModel_[userAgentPrefixIfKnown_]cloudId_monotonicallyIncreasingInteger
	 *
	 * Right now because of the way the REST API works, a bunch of numbers are
	 * created and thrown away. So the values are monotonically increasing but not
	 * contiguous.
	 *
	 * @param desc
	 *            Model description.
	...
	synchronized public static String calcNextUniqueModelId(String desc) {
		...
		//
		// I actually tried only doing the addAndGet only for POST requests (and junk
		// UUID otherwise),
	...
	// It's used to inform the Admin user the last time each H2O instance did
	// something.
	// Admins can take this information and decide whether to kill idle clusters to
	// reclaim tied up resources.
	...
	/**
	 * Update the last time that something happened to reset the idle timer. This is
	 * meant to be callable safely from almost anywhere.
	 */
	...
	/**
	 * Throw an exception that will cause the request to fail, but the cluster to
	 * continue.
	 * 
	...
	/**
	 * H2O.fail is intended to be used in code where something should never happen,
	 * and if it does it's a coding error that needs to be addressed immediately.
	 * Examples are: AutoBuffer serialization for an object you're trying to
	 * serialize isn't available; there's a typing error on your schema; your switch
	 * statement didn't cover all the AstRoot subclasses available in Rapids.
	 * <p>
	 * It should *not* be used when only the single request should fail, it should
	 * *only* be used if the error means that someone needs to go add some code
	 * right away.
	 *
	 * @param msg
	 *            Message to Log.fatal()
	 * @param cause
	 *            Optional cause exception to Log.fatal()
	...
	/**
	 * Return an error message with an accompanying URL to help the user get more
	...
	public static String technote(int number, String message) {
		return message + "\n\n" + "For more information visit:\n" + "  http://jira.h2o.ai/browse/TN-"
				+ Integer.toString(number);
	}

	/**
	 * Return an error message with an accompanying list of URLs to help the user
	 * get more detailed information.
	 *
	 * @param numbers
	 *            H2O tech note numbers.
	 * @param message
	 *            Message to present to the user.
	...
	private static class PrioritizedForkJoinPool extends ForkJoinPool {
		...
		private PrioritizedForkJoinPool(int p, int cap) {
			super((ARGS.nthreads <= 0) ? NUMCPUS : ARGS.nthreads, new FJWThrFact(cap), null,
					p >= MIN_HI_PRIORITY /*
											 * low priority FJQs should use the default FJ settings to use LIFO order of
											 * thread private queues.
											 */);
			_priority = p;
		}
	...
	public static <T extends H2OCountedCompleter> T submitTask(T task) {
		...
		assert MIN_PRIORITY <= priority && priority <= MAX_PRIORITY : "priority " + priority
				+ " is out of range, expected range is < " + MIN_PRIORITY + "," + MAX_PRIORITY + ">";
	...
	/**
	 * Simple wrapper over F/J {@link CountedCompleter} to support priority queues.
	 * F/J queues are simple unordered (and extremely light weight) queues. However,
	 * we frequently need priorities to avoid deadlock and to promote efficient
	 * throughput (e.g. failure to respond quickly to {@link TaskGetKey} can block
	 * an entire node for lack of some small piece of data). So each attempt to do
	 * lower-priority F/J work starts with an attempt to work and drain the
	 * higher-priority queues.
	 */
	public static abstract class H2OCountedCompleter<T extends H2OCountedCompleter> extends CountedCompleter
		...
		protected H2OCountedCompleter(H2OCountedCompleter completer) {
			this(completer, computePriority(completer));
		}
		...
		/**
		 * Used by the F/J framework internally to do work. Once per F/J task, drain the
		 * high priority queue before doing any low priority work. Calls
		 * {@link #compute2} which contains actual work.
		 */
		@Override
		public final void compute() {
			...
			try {
				assert priority() == pp : " wrong priority for task " + getClass().getSimpleName() + ", expected "
						+ priority() + ", but got " + pp; // Job went to the correct queue?
		...
		/**
		 * Override compute3() with actual work without having to worry about
		 * tryComplete()
		 */
		...
		/**
		 * If this is a F/J thread, return it's priority - used to lift the priority of
		 * a blocking remote call, so the remote node runs it at a higher priority - so
		 * we don't deadlock when we burn the local thread.
		 */
		...
		protected Icer<T> icer() {
			...
			if (id != 0) {
				int tyid;
				if (id != 0) {
					assert id == (tyid = TypeMap.onIce(this)) : "incorrectly cashed id " + id + ", typemap has " + tyid
		...
		@Override
		final public AutoBuffer write(AutoBuffer ab) {
			return icer().write(ab, (T) this);
		}

		@Override
		final public AutoBuffer writeJSON(AutoBuffer ab) {
			return icer().writeJSON(ab, (T) this);
		}

		@Override
		final public T read(AutoBuffer ab) {
			return icer().read(ab, (T) this);
		}

		@Override
		final public T readJSON(AutoBuffer ab) {
			return icer().readJSON(ab, (T) this);
		}

		@Override
		final public int frozenType() {
	...
	public static String getURL(String schema, InetAddress address, int port, String contextPath) {
		return String.format(address instanceof Inet6Address ? "%s://[%s]:%d%s" : "%s://%s:%d%s", schema,
				address.getHostAddress(), port, contextPath);
	}
	...
	/**
	 * IP address of this node used for communication with other nodes.
	 */
	public static InetAddress SELF_ADDRESS;

	/*
	 * Global flag to mark this specific cloud instance IPv6 only. Right now, users
	 * have to force IPv6 stack by specifying the following JVM options:
	...
	/*
	 * A static list of acceptable Cloud members passed via -flatfile option. It is
	 * updated also when a new client appears.
	 */
	private static HashSet<H2ONode> STATIC_H2OS = null;

	/*
	 * List of all clients that ever connected to this cloud. Keys are IP:PORT of
	 * these clients
	 */
	...
	private static void printAndLogVersion(String[] arguments) {
		...
		if (ABV.isTooOld()) {
			Log.warn("\n*** Your H2O version is too old! Please download the latest version " + latestVersion
		...
		Log.info("Java version: Java " + System.getProperty("java.version") + " (from "
		...
		Log.info("OS version: " + System.getProperty("os.name") + " " + System.getProperty("os.version") + " ("
	...
	/**
	 * Initializes the local node and the local cloud with itself as the only
	 * member.
	 */
	private static void startLocalNode() {
		...
		Log.info("H2O cloud name: '" + ARGS.name + "' on " + SELF
				+ (ARGS.flatfile == null ? (", discovery address " + CLOUD_MULTICAST_GROUP + ":" + CLOUD_MULTICAST_PORT)
		...
		if (!H2O.ARGS.disable_web) {
			Log.info(
					"If you have trouble connecting, try SSH tunneling from your local machine (e.g., via port 55555):\n"
							+ "  1. Open a terminal and run 'ssh -L 55555:localhost:" + API_PORT + " "
							+ System.getProperty("user.name") + "@" + SELF_ADDRESS.getHostAddress() + "'\n"
	...
	/**
	 * Starts the worker threads, receiver threads, heartbeats and all other network
	 * related services.
	 */
	...
	@Deprecated
	static public void register(String method_url, Class<? extends water.api.Handler> hclass, String method,
			String apiName, String summary) {
	...
	/**
	 * Start the web service; disallow future URL registration. Blocks until the
	 * server is up.
	 *
		...
		if (_node_ip_to_index != null) {
			...
			if (index != null) {
				if (index == -1) {
					...
					throw new RuntimeException("Mapping from node id to node index contains: " + index
	 ...
	 *
	 * Having to call this manually from user code is a sign that something is wrong
	 * and a better heuristic is needed internally.
	 */
	...
	public static void main(String[] args) {
		...
		if ((new File(".h2o_no_collect")).exists()
				...
				|| ARGS.ga_opt_out || (gaidList = JarHash.getResourcesList("gaid")).contains("CRAN")
		...
		// The "Cloud of size N formed" message printed out by doHeartbeat is the
		// trigger
	...
	/**
	 * Add node to a manual multicast list. Note: the method is valid only if
	 * -flatfile option was specified on commandline*
	 * 
	 * @param node
	 *            h2o node
	...
	/**
	 * Remove node from a manual multicast list. Note: the method is valid only if
	 * -flatfile option was specified on commandline*
	 * 
	 * @param node
	 *            h2o node
	...
	/**
	 * Check if a node is included in a manual multicast list. Note: the method is
	 * valid only if -flatfile option was specified on commandline
	 *
	 * @param node
	 *            h2o node
	 * @return true if node was already in the multicast list.
	...
	/**
	 * Returns a set of nodes which are contacted during manual multicast. The
	 * returned value can be modified by the user since the call return a copy of
	 * the original set.
	 * 
	 * @return set of nodes
	 */
