...
/**
 * Jobs are used to do minimal tracking of long-lifetime user actions, including
 * progress-bar updates and the ability to review in progress or completed Jobs,
 * and cancel currently running Jobs.
 * <p>
 * Jobs are {@link Keyed}, because they need to Key to control e.g. atomic
 * updates.
 * <p>
...
public final class Job<T extends Keyed> extends Keyed<Job> {
	 ...
	 * @param clz_of_T
	 *            String class of the Keyed result
	 * @param desc
	 *            String description
	 */
	 ...
	 *            String description
	 * @param warningStr
	 *            String contains a warning on model_id
	 */
	...
	private static Key<Job> defaultJobKey() {
		return Key.make((byte) 0, Key.JOB, false, H2O.SELF);
	}
	...
	private boolean running() {
		return _start_time != 0 && _end_time == 0;
	}
	...
	public boolean isStopping() {
		return isRunning() && _stop_requested;
	}

	public boolean isDone() {
		return isStopped() && _ex == null;
	}

	public boolean isCrashing() {
		return isRunning() && _ex != null;
	}
	...
	public boolean readyForView() {
		return _ready_for_view;
	}

	public void setReadyForView(boolean ready) {
		_ready_for_view = ready;
	}

	/**
	 * Jobs may be requested to Stop. Each individual job will respond to this on a
	 * best-effort basis, and make some time to stop. Stop really means "the Job
	 * stops", but is not an indication of any kind of error or fail. Perhaps the
	 * user simply got bored. Because it takes time to stop, a Job may be both in
	 * state isRunning and stop_requested, and may later switch to isStopped and
	 * stop_requested. Also, an exception may be posted.
	 */
	...
	/**
	 * Any exception thrown by this Job, or null if none. Note that while setting an
	 * exception generally triggers stopping a Job, stopping takes time, so the Job
	 * might still be running with an exception posted.
	 */
	private byte[] _ex;
	...
	/**
	 * Returns a float from 0 to 1 representing progress. Polled periodically. Can
	 * default to returning e.g. 0 always.
	 */
	...
	public final void update(final long newworked) {
		update(newworked, (String) null);
	}

	public static void update(final long newworked, Key<Job> jobkey) {
		update(newworked, null, jobkey);
	}
	...
	/**
	 * Start this task based on given top-level fork-join task representing job
	 * computation.
	 * 
	 * @param fjtask
	 *            top-level job computation task.
	 * @param work
	...
	static public boolean isCancelledException(Throwable ex) {
		return ex instanceof JobCancelledException
				|| ex.getCause() != null && ex.getCause() instanceof JobCancelledException;
	}
	...
	private static class Barrier1OnExCom extends JAtomic {
		...
		} // Already stopped & exception'd

		@Override
		void update(Job job) {
			if (job._ex == null) {
				job._ex = _dex; // Keep first exception ever
			}
	...
	public T get() {
		...
		if (bar != null) { // Barrier may be null if task already completed
			bar.join(); // Block on the *barrier* task, which blocks until the fjtask on*Completion code
						// runs completely
		}
		assert isStopped();
		if (_ex != null) {
		...
		// Maybe null return, if the started fjtask does not actually produce a result
		// at this Key
