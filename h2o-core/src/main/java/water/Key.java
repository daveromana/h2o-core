 ...
 * Model consistency. Keys are a means to find a {@link Value} somewhere in the
 * Cloud, to cache it locally, to allow globally consistent updates to a
 ...
 * <p>
 * Keys are defined as a simple byte-array, plus a hashCode and a small cache of
 * Cloud-specific information. The first byte of the byte-array determines if
 * this is a user-visible Key or an internal system Key; an initial byte of
 ...
 * <p>
 * System keys for {@link Job}, {@link Vec}, {@link Chunk} and
 * {@link water.fvec.Vec.VectorGroup} have special initial bytes; Keys for these
 * classes can be determined without loading the underlying Value. Layout for
 * {@link Vec} and {@link Chunk} is further restricted, so there is an efficient
 * mapping between a numbered Chunk and it's associated Vec.
 * <p>
 ...
 * bytes. Otherwise home nodes are selected by pseudo-random hash. Selecting a
 * home node is sometimes useful for Keys with very high update rates coming
...
final public class Key<T extends Keyed> extends Iced<Key<T>> implements Comparable {
	...
	// 1 - homing byte, always -1/0xFF as these keys use the hash to figure their
	// home out
	// 4 - Vector Group
	...
	public final boolean isVec() {
		return _kb != null && _kb.length > 0 && _kb[0] == VEC;
	}
	...
	public final boolean isChunkKey() {
		return _kb != null && _kb.length > 0 && _kb[0] == CHK;
	}
	...
	public final Key getVecKey() {
		assert isChunkKey();
		return water.fvec.Vec.getVecKey(this);
	}
	...
	int D(int repl) {
		...
		if (!user_allowed() && repl < _kb[1]) { // Asking for a replica# from the homed list?
			...
			H2ONode h2o = H2ONode.intern(_kb,
					2 + repl * (H2ONode.H2Okey.SIZE /* serialized bytesize of H2OKey - depends on IP protocol */));
	...
	private static final AtomicLongFieldUpdater<Key> _cacheUpdater = AtomicLongFieldUpdater.newUpdater(Key.class,
			"_cache");
	...
	// data is a cache of.
	private static int cloud(long cache) {
		return (int) (cache >>> 0) & 0x00FF;
	}
	...
	private static int home(long cache) {
	...
	// changed Cloud.
	private static int replica(long cache) {
		return (byte) (cache >>> 24) & 0x00FF;
	}

	// Desired replication factor. Can be zero for temp keys. Not allowed to
	// later, because it messes with e.g. meta-data on disk.
	private static int desired(long cache) {
		return (int) (cache >>> 32) & 0x00FF;
	}

	private static long build_cache(int cidx, int home, int replica, int desired) {
		return // Build the new cache word
		((long) (cidx & 0xFF) << 0) | ((long) (home & 0xFFFF) << 8) | ((long) (replica & 0xFF) << 24)
				| ((long) (desired & 0xFF) << 32) | ((long) (0) << 40);
	}

	int home(H2O cloud) {
		return home(cloud_info(cloud));
	}

	int replica(H2O cloud) {
		return replica(cloud_info(cloud));
	}

	int desired() {
		return desired(_cache);
	}
	...
	public static <P extends Keyed> Key<P> make(byte[] kb) {
		return make(kb, DEFAULT_DESIRED_REPLICA_FACTOR);
	}
	 ...
	 * 
	 * @param node
	 *            a node at which the new key is homed.
	 * @return the new key
	 */
	...
	static <P extends Keyed> Key<P> make(String s, byte rf) {
		return make(decodeKeyName(s), rf);
	}
	...
	/**
	 * Factory making a homed system Key. Requires the initial system byte but then
	 * allows a String for the remaining bytes. Requires a list of exactly one
	 * H2ONode to home at. The hint specifies if it is an error to name an H2ONode
	 * that is NOT in the Cloud, or if some other H2ONode can be substituted. The rf
	 * parameter and passing more than 1 H2ONode are both depreciated.
	 * 
	...
	/**
	 * Factory making a homed system Key. Requires the initial system byte and uses
	 * {@link #rand} for the remaining bytes. Requires a list of exactly one H2ONode
	 * to home at. The hint specifies if it is an error to name an H2ONode that is
	 * NOT in the Cloud, or if some other H2ONode can be substituted. The rf
	 * parameter and passing more than 1 H2ONode are both depreciated.
	 * 
	 * @return the desired Key
	 */
	...
	public static <P extends Keyed> Key<P> make(byte[] kb, byte rf, byte systemType, boolean required,
			H2ONode... replicas) {
	...
	/**
	 * Return the classname for the Value that this Key points to, if any (e.g.,
	 * "water.fvec.Frame").
	 */
	public String valueClass() {
		...
		//
		// Therefore, we have to fetch the type of the item the Key is pointing to at
		// runtime.
	...
	/**
	 * Return the base classname (not including the package) for the Value that this
	 * Key points to, if any (e.g., "Frame").
	 */
	...
	public String toString() {
		...
		if (len >= 0) {
			...
			sb.append(MAGIC_CHAR);
			for (int i = len + 1; i < _kb.length; ++i)
				sb.append((char) _kb[i]);
			return sb.toString();
		} else {
