...
/**
 * Convenience class for easy access to user-visible keys in the cloud with
 * enabled caching.
 *
 * This class represents snapshot of user keys currently stored in the cloud and
 * contains methods to retrieve it. It contains all user keys stored in the
 * cloud at one particular point in time (marked by timestamp member variable).
 * Snapshot does not contain the actual values and no values are fetched from
 * remote by requesting new snapshot.
 *
 * KeySnapshot itself is a set of user keys with some additional info (e.g. type
 * and size) and some convenience functions supporting filtering and
 * instantiating of classes pointed to by the keys
 *
 * @author tomas
 */
public class KeySnapshot {
	...
	/**
	 * Class containing information about user keys. Contains the actual key and all
	 * interesting information except the data itself.
	 */
	public static final class KeyInfo extends Iced implements Comparable<KeyInfo> {
		...
		public int compareTo(KeyInfo ki) {
			return _key.compareTo(ki._key);
		}

		public boolean isFrame() {
			return _type == TypeMap.FRAME;
		}
	...
	/**
	 * Filter the snapshot providing custom filter. Only the keys for which filter
	 * returns true will be present in the new snapshot.
	 * 
	 * @param kvf
	 *            The filter
	...
	/**
	 * @param c
	 *            Class objects of which should be instantiated
	 * @param <T>
	 *            Generic class being fetched
	 * @return all objects (of the proper class) pointed to by this key snapshot
	 *         (and still present in the K/V at the time of invocation).
	 */
	public <T extends Iced> Map<String, T> fetchAll(Class<T> c) {
		return fetchAll(c, false, 0, Integer.MAX_VALUE);
	}
	...
	public <T extends Iced> Map<String, T> fetchAll(Class<T> c, boolean exact) {
		return fetchAll(c, exact, 0, Integer.MAX_VALUE);
	}
	 ...
	 *            - subclasses will not be included if set.
	 * @param offset
	 *            - skip first offset values matching the given type
	 * @param limit
	...
	/**
	 * Get the user keys from this node only. Includes non-local keys which are
	 * cached locally.
	 * 
	 ...
	 * 
	 * @param homeOnly
	 *            - exclude the non-local (cached) keys if set
	 * @return KeySnapshot containing keys from the local K/V.
	 */
	public static KeySnapshot localSnapshot(boolean homeOnly) {
		...
		for (int i = 2; i < kvs.length; i += 2) {
			...
			// Raw array can contain regular and also wrapped values into Prime marker
			// class:
			// - if we see Value object, create instance of KeyInfo
			// - if we do not see Value object directly (it can be wrapped in Prime marker
			// class),
		...
		public void reduce(GlobalUKeySetTask gbt) {
			if (_res == null) {
				...
				for (int i = 0; i < res.length; ++i)
					res[i] = j < gbt._res.length && (k == _res.length || gbt._res[j].compareTo(_res[k]) < 0)
							? gbt._res[j++]
							: _res[k++];
				_res = res;
			}
