 ...
 * <p>
 * MRTask provides several <code>map</code> and <code>reduce</code> methods that
 * can be overridden to specify a computation. Several instances of this class
 * will be created to distribute the computation over F/J threads and machines.
 * Non-transient fields are copied and serialized to instances created for map
 * invocations. Reduce methods can store their results in fields. Results are
 * serialized and reduced all the way back to the invoking node. When the last
 * reduce method has been called, fields of the initial MRTask instance contains
 * the computation results.
 * </p>
 * <p>
 * Apart from small reduced POJO returned to the calling node, MRTask can
 * produce output vector(s) as a result. These will have chunks co-located with
 * the input dataset, however, their number of lines will generally differ so
 * they won't be strictly compatible with the original. To produce output
 ...
 * NewChunks. MRTask will automatically close the new Appendable vecs and a call
 * to <code>outputFrame</code> will make a frame with newly created Vecs.
 * </p>
 *
 * <p>
 * <b>Overview</b>
 ...
 * <code>dfork</code>, or <code>dfork</code>. <code>doAll</code> simply calls
 * <code>dfork</code> and <code>dfork</code> before blocking; <code>dfork</code>
 * and <code>dfork</code> are non-blocking. The main pardigm is
 ...
 * <code>reduce</code> is called between each pair of calls to <code>map</code>.
 * </p>
 * <p>
 * MRTask can also be called with <code>doAllNodes</code>, in which case only
 * the setupLocal call is made once per node; neither map nor reduce are called.
 * </p>
 * <p>
 * 
 ...
 * <code>setupLocal</code> is called once per node before any map calls are made
 * on that node (but perhaps other nodes have already started); in reverse
 * <code>closeLocal</code> is called after the last map call completes on a node
 * (but perhaps other nodes are still computing maps). <code>postGlobal</code>
 * is called once only after all maps, reduces and closeLocals, and only on the
 * home node.
 * </p>
 */
public abstract class MRTask<T extends MRTask<T>> extends DTask<T> implements ForkJoinPool.ManagedBlocker {
	 ...
	 *
	 * There are several internal flags and counters used throughout. They are
	 * gathered in this note to help you reason about the execution of an MRTask.
	 *
	 * internal "top-level" fields --------------------------- - RPC<T> _nleft,
	 * _nrite: "child" node/JVMs that are doing work - boolean _topLocal : "root"
	 * MRTask on a local machine - boolean _topGlobal : "root" MRTask on the "root"
	 * node - T _left, _rite : "child" MRTasks on a local machine - T _res :
	 * "result" MRTask (everything reduced into here) - int _nlo,_nhi : range of
	 * nodes to do remote work on (divide-conquer; see Diagram 2) - Futures _fs :
	 * _topLocal task blocks on _fs for _left and _rite to complete
	 *
	 * Diagram 1: N is for Node; T is for Task -------------------------------------
	 * 3 node cloud Inside one of the 'N' nodes: N1 T _topLocal** / \ / \ N2
	 * (_nleft) N3 (_nrite) T (_left) T (_rite)
	 *
	 * **: T is also _topGlobal if N==N1
	 *
	 * These fields get set in the <code>SetupLocal0<code> call. Let's see what it
	 * does:
	 *
	 * Diagram 2: ---------- dfork on N1 - _topGlobal=true - _nlo=0 -
	 * _nhi=CLOUD_SIZE || || || ==> setupLocal0 on N1 - topLocal=true - _fs = new
	 * Futures() - nmid = (_nlo + _nhi) >> 1 => split the range of nodes
	 * (divide-conquer) - _nleft = remote_compute(_nlo,nmid) => chooses a node in
	 * range and does new RPC().call() - _nrite = remote_compute(nmid,_nhi)
	 * serializing MRTask and call dinvoke on remote. / \ / \ / \ dinvoke on N2
	 * dinvoke on N3 setupLocal0 on N2 setupLocal0 on N3 - topLocal=true -
	 * topLocal=true - _fs = new Futures() - _fs = new Futures() - (continue
	 * splitting) - (continue splitting) H2O.submitTask(this) => compute2
	 * H2O.submitTask(this) => compute2
	 *
	 */
	...
	/**
	 * This Frame instance is the handle for computation over a set of Vec
	 * instances. Recall that a Frame is a collection Vec instances, so this
	 * includes any invocation of <code>doAll</code> with Frame and Vec[] instances.
	 * Top-level calls to <code>doAll</code> wrap Vec instances into a new Frame
	 * instance and set this into <code>_fr</code> during a call to
	 * <code>dfork</code>.
	 */
	public Frame _fr;

	/**
	 * This <code>Key[]</code> instance is the handle used for computation when an
	 * MRTask is invoked over an array of <code>Key</code>instances.
	 */
	public Key[] _keys;

	/**
	 * The number and type of output Vec instances produced by an MRTask. If null
	 * then there are no outputs, _appendables will be null, and calls to
	...
	/**
	 * Appendables are treated separately (roll-ups computed in map/reduce style,
	 * can not be passed via K/V store).
	 */
	...
	public String profString() {
		return _profile != null ? _profile.toString() : "Profiling turned off";
	}
	...
	/**
	 * Get the resulting Frame from this invoked MRTask. <b>This Frame is not in the
	 * DKV.</b> AppendableVec instances are closed into Vec instances, which then
	 * appear in the DKV.
	 *
	 * @return null if no outputs, otherwise returns the resulting Frame from the
	 *         MRTask. The Frame has no column names nor domains.
	 */
	public Frame outputFrame() {
		return outputFrame(null, null, null);
	}
	 ...
	 *
	 * @param names
	 *            The names of the columns in the resulting Frame.
	 * @param domains
	...
	public Frame outputFrame(String[] names, String[][] domains) {
		return outputFrame(null, names, domains);
	}

	/**
	 * Get the resulting Frame from this invoked MRTask. If the passed in
	 * <code>key</code> is not null, then the resulting Frame will appear in the
	 * DKV. AppendableVec instances are closed into Vec instances, which then appear
	 * in the DKV.
	 *
	 * @param key
	 *            If null, then the Frame will not appear in the DKV. Otherwise,
	 *            this result will appear in the DKV under this key.
	 * @param names
	 *            The names of the columns in the resulting Frame.
	 * @param domains
	 *            The domains of the columns in the resulting Frame.
	 ...
	 * <strong>local</strong> input Chunk. It is meant for map/reduce jobs that use
	 * a single column in a input Frame. All map variants are called, but only one
	 ...
	 * <strong>local</strong> Chunks. All map variants are called, but only one is
	 ...
	 * <strong>local</strong> input Chunks. All map variants are called, but only
	 * one is expected to be overridden.
	 */
	...
	/**
	 * Override with your map implementation. This overload is given an array of
	 * <strong>local</strong> input Chunks, for Frames with arbitrary column
	...
	/**
	 * Override with your map implementation. Used when doAll is called with an
	 * array of Keys, and called once-per-Key on the Key's Home node
	 */
	public void map(Key key) {
	}

	/**
	 * Override to combine results from 'mrt' into 'this' MRTask. Both 'this' and
	 * 'mrt' are guaranteed to either have map() run on them, or be the results of a
	 * prior reduce(). Reduce is optional if, e.g., the result is some output
	 * vector.
	 */
	...
	/**
	 * Override to do any remote initialization on the 1st remote instance of this
	 * object, for initializing node-local shared data structures.
	 */
	protected void setupLocal() {
	}

	/**
	 * Override to do any remote cleaning on the last remote instance of this
	 * object, for disposing of node-local shared data structures.
	 */
	...
	private static class MRProfile extends Iced {
		...
		long sumTime() {
			return _onCdone - (_localstart == 0 ? _mapstart : _localstart);
		}
		...
		void gather(MRProfile p, int size_rez) {
			...
			if (_last == null) {
				...
				if (first._onCdone > _done1st) {
					_time1st = first.sumTime();
					_done1st = first._onCdone;
				}
			}
		...
		private StringBuilder print(StringBuilder sb, int d) {
			if (d == 0) {
				sb.append(_clz).append("\n");
			}
			for (int i = 0; i < d; i++)
				sb.append("  ");
			if (_localstart != 0) {
				sb.append("Node local ").append(_localdone - _localstart).append("ms, ");
			}
			if (_last != null) { // Forked job?
				sb.append("Slow wait ").append(_mapstart - _localdone).append("ms + work ").append(_last.sumTime())
						.append("ms, ");
				sb.append("Fast work ").append(_time1st).append("ms + wait ").append(_onCstart - _done1st)
						.append("ms\n");
			...
			if (_remoteBlkDone != 0) {
				...
				sb.append("ms, size ").append(PrettyPrint.bytes(_size_rez0)).append("+")
						.append(PrettyPrint.bytes(_size_rez1));
			}
			sb.append(")\n");
			return sb;
		}
	...
	public final T doAll(byte[] types, Vec... vecs) {
		return doAll(types, new Frame(vecs), false);
	}

	public final T doAll(byte type, Vec... vecs) {
		return doAll(new byte[] { type }, new Frame(vecs), false);
	}

	public final T doAll(Vec vec, boolean run_local) {
		return doAll(null, vec, run_local);
	}

	public final T doAll(byte[] types, Vec vec, boolean run_local) {
		return doAll(types, new Frame(vec), run_local);
	}
	...
	public final T doAll(Frame fr, boolean run_local) {
		return doAll(null, fr, run_local);
	}
	...
	public final T doAll(byte type, Frame fr) {
		return doAll(new byte[] { type }, fr, false);
	}
	...
	public void dfork(Key... keys) {
		...
		_keys = keys;
		_nlo = selfidx();
		_nhi = (short) H2O.CLOUD.size(); // Do Whole Cloud
	 ...
	 * <code>outputs</code> Vec instances. This call is asynchronous. It returns
	 * 'this', on which <code>getResult</code> may be invoked by the caller to block
	 * for pending computation to complete.
	 *
	 * @param types
	 *            The type of output Vec instances to create.
	 * @param vecs
	 *            The input set of Vec instances upon which computation is
	 *            performed.
	 * @return this
	 */
	public final T dfork(byte[] types, Vec... vecs) {
		return dfork(types, new Frame(vecs), false);
	}

	public final T dfork(Vec... vecs) {
		return dfork(null, new Frame(vecs), false);
	}

	/**
	 * Invokes the map/reduce computation over the given Frame instance. This call
	 * is asynchronous. It returns 'this', on which <code>getResult</code> may be
	 * invoked by the caller to block for pending computation to complete. This call
	 * produces no output Vec instances or Frame instances.
	 *
	 * @param fr
	...
	/**
	 * Fork the task in strictly non-blocking fashion. Same functionality as dfork,
	 * but does not raise priority, so user is should *never* block on it. Because
	 * it does not raise priority, these can be tail-call chained together for any
	 * length.
	 */
	...
	/**
	 * Block for and get any final results from a dfork'd MRTask. Note: the desired
	 * name 'get' is final in ForkJoinTask.
	 */
		...
		do {
			try {
				if (fjManagedBlock) {
					...
					// For the cases when we really want to block this thread without FJ framework
					// scheduling a new worker thread.
					// Model use is in MultifileParseTask - we want to be parsing at most cluster
					// ncores files in parallel.
					block();
				...
				throw (re instanceof DistributedException) ? new DistributedException(re.getMessage(), re.getCause())
						: new DistributedException(re);
			}
		} while (!isReleasable());
	...
	/*
	 * Set top-level fields and fire off remote work (if there is any to do) to 2
	 * selected child JVM/nodes. Setup for local work: fire off any global work to
	 * cloud neighbors; do all chunks; call user's init.
	 */
	...
	/**
	 * Called from FJ threads to do local work. The first called Task (which is also
	 * the last one to Complete) also reduces any global work. Called internal by
		...
		if (_fr == null) { // No Frame, so doing Keys?
			...
			if (_run_local || v0.chunkKey(_lo).home()) { // And chunk is homed here?
				...
				for (int i = 0; i < vecs.length; i++)
					if (vecs[i] != null) {
						assert _run_local || vecs[i].chunkKey(_lo).home() : "Chunk=" + _lo + " v0=" + v0 + ", k="
								+ v0.chunkKey(_lo) + "   v[" + i + "]=" + vecs[i] + ", k=" + vecs[i].chunkKey(_lo);
				...
				if (num_outputs == 0) {
					...
					// else if (fr_vecs_length == 3) map(bvs[0], bvs[1], bvs[2],
					// appendableChunks[0]);
					...
					// else if (fr_vecs_length == 2) map(bvs[0], bvs[1], appendableChunks[0],
					// appendableChunks[1]);
					// else if (fr_vecs_length == 3) map(bvs[0], bvs[1], bvs[2],
					// appendableChunks[0], appendableChunks[1]);
				...
				for (Chunk bv : bvs)
					bv.close(_lo, _fs);
				if (_output_types != null) {
					for (NewChunk nch : appendableChunks)
						nch.close(_lo, _fs);
				}
			}
		}
	...
	/**
	 * OnCompletion - reduce the left and right into self. Called internal by F/J.
	 * Not expected to be user-called.
	 */
	@Override
	...
	// Copy any final results into 'this', such that a return of 'this' has the
	// results.
	private void postLocal0() {
		...
		if (_res == null) {
			_nhi = -1; // Flag for no local results *at all*
	...
	/**
	 * Call user's reduction. Also reduce any new AppendableVecs. Called internal by
	 * F/J. Not expected to be user-called.
	 */
	void reduce4(T mrt) {
	 ...
	 * exceptions (which is the F/J default). Called internal by F/J. Not expected
	 * to be user-called.
	 */
	@Override
