...
abstract public class MemoryManager {
	...
	static void set_goals(String msg, boolean oom, long bytes) {
		...
		assert freeHeap >= 0 : "I am really confused about the heap usage; MEM_MAX=" + MEM_MAX + " heapUsedGC="
				+ heapUsedGC;
		...
		d -= 2 * p - bytes; // Allow for the effective POJO, and again to throttle GC rate (and allow for
							// this allocation)
		...
		if (cacheUsageNow > Cleaner.DESIRED) {
			...
			if (oom) {
				setMemLow();
			} // Stop allocations; trigger emergency clean
		...
		String s = m + msg + ", (K/V:" + PrettyPrint.bytes(cacheUsageGC) + " + POJO:" + PrettyPrint.bytes(pojoUsedGC)
				+ " + FREE:" + PrettyPrint.bytes(freeHeap) + " == MEM_MAX:" + PrettyPrint.bytes(MEM_MAX)
				+ "), desiredKV=" + PrettyPrint.bytes(Cleaner.DESIRED) + (oom ? " OOM!" : " NO-OOM");
		if (CAN_ALLOC) {
			if (oom) {
				Log.warn(s);
			} else
	...
	/**
	 * Monitors the heap usage after full gc run and tells Cleaner to free memory if
	 * mem usage is too high. Stops new allocation if mem usage is critical.
	 * 
	 * @author tomas
	 */
	private static class HeapUsageMonitor implements javax.management.NotificationListener {
		...
		/**
		 * Callback routine called by JVM after full gc run. Has two functions: 1) sets
		 * the amount of memory to be cleaned from the cache by the Cleaner 2) sets the
		 * CAN_ALLOC flag to false if memory level is critical
		 */
		@Override
		public void handleNotification(Notification notification, Object handback) {
			...
			Log.debug("GC CALLBACK: " + Cleaner.TIME_AT_LAST_GC + ", USED:"
					+ PrettyPrint.bytes(Cleaner.HEAP_USED_AT_LAST_GC) + ", CRIT: " + MEM_LOW_CRITICAL);
			set_goals("GC CALLBACK", MEM_LOW_CRITICAL);
			// if( MEM_LOW_CRITICAL ) { // emergency measure - really low on memory, stop
			...
			// setMemGood(); // In use memory is < 1/2 heap; allow allocations even if
			// Cleaner is still running
		}
	}
	...
	public static short[] malloc2(int size) {
		return (short[]) malloc(size, size * 2L, 2, null, 0);
	}

	public static int[] malloc4(int size) {
		return (int[]) malloc(size, size * 4L, 4, null, 0);
	}

	public static long[] malloc8(int size) {
		return (long[]) malloc(size, size * 8L, 8, null, 0);
	}

	public static float[] malloc4f(int size) {
		return (float[]) malloc(size, size * 4L, 5, null, 0);
	}
	...
	public static boolean[] mallocZ(int size) {
		return (boolean[]) malloc(size, size, 0, null, 0);
	}

	public static Object[] mallocObj(int size) {
		return (Object[]) malloc(size, size * 8L, 10, null, 0, false);
	}
	...
	public static int[] arrayCopyOfRange(int[] orig, int from, int sz) {
		return (int[]) malloc(sz, (sz - from) * 4, -4, orig, from);
	}

	public static long[] arrayCopyOfRange(long[] orig, int from, int sz) {
		return (long[]) malloc(sz, (sz - from) * 8, -8, orig, from);
	}

	public static double[] arrayCopyOfRange(double[] orig, int from, int sz) {
		return (double[]) malloc(sz, (sz - from) * 8, -9, orig, from);
	}
	 ...
	 * Tasks have a shared pool of memory which they should ask for in advance
	 * before they even try to allocate it.
	 *
	 * This method is another backpressure mechanism to make sure we do not exhaust
	 * system's resources by running too many tasks at the same time. Tasks are
	 * expected to reserve memory before proceeding with their execution and making
	 * sure they release it when done.
	 *
	 * @param m
