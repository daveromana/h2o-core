 ...
 * <code>Scope.exit()</code> at some point. Only user keys &amp; Vec keys are
 * tracked.
 * </p>
 *
 * <p>
...
public class Scope {
	...
	static public void enter() {
		_scope.get()._keys.push(new HashSet<Key>());
	}
 static void Key (int arLent, int f, Key k) {
	Futures fs = new Futures();
	
	
	if ((arLent== 0 || f < 0) && k != null) {
		Keyed.remove(k, fs);}
}
	  ...
	  static public Key[] exit(Key... keep) {
	    ...
	    if (keys.size() > 0) {
	      ...
	      for (Key key : keys.pop()) {
	        ...
	        int arLen = arrkeep.length;
	      
	        Scope.Key(arLen, found, key);
	      }  
	      fs.blockForPending();
	    }
	    return keep;
