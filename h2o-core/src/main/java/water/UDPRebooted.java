...
public class UDPRebooted extends UDP {
	...
	public static enum T {
		none, reboot, shutdown, oom, error, locked, mismatch;
	...
	public static class ShutdownTsk extends DTask<ShutdownTsk> {
		...
		public void compute2() {
			...
			// start a separate thread which will force termination after timeout expires
			// (in case we don't get ack ack in time)
			new Thread() {
				@Override
				public void run() {
					try {
					...
					doShutdown(_exitCode, "Orderly shutdown may not have been acknowledged to " + _killer
							+ " (no ackack), exiting with exit code " + _exitCode + ".");
				}
			}.start();
			tryComplete();
		}
		...
		public void onAckAck() {
			doShutdown(_exitCode,
					"Orderly shutdown acknowledged to " + _killer + ", exiting with exit code " + _exitCode + ".");
		}
	...
	static void suicide(T cause, final H2ONode killer) {
		...
		case shutdown:
			Log.warn(
					"Orderly shutdown should be handled via ShutdownTsk. Message is from outside of the cloud? Ignoring it.");
			return;
		case oom:
