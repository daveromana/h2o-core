 ...
 * {@link MemoryManager}, which is the {@link Iced} serialized version of the
 * POJO, and a cached copy of the POJO itself.
 * <p>
 * Requests to extract the POJO from the Value object first try to return the
 * cached POJO. If that is missing, then they will re-inflate the POJO from the
 * {@link Iced} byte[]. If that is missing it is only because the byte[] was
 * swapped to disk by the {@link Cleaner}. It will be reloaded from disk and
 * then inflated as normal.
 * <p>
 ...
 * thread keeps a pointer to the POJO, they can update it at will. If they wish
 * to recover the POJO from the DKV at a later time with all updates intact,
 * they <em>must</em> do a final Put after all updates.
 * <p>
...
public final class Value extends Iced implements ForkJoinPool.ManagedBlocker {
	...
	/**
	 * The Key part of a Key/Value store. Transient, because the Value is typically
	 * found via its Key, and so the Key is available before we get the Value and
	 * does not need to be passed around the wire. Not final, because Keys are
	 * interned slowly (for faster compares) and periodically a Value's Key will be
	 * updated to an interned but equivalent Key.
	 * <p>
	...
	public static final int MAX = Integer.MAX_VALUE; // DeepWater models can contain a single byte[] state as large as
														// 3GB

	/**
	 * Size of the serialized wad of bits. Values are wads of bits; known small
	 * enough to 'chunk' politely on disk, or fit in a Java heap (larger Vecs are
	 * built via Chunks) but (much) larger than a UDP packet. Values can point to
	 * either the disk or ram version or both. There's no compression smarts (done
	 * by the big data Chunks) nor de-dup smarts (done by the nature of a K/V). This
	 * is just a local placeholder for some user bits being held at this local Node.
	 */
	public int _max;
	...
	/**
	 * Invalidate byte[] cache. Only used to eagerly free memory, for data which is
	 * expected to be read-once.
	 */
	...
	/**
	 * Invalidate POJO cache. Only used to eagerly free memory, for data which is
	 * expected to be read-once.
	 */
	...
	/**
	 * The FAST path get-byte-array - final method for speed. Will (re)build the mem
	 * array from either the POJO or disk. Never returns NULL.
	 * 
	...
	final boolean isEmpty() {
		return _max > 0 && _mem == null && _pojo == null && !isPersisted();
	}

	/**
	 * The FAST path get-POJO as an {@link Iced} subclass - final method for speed.
	...
	/**
	 * The FAST path get-POJO as a {@link Freezable} - final method for speed. Will
	 * (re)build the POJO from the _mem array. Never returns NULL. This version has
	 * more type-checking.
	 * 
	 * @return The POJO, probably the cached instance.
	 */
	...
	/**
	 * The FAST path get-POJO as a {@link Freezable} - final method for speed. Will
	 * (re)build the POJO from the _mem array. Never returns NULL.
	 * 
	 * @return The POJO, probably the cached instance.
	 */
	...
	// Exposed and used for testing only; used to trigger premature
	// cleaning/disk-swapping
	...
	final byte backend() {
		return (byte) (_persist & BACKEND_MASK);
	}
	...
	public final boolean isPersisted() {
		return (_persist & ON_dsk) != 0;
	}

	public final void setDsk() {
		_persist |= ON_dsk;
	} // note: not atomic, but only monotonically set bit
	...
	/**
	 * Check if the Value's POJO is a subtype of given type integer. Does not
	 * require the POJO.
	 * 
	...
	/**
	 * Check if the Value's POJO is a {@link Key} subtype. Does not require the
	 * POJO.
	 * 
	 * @return True if the Value's POJO is a {@link Key} subtype.
	 */
	public boolean isKey() {
		return _type != TypeMap.PRIM_B && TypeMap.theFreezable(_type) instanceof Key;
	}

	/**
	 * Check if the Value's POJO is a {@link Frame} subtype. Does not require the
	 * POJO.
	 * 
	 * @return True if the Value's POJO is a {@link Frame} subtype.
	 */
	public boolean isFrame() {
		return _type != TypeMap.PRIM_B && TypeMap.theFreezable(_type) instanceof Frame;
	}

	/**
	 * Check if the Value's POJO is a {@link water.fvec.Vec.VectorGroup} subtype.
	 * Does not require the POJO.
	 * 
	 * @return True if the Value's POJO is a {@link water.fvec.Vec.VectorGroup}
	 *         subtype.
	 */
	public boolean isVecGroup() {
		return _type == TypeMap.VECGROUP;
	}

	/**
	 * Check if the Value's POJO is a {@link water.fvec.Vec.ESPC} subtype. Does not
	 * require the POJO.
	 * 
	 * @return True if the Value's POJO is a {@link water.fvec.Vec.ESPC} subtype.
	 */
	public boolean isESPCGroup() {
		return _type == TypeMap.ESPCGROUP;
	}

	/**
	 * Check if the Value's POJO is a {@link Lockable} subtype. Does not require the
	 * POJO.
	 * 
	 * @return True if the Value's POJO is a {@link Lockable} subtype.
	 */
	public boolean isLockable() {
		return _type != TypeMap.PRIM_B && TypeMap.theFreezable(_type) instanceof Lockable;
	}

	/**
	 * Check if the Value's POJO is a {@link Vec} subtype. Does not require the
	 * POJO.
	 * 
	 * @return True if the Value's POJO is a {@link Vec} subtype.
	 */
	public boolean isVec() {
		return _type != TypeMap.PRIM_B && TypeMap.theFreezable(_type) instanceof Vec;
	}

	/**
	 * Check if the Value's POJO is a {@link hex.Model} subtype. Does not require
	 * the POJO.
	 * 
	 * @return True if the Value's POJO is a {@link hex.Model} subtype.
	 */
	public boolean isModel() {
		return _type != TypeMap.PRIM_B && TypeMap.theFreezable(_type) instanceof hex.Model;
	}

	/**
	 * Check if the Value's POJO is a {@link Job} subtype. Does not require the
	 * POJO.
	 * 
	 * @return True if the Value's POJO is a {@link Job} subtype.
	 */
	public boolean isJob() {
		return _type != TypeMap.PRIM_B && TypeMap.theFreezable(_type) instanceof Job;
	}
	 ...
	 * already-existing Files, so that the File contents will be lazily swapped-in
	 * as the Values are first used.
	 */
	...
	Value(Key k, byte[] mem) {
		this(k, mem.length, mem, TypeMap.PRIM_B, ICE);
	}

	Value(Key k, String s) {
		this(k, StringUtils.bytesOf(s));
	}
	...
	public final Value read_impl(AutoBuffer bb) {
		...
		_mem = bb.getA1();
		_max = _mem.length;
		assert _max < MAX : "Value size=0x" + Integer.toHexString(_max) + " during read is larger than "
				+ Integer.toHexString(MAX) + ", type: " + TypeMap.className(_type);
		_pojo = null;
	...
	private boolean RW_CAS(int old, int nnn, String msg) {
		if (!_rwlock.compareAndSet(old, nnn)) {
			return false;
		}
	...
	private static final AtomicReferenceFieldUpdater<Value, byte[]> REPLICAS_UPDATER = AtomicReferenceFieldUpdater
			.newUpdater(Value.class, byte[].class, "_replicas");
	 ...
	 * flagged against future replication with a -1. Also bumps the active Get
	 * count, which remains until the Get completes (we receive an ACKACK).
	 */
	 ...
	 * TaskPutKey attempt (only 1 thread can ever extract and thus call here). No
	 * future lookups will find this Value, but there may be existing uses.
	 * Atomically set the rwlock count to -1 locking it from further GETs and ship
	 * out invalidates to caching replicas. May need to block on active GETs.
	 * Updates a set of Future invalidates that can be blocked against.
	 */
	...
	void blockTillNoReaders() {
		...
		while (true) { // Repeat, in case racing GETs are bumping the counter
			int old = _rwlock.get();
			if (old <= 0) {
				return; // No readers, or this Value already replaced with a later value
			}
			// Active readers: need to block until the GETs (of this very Value!) all
			// complete
			try {
				ForkJoinPool.managedBlock(this);
			} catch (InterruptedException ignore) {
	...
	/**
	 * Initialize the _replicas field for a PUT. On the Home node (for remote PUTs),
	 * it is initialized to the one replica we know about, and not read-locked. Used
	 * on a new Value about to be PUT on the Home node.
	 */
	void initReplicaHome(H2ONode h2o, Key key) {
		...
		// rest clear.
		replicas()[h2o._unique_idx] = 1;
		_rwlock.set(1); // An initial read-lock, so a fast PUT cannot wipe this one out before
						// invalidates have a chance of being counted
	}

	/**
	 * Block this thread until all prior remote PUTs complete - to force remote-PUT
	 * ordering on the home node.
	 */
	void startRemotePut() {
		assert !_key.home();
		int x;
	...
	/**
	 * Return true if blocking is unnecessary. Alas, used in TWO places and the
	 * blocking API forces them to share here.
	 */
	@Override
	public boolean isReleasable() {
		int r = _rwlock.get();
		if (_key.home()) { // Called from lock_and_invalidate
			// Home-key blocking: wait for active-GET count to fall to zero, or blocking on
			// deleted object
			return r <= 0;
	...
	/**
	 * Possibly blocks the current thread. Returns true if isReleasable would return
	 * true. Used by the FJ Pool management to spawn threads to prevent deadlock is
	 * otherwise all threads would block on waits.
	 */
	@Override
