...
public class Weaver {
	 ...
	 * {@link Iced} serialization classes, skipping static and transient fields, and
	 * the required _ice_id field.
	 * 
	...
	public static Field[] getWovenFields(Class clz) {
		...
		while (Iced.class.isAssignableFrom(clz) || Freezable.class.isAssignableFrom(clz)
	...
	/**
	 * Map of class names to their respective loader. Contains references of the
	 * node-local ClassLoaders so that {@link TypeMap#theFreezable(int)} can make
	 * the correct {@link Class#forName(String, boolean, ClassLoader)} call.
	 */
	private static final transient Map<String/* className */, ClassLoader> CLASSLOADERS;

	/**
	 * Map of class names to their respective ClassPath instance in the
	 * {@link Weaver#_pool}. Class reloads will prune their classpaths.
	 */
	...
	static Class classForName(String className) throws ClassNotFoundException {
		...
		} // class not dynamically loaded, use Weaver's ClassLoader
	...
	// private static boolean hasWovenJSONFields( CtClass cc ) throws
	// NotFoundException {
	...
	// if( !javassist.Modifier.isTransient(mods) &&
	// !javassist.Modifier.isStatic(mods) ) return true;
	// }
	// return false;
	// }
	 ...
	 *
	 * Loading classes at runtime is a matter of simply injecting the new code into
	 * the {@link ClassPool}, and then {@link Weaver#javassistLoadClass(int, Class)}
	 * resolves the generation of (de)serializers. In order to reload classes,
	 * though, each dynamically loaded class must have its very own
	 * {@link ClassLoader}, and all previous {@link Icer}s must be removed. In order
	 * to maintain cluster-wide coherency about which classes are loaded, the
	 * {@link TypeMap} is likewise updated whenever a class is reloaded.
	 *
	 * In order to successfully load classes at runtime (for example a subclass of
	 * {@link MRTask}), each node takes the bytecode and class name and puts a new
	 * {@link ByteArrayClassPath} onto {@link Weaver#_pool}'s classpath. Since there
	 * is no mechanism for retrieving these {@link ClassPath} instances later, they
	 * are stored in {@link Weaver#CLASSPATHS} so that reload events can remove the
	 * old paths. Similarly, {@link Weaver#CLASSLOADERS} holds on to the loaders of
	 * dynamically created classes so that classes can be reloaded and old
	 * {@link ClassLoader} instances pruned.
	 *
	 * Finally, in order to {@link Weaver#genDelegate(int, Class)} during a class
	 * reload, then the previous {@link Icer} must be {@link CtClass#detach}ed. In
	 * addition, the {@link TypeMap#goForGold(int)} must turn a null for the
	 * {@link Icer}.
	 *
	 * @param name
	 *            class name
	...
	private static class LoadClazz extends DTask<LoadClazz> {
		...
		LoadClazz(String name, byte[] bytes) {
			_name = name;
			_bytes = bytes;
		}
	...
	private static Class javassistLoadClass(int id, Class iced_clazz)
			throws CannotCompileException, NotFoundException, InstantiationException, IllegalAccessException,
			NoSuchFieldException, ClassNotFoundException, InvocationTargetException {
		...
		// if(!Freezable.class.isAssignableFrom(iced_clazz.getSuperclass())) return
		// water.Icer.class;
		...
		if (icer_cc != null) {
			synchronized (iced_clazz) {
				...
				return Class.forName(icer_name, true, iced_clazz.getClassLoader()); // Found a pre-cooked Icer
																					// implementation
			}
		}
		...
		synchronized (iced_clazz) {
			...
			icer_cc.toClass(iced_clazz.getClassLoader()); // Load class (but does not link & init)
			return Class.forName(icer_name, true, iced_clazz.getClassLoader()); // Initialize class now, before
																				// subclasses
		}
	}

	// Generate the Icer class
	private static CtClass genIcerClass(int id, CtClass iced_cc, Class iced_clazz, String icer_name, int super_id,
			CtClass super_icer, boolean super_has_jfields)
		...
		for (CtField ctf : ctfs) {
			...
			if (base.subtypeOf(_enum)) { // either an enum or an array of enum
				...
				String src = "  private final " + base.getName().replace('$', '.') + "[] " + name + " = "
						+ base.getName().replace('$', '.') + ".values();\n";
				if (debug_print) {
		...
		// The write call
		String debug = make_body(icer_cc, iced_cc, iced_clazz, "write", null, null,
		...
		String debugJ = make_body(icer_cc, iced_cc, iced_clazz, "writeJSON", "(supers?ab.put1(','):ab).",
				"    ab.put1(',').",
				"  protected final water.AutoBuffer writeJSON" + id + "(water.AutoBuffer ab, " + iced_name
						+ " ice) {\n",
		...
		// The read call
		String rbody_impl = make_body(icer_cc, iced_cc, iced_clazz, "read", null, null,
		...
		if (debug_print) {
			System.out.println(rbody_impl);
		}
		String rbodyJ_impl = make_body(icer_cc, iced_cc, iced_clazz, "readJSON", null, null,
				"  protected final " + iced_name + " readJSON" + id + "(water.AutoBuffer ab, " + iced_name
						+ " ice) {\n",
		...
		if (iced_cc.subclassOf(_dtask)) {
			String cpbody_impl = make_body(icer_cc, iced_cc, iced_clazz, "copyOver", null, null,
	...
	private static String make_body(CtClass icer_cc, CtClass iced_cc, Class iced_clazz, String impl, String field_sep1,
			String field_sep2, String header, String supers, String prims, String prims_unsafe, String enums,
			String enums_unsafe, String iced, String iced_unsafe, String trailer)
			throws CannotCompileException, NotFoundException, NoSuchFieldException {
		...
		for (CtMethod mth : iced_cc.getDeclaredMethods())
			if (mth.getName().equals(mimpl)) { // Found custom serializer?
				...
				if (javassist.Modifier.isStatic(mods)) {
					...
					throw barf(iced_cc,
							" Custom serialization methods must be declared either static or final. Failed for method "
									+ mimpl);
		...
		for (CtField ctf : ctfs) {
			...
			if (field_sep1 != null) {
				...
				field_sep1 = null;
			} else if (field_sep2 != null) {
				sb.append(field_sep2);
			}
	...
	private static void addMethod(String body, CtClass icer_cc) throws CannotCompileException {
		try {
			...
			System.err.println(
					"--- Compilation failure while compiling " + icer_cc.getName() + "\n" + body + "\n------\n" + ce);
			throw ce;
		}
	}

	static private final String[] FLDSZ1 = { "Z", "1", "2", "2", "4", "4f", "8", "8d", // Primitives
	...
	private static int ftype(CtClass ct, String sig) throws NotFoundException {
		switch (sig.charAt(0)) {
		case 'Z':
			return 0; // Booleans: I could compress these more
		case 'B':
			return 1; // Primitives
		case 'C':
			return 2;
	...
	static private void subsub(StringBuilder sb, String s1, String s2) {
		int idx;
		while ((idx = sb.indexOf(s1)) != -1)
			sb.replace(idx, idx + 2, s2);
	}
