		...
		for (int i = 0; i < vecs.length; i++) {
			if (vecs[i].isCategorical()) {
				...
				if (idx == -1 && vecs.length == 1) {
					throw new H2OCategoricalLevelNotFoundArgumentException("match", find.match, frame._key.toString(),
							frame.name(i));
				}
				ds[i] = idx;
				...
				try {
					...
					if (vecs.length == 1) {
						// There's only one Vec and it's a numeric Vec and our search string isn't a
						// number
						IcedHashMapGeneric.IcedHashMapStringObject values = new IcedHashMapGeneric.IcedHashMapStringObject();
						String msg = "Frame: " + frame._key.toString()
								+ " as only one column, it is numeric, and the find pattern is not numeric: "
								+ find.match;
