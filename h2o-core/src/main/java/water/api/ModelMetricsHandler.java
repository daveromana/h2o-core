...
class ModelMetricsHandler extends Handler {
	/**
	 * Class which contains the internal representation of the ModelMetrics list and
	 * params.
	 */
	...
	/**
	 * Schema for a list of ModelMetricsBaseV3. This should be common across all
	 * versions of ModelMetrics schemas, so it lives here. TODO: move to
	 * water.api.schemas3
	 */
	public static final class ModelMetricsListSchemaV3
		...
		public ModelMetricsHandler.ModelMetricsList fillImpl(ModelMetricsList mml) {
			...
			mml._predictions_name = (null == this.predictions_frame || null == this.predictions_frame.key() ? null
					: this.predictions_frame.key().toString());
		...
		public ModelMetricsListSchemaV3 fillFromImpl(ModelMetricsList mml) {
			// TODO: this is failing in PojoUtils with an IllegalAccessException. Why?
			...
			this.predictions_frame = (mml._predictions_name == null ? null
					: new KeyV3.FrameKeyV3(Key.<Frame>make(mml._predictions_name)));
			this.deviances_frame = (mml._deviances_name == null ? null
					: new KeyV3.FrameKeyV3(Key.<Frame>make(mml._deviances_name)));
	...
	public ModelMetricsListSchemaV3 score(int version, ModelMetricsListSchemaV3 s) {
		...
		parms._model.score(parms._frame, parms._predictions_name).remove(); // throw away predictions, keep metrics as a
																			// side-effect
	...
	public static final class ModelMetricsMakerSchemaV3 extends SchemaV3<ModelMetricsMaker, ModelMetricsMakerSchemaV3> {
		...
		@API(help = "Distribution (for regression).", direction = API.Direction.INOUT, values = { "gaussian", "poisson",
				"gamma", "laplace" })
	...
	public ModelMetricsMakerSchemaV3 make(int version, ModelMetricsMakerSchemaV3 s) {
		...
		if (s.domain == null) {
			if (pred.numCols() != 1) {
				throw new H2OIllegalArgumentException("predictions_frame", "make",
						"For regression problems (domain=null), the predictions_frame must have exactly 1 column.");
			}
			...
			if (pred.numCols() != 1) {
				throw new H2OIllegalArgumentException("predictions_frame", "make",
						"For domains with 2 class labels, the predictions_frame must have exactly one column containing the class-1 probabilities.");
			}
			...
			if (pred.numCols() != s.domain.length) {
				throw new H2OIllegalArgumentException("predictions_frame", "make",
						"For domains with " + s.domain.length
								+ " class labels, the predictions_frame must have exactly " + s.domain.length
								+ " columns containing the class-probabilities.");
			}
	...
	/**
	 * Score a frame with the given model and return the metrics AND the prediction
	 * frame.
	 */
	@SuppressWarnings("unused") // called through reflection by RequestServer
	public JobV3 predictAsync(int version, final ModelMetricsListSchemaV3 s) {
		...
		// predict2 does not return modelmetrics, so cannot handle deeplearning:
		// reconstruction_error (anomaly) or GLRM: reconstruct and archetypes
		...
		if (s.deep_features_hidden_layer > 0 || s.deep_features_hidden_layer_name != null) {
			...
			if (parms._exemplar_index >= 0) {
				parms._predictions_name = "members_" + parms._model._key.toString() + "_for_exemplar_"
						+ parms._exemplar_index;
			} else {
				parms._predictions_name = "predictions" + Key.make().toString().substring(0, 5) + "_"
						+ parms._model._key.toString() + "_on_" + parms._frame._key.toString();
			}
		}
		...
		H2O.H2OCountedCompleter work = new H2O.H2OCountedCompleter() {
			...
			public void compute2() {
				if (s.deep_features_hidden_layer < 0 && s.deep_features_hidden_layer_name == null) {
					...
					try {
						predictions = ((Model.DeepFeatures) parms._model).scoreDeepFeatures(parms._frame,
								s.deep_features_hidden_layer_name, j);
					} catch (IllegalArgumentException e) {
						Log.warn(e.getMessage());
						throw e;
					}
					if (predictions != null) {
						predictions = new Frame(Key.<Frame>make(parms._predictions_name), predictions.names(),
								predictions.vecs());
						DKV.put(predictions._key, predictions);
					}
				} else {
					Frame predictions = ((Model.DeepFeatures) parms._model).scoreDeepFeatures(parms._frame,
							s.deep_features_hidden_layer, j);
					predictions = new Frame(Key.<Frame>make(parms._predictions_name), predictions.names(),
							predictions.vecs());
					DKV.put(predictions._key, predictions);
				}
				if ((parms._model._warningsP != null) && (parms._model._warningsP.length > 0)) { // add prediction
																									// warning here only
					String[] allWarnings = (String[]) ArrayUtils.addAll(j.warns(), parms._model._warningsP); // copy
																												// both
																												// over
		...
		if (!s.reconstruction_error && !s.reconstruction_error_per_feature && s.deep_features_hidden_layer < 0
			...
			if (s.deviances) {
				...
				if (null == parms._deviances_name) {
					parms._deviances_name = "deviances" + Key.make().toString().substring(0, 5) + "_"
							+ parms._model._key.toString() + "_on_" + parms._frame._key.toString();
				}
			...
			if (s.deviances) {
				throw new H2OIllegalArgumentException(
						"Cannot compute deviances in combination with other special predictions.");
			}
			if (Model.DeepFeatures.class.isAssignableFrom(parms._model.getClass())) {
				if (s.reconstruction_error || s.reconstruction_error_per_feature) {
					...
					if (null == parms._predictions_name) {
						parms._predictions_name = "reconstruction_error" + Key.make().toString().substring(0, 5) + "_"
								+ parms._model._key.toString() + "_on_" + parms._frame._key.toString();
					}
					predictions = ((Model.DeepFeatures) parms._model).scoreAutoEncoder(parms._frame,
							Key.make(parms._predictions_name), parms._reconstruction_error_per_feature);
				} else {
					if (s.deep_features_hidden_layer < 0) {
				...
				if (s.project_archetypes) {
					if (parms._predictions_name == null) {
						parms._predictions_name = "reconstructed_archetypes_" + Key.make().toString().substring(0, 5)
								+ "_" + parms._model._key.toString() + "_of_" + parms._frame._key.toString();
					}
					predictions = ((Model.GLRMArchetypes) parms._model).scoreArchetypes(parms._frame,
							Key.<Frame>make(parms._predictions_name), s.reverse_transform);
				} else {
					assert s.reconstruct_train;
					if (parms._predictions_name == null) {
						parms._predictions_name = "reconstruction_" + Key.make().toString().substring(0, 5) + "_"
								+ parms._model._key.toString() + "_of_" + parms._frame._key.toString();
					}
					predictions = ((Model.GLRMArchetypes) parms._model).scoreReconstruction(parms._frame,
							Key.<Frame>make(parms._predictions_name), s.reverse_transform);
				}
			} else if (s.leaf_node_assignment) {
				assert (Model.LeafNodeAssignment.class.isAssignableFrom(parms._model.getClass()));
				if (null == parms._predictions_name) {
					parms._predictions_name = "leaf_node_assignment" + Key.make().toString().substring(0, 5) + "_"
							+ parms._model._key.toString() + "_on_" + parms._frame._key.toString();
				}
				predictions = ((Model.LeafNodeAssignment) parms._model).scoreLeafNodeAssignment(parms._frame,
						Key.<Frame>make(parms._predictions_name));
				...
				predictions = ((Model.ExemplarMembers) parms._model)
						.scoreExemplarMembers(Key.<Frame>make(parms._predictions_name), parms._exemplar_index);
			} else
				throw new H2OIllegalArgumentException("Requires a Deep Learning, GLRM, DRF or GBM model.",
