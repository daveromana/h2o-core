...
public class CStrChunk extends Chunk {
	 ...
	 * 
	 * @param sslen
	 *            Next offset into ss for placing next String
	 * @param ss
	 *            Bytes of appended strings, including trailing 0
	 * @param sparseLen
	 *            Length of sparse chunk (number of extracted (non-zero) elements)
	 * @param idxLen
	 *            Length of chunk
	 * @param id
	 *            Indices (row numbers) of stored values, used for sparse
	 * @param is
	 *            Index of strings - holds offsets into ss[]. is[i] == -1 means
	 *            NA/sparse
	 */
	...
	private void init(int sslen, byte[] ss, int sparseLen, int idxLen, int[] id, int[] is) {
		...
		for (int i = 0; i < sparseLen; ++i) // Copy the sparse indices
			UnsafeUtils.set4(_mem, idx(id == null ? i : id[i]), is == null ? 0 : is[i]); // Need to check if id and is
																							// are null since both are
																							// not always needed for mem
																							// allocation
	...
	public int intAt(int i) {
		return UnsafeUtils.get4(_mem, idx(i));
	}

	public byte byteAt(int i) {
		return _mem[_valstart + i];
	}
	...
	/**
	 * Optimized toLower() method to operate across the entire CStrChunk buffer in
	 * one pass. This method only changes the values of ASCII uppercase letters in
	 * the text.
	 *
	 * NewChunk is the same size as the original.
	 *
	 * @param nc
	 *            NewChunk to be filled with the toLower version of ASCII strings in
	 *            this chunk
	...
	/**
	 * Optimized toUpper() method to operate across the entire CStrChunk buffer in
	 * one pass. This method only changes the values of ASCII lowercase letters in
	 * the text.
	 *
	 * NewChunk is the same size as the original.
	 *
	 * @param nc
	 *            NewChunk to be filled with the toUpper version of ASCII strings in
	 *            this chunk
	 * @return Filled NewChunk
	 */
	...
	/**
	 * Optimized trim() method to operate across the entire CStrChunk buffer in one
	 * pass. This mimics Java String.trim() by only considering characters of value
	 * <code>'&#92;u0020'</code> or less as whitespace to be trimmed. This means
	 * that like Java's String.trim() it ignores 16 of the 17 characters regarded as
	 * a space in UTF.
	 *
	 * NewChunk is the same size as the original, despite trimming.
	 *
	 * @param nc
	 *            NewChunk to be filled with trimmed version of strings in this
	 *            chunk
	 * @return Filled NewChunk
	 */
	public NewChunk asciiTrim(NewChunk nc) {
		...
		for (int i = 0; i < _len; i++) {
			...
			if (off != NA) {
				// UTF chars will appear as negative values. In Java spec, space is any char
				// 0x20 and lower
				...
				j--;
				while (_mem[_valstart + off + j] > 0 && _mem[_valstart + off + j] < 0x21) { // March back to find first
																							// non-space
	...
	/**
	 * Optimized substring() method for a buffer of only ASCII characters. The
	 * presence of UTF-8 multi-byte characters would give incorrect results for the
	 * string length, which is required here.
	 *
	 * @param nc
	 *            NewChunk to be filled with substrings in this chunk
	 * @param startIndex
	...
	/**
	 * Optimized length() method for a buffer of only ASCII characters. This is a
	 * straight byte count for each word in the chunk. The presence of UTF-8
	 * multi-byte characters would give incorrect results.
	 *
	 * @param nc
	 *            NewChunk to be filled with lengths of strings in this chunk
	 * @return Filled NewChunk
	 */
	...
	/**
	 * Optimized lstrip() & rstrip() methods to operate across the entire CStrChunk
	 * buffer in one pass.
	 *
	 * NewChunk is the same size as the original, despite trimming.
	 *
	 * @param nc
	 *            NewChunk to be filled with strip version of strings in this chunk
	 * @param chars
	 *            chars to strip, treated as ASCII
	 * @return Filled NewChunk
	 */
