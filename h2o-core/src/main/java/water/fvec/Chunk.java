...
/**
 * A compression scheme, over a chunk of data - a single array of bytes. Chunks
 * are mapped many-to-1 to a {@link Vec}. The <em>actual</em> vector header info
 * is in the Vec - which contains info to find all the bytes of the distributed
 * vector. Subclasses of this abstract class implement (possibly empty)
 * compression schemes.
 *
 * <p>
 * Chunks are collections of elements, and support an array-like API. Chunks are
 * subsets of a Vec; while the elements in a Vec are numbered starting at 0, any
 * given Chunk has some (probably non-zero) starting row, and a length which is
 * smaller than the whole Vec. Chunks are limited to a single Java byte array in
 * a single JVM heap, and only an int's worth of elements. Chunks support both
 * the notions of a global row-number and a chunk-local numbering. The global
 * row-number calls are variants of {@code
 ...
 *
 * <p>
 * The array-like API allows loading and storing elements in and out of Chunks.
 * When loading, values are decompressed. When storing, an attempt to compress
 * back into the actual underlying Chunk subclass is made; if this fails the
 * Chunk is "inflated" into a {@link NewChunk}, and the store completed there.
 * Later the NewChunk will be compressed (probably into a different underlying
 * Chunk subclass) and put back in the K/V store under the same Key -
 * effectively replacing the original Chunk; this is done when {@link #close} is
 * called, and is taken care of by the standard {@link MRTask} calls.
 *
 * <p>
 ...
 *
 * <p>
 * In addition to normal load and store operations, Chunks support the notion a
 * missing element via the {@link #isNA} call, and a "next non-zero" notion for
 * rapidly iterating over sparse data.
 *
 * <p>
 * <b>Data Types</b>
 *
 * <p>
 * Chunks hold Java primitive values, timestamps, UUIDs, or Strings. All the
 * Chunks in a Vec hold the same type. Most of the types are compressed. Integer
 * types (boolean, byte, short, int, long) are always lossless. Float and Double
 * types might lose 1 or 2 ulps in the compression. Time data is held as
 * milliseconds since the Unix Epoch. UUIDs are held as 128-bit integers (a pair
 * of Java longs). Strings are compressed in various obvious ways. Sparse data
 * is held... sparsely; e.g. loading data in SVMLight format will not "blow up"
 * the in-memory representation. Categoricals/factors are held as small
 * integers, with a shared String lookup table on the side.
 *
 * <p>
 * Chunks support the notion of <em>missing</em> data. Missing float and double
 * data is always treated as a NaN, both if read or written. There is no
 * equivalent of NaN for integer data; reading a missing integer value is a
 * coding error and will be flagged. If you are working with integer data with
 * missing elements, you must first check for a missing value before loading it:
 * 
 * <pre>
 * {@code
 ...
 *
 * <p>
 * The same holds true for the other non-real types (timestamps, UUIDs, Strings,
 * or categoricals): they must be checked for missing before being used.
 *
 * <p>
 * <b>Performance Concerns</b>
 *
 * <p>
 * The standard {@code for} loop mentioned above is the fastest way to access
 * data; definitely faster (and less error prone) than iterating over global row
 * numbers. Iterating over a single Chunk is nearly always memory-bandwidth
 * bound. Often code will iterate over a number of Chunks aligned together (the
 * common use-case of looking a whole rows of a dataset). Again, typically such
 * a code pattern is memory-bandwidth bound although the X86 will stop being
 * able to prefetch well beyond 100 or 200 Chunks.
 *
 * <p>
 * Note that Chunk alignment is guaranteed within all the Vecs of a Frame: Same
 * numbered Chunks of <em>different</em> Vecs will have the same global row
 * numbering and the same length, enabling a particularly simple and efficient
 * way to iterate over all rows.
 *
 * <p>
 * This example computes the Euclidean distance between all the columns and a
 * given point, and stores the squared distance back in the last column. Note
 * that due "NaN poisoning" if any row element is missing, the entire distance
...
public abstract class Chunk extends Iced<Chunk> implements Vec.Holder {
	...
	/**
	 * Number of rows in this Chunk; publicly a read-only field. Odd API design
	 * choice: public, not-final, read-only, NO-ACCESSOR.
	 *
	 * <p>
	 * NO-ACCESSOR: This is a high-performance field, and must have a known
	 * zero-cost cost-model; accessors hide that cost model, and make it not-obvious
	 * that a loop will be properly optimized or not.
	 *
	 * <p>
	...
	/**
	 * Internal set of _len. Used by lots of subclasses. Not a publically visible
	 * API.
	 */
	int set_len(int len) {
		return _len = len;
	...
	/**
	 * Normally==null, changed if chunk is written to. Not a publically readable or
	 * writable field.
	 */
	...
	/**
	 * The Big Data. Frequently set in the subclasses, but not otherwise a
	 * publically writable field.
	 */
	byte[] _mem;

	/**
	 * Short-cut to the embedded big-data memory. Generally not useful for public
	 * consumption, since the data remains compressed and holding on to a pointer to
	 * this array defeats the user-mode spill-to-disk.
	 */
	...
	/**
	 * Load a {@code double} value using absolute row numbers. Returns Double.NaN if
	 * value is missing.
	 *
	 * <p>
	 * This version uses absolute element numbers, but must convert them to
	 * chunk-relative indices - requiring a load from an aliasing local var, leading
	 * to lower quality JIT'd code (similar issue to using iterator objects).
	 *
	 * <p>
	...
	/**
	 * Load a {@code double} value using chunk-relative row numbers. Returns
	 * Double.NaN if value is missing.
	 * 
	 * @return double value at the given row, or NaN if the value is missing
	 */
	public final double atd(int i) {
		return _chk2 == null ? atd_impl(i) : _chk2.atd_impl(i);
	}

	/**
	 * Load a {@code long} value using chunk-relative row numbers. Floating point
	...
	public final long at8(int i) {
		return _chk2 == null ? at8_impl(i) : _chk2.at8_impl(i);
	}
	...
	public final long at16l(int i) {
		return _chk2 == null ? at16l_impl(i) : _chk2.at16l_impl(i);
	}
	...
	public final long at16h(int i) {
		return _chk2 == null ? at16h_impl(i) : _chk2.at16h_impl(i);
	}
	...
	public final BufferedString atStr(BufferedString bStr, int i) {
		return _chk2 == null ? atStr_impl(bStr, i) : _chk2.atStr_impl(bStr, i);
	}
	...
	/**
	 * Write a {@code long} using absolute row numbers. There is no way to write a
	 * missing value with this call. Under rare circumstances this can throw: if the
	 * long does not fit in a double (value is larger magnitude than 2^52), AND
	 * float values are stored in Vector. In this case, there is no common
	 * compatible data representation.
	 *
	 * <p>
	 * As with all the {@code set} calls, if the value written does not fit in the
	 * current compression scheme, the Chunk will be inflated into a NewChunk and
	 * the value written there. Later, the NewChunk will be compressed (after a
	 * {@link #close} call) and written back to the DKV. i.e., there is some
	 * interesting cost if Chunk compression-types need to change.
	 *
	 * <p>
	 * This version uses absolute element numbers, but must convert them to
	 * chunk-relative indices - requiring a load from an aliasing local var, leading
	...
	/**
	 * Write a {@code double} using absolute row numbers; NaN will be treated as a
	 * missing value.
	 *
	 * <p>
	 * As with all the {@code set} calls, if the value written does not fit in the
	...
	/**
	 * Write a {@code float} using absolute row numbers; NaN will be treated as a
	 * missing value.
	 *
	 * <p>
	 * As with all the {@code set} calls, if the value written does not fit in the
	...
	/**
	 * Write a {@code long} with check-relative indexing. There is no way to write a
	 * missing value with this call. Under rare circumstances this can throw: if the
	 * long does not fit in a double (value is larger magnitude than 2^52), AND
	 * float values are stored in Vector. In this case, there is no common
	 * compatible data representation.
	...
	/**
	 * Write a {@code double} with check-relative indexing. NaN will be treated as a
	 * missing value.
	 *
	 * <p>
	 * As with all the {@code set} calls, if the value written does not fit in the
	...
	/**
	 * Write a {@code float} with check-relative indexing. NaN will be treated as a
	 * missing value.
	 *
	 * <p>
	 * As with all the {@code set} calls, if the value written does not fit in the
	...
	/**
	 * Write a {@code String} with check-relative indexing. {@code null} will be
										...
										: x instanceof Integer ? set(idx, ((Integer) x).longValue())
												: x instanceof UUID ? set(idx, (UUID) x)
														: x instanceof java.util.Date
																? set(idx, ((java.util.Date) x).getTime())
																:
	 ...
	 * NewChunk was needed, it will be compressed into some other kind of Chunk. The
	 * resulting Chunk (either a modified self, or a compressed NewChunk) will be
	 * written to the DKV. Only after that {@code DKV.put} completes will all
	 * readers of this Chunk witness the changes.
	 * 
	...
	/**
	 * Chunk-specific writer. Returns false if the value does not fit in the current
	 * compression scheme.
	 */
	...
	/**
	 * Sparse Chunks have a significant number of zeros, and support for skipping
	 * over large runs of zeros in a row.
	 * 
	 ...
	 * over large runs of zeros in a row.
	 * 
	 * @return At least as large as the count of non-zeros, but may be significantly
	 *         smaller than the {@link #_len}
	 */
	...
	/**
	 * Sparse Chunks have a significant number of NAs, and support for skipping over
	 * large runs of NAs in a row.
	 * 
	 ...
	 * large runs of NAs in a row.
	 * 
	 * @return At least as large as the count of non-NAs, but may be significantly
	 *         smaller than the {@link #_len}
	 */
	 ...
	 * stored in this chunk. For dense chunks, this will contain indices of all the
	 * rows in this chunk.
	 * 
	 * @return array of chunk-relative indices of values stored in this chunk.
	 */
	...
	/**
	 * Report the Chunk min-value (excluding NAs), or NaN if unknown. Actual min can
	 * be higher than reported. Used to short-cut RollupStats for constant and
	 * boolean chunks.
	 */
	double min() {
		return Double.NaN;
	...
	/**
	 * Report the Chunk max-value (excluding NAs), or NaN if unknown. Actual max can
	 * be lower than reported. Used to short-cut RollupStats for constant and
	 * boolean chunks.
	 */
	double max() {
		return Double.NaN;
	...
	public final NewChunk inflate() {
		return extractRows(new NewChunk(this), 0, _len);
	}
	...
	public Chunk nextChunk() {
		return _vec.nextChunk(this);
	}

	/** @return String version of a Chunk, class name and range */
	@Override
	public String toString() {
		return getClass().getSimpleName() + "[" + _start + ".." + (_start + _len - 1) + "]";
	}
	...
	/**
	 * Custom serializers implemented by Chunk subclasses: the _mem field contains
	 * ALL the fields already.
	 */
	public final AutoBuffer write_impl(AutoBuffer bb) {
		return bb.putA1(_mem);
	}
	...
	public byte precision() {
		return -1;
	} // Digits after the decimal, or -1 for "all"
	...
	// //int w=1/*blank/sign*/+lg/*compression limits
	// digits*/+1/*dot*/+1/*e*/+1/*neg exp*/+2/*digits of exp*/;
	// //return w;
	// }

	/**
	 * Used by the parser to help report various internal bugs. Not intended for
	 * public use.
	 */
	public final void reportBrokenCategorical(int i, int j, long l, int[] cmap, int levels) {
		StringBuilder sb = new StringBuilder("Categorical renumber task, column # " + i + ": Found OOB index " + l
				+ " (expected 0 - " + cmap.length + ", global domain has " + levels + " levels) pulled from "
	...
	/**
	 * Sparse bulk interface, stream through the compressed values and extract them
	 * into dense double array.
	 * 
	 * @param vals
	 *            holds extracted values, length must be >= this.sparseLen()
	 * @param ids
	 *            holds extracted chunk-relative row ids, length must be >=
	 *            this.sparseLen()
	 * @return number of extracted (non-zero) elements, equal to sparseLen()
	 */
	public int getSparseDoubles(double[] vals, int[] ids) {
		return getSparseDoubles(vals, ids, Double.NaN);
	}
