...
/**
 * A collection of named {@link Vec}s, essentially an R-like Distributed Data
 * Frame.
 *
 * <p>
 * Frames represent a large distributed 2-D table with named columns
 * ({@link Vec}s) and numbered rows. A reasonable <em>column</em> limit is 100K
 * columns, but there's no hard-coded limit. There's no real <em>row</em> limit
 * except memory; Frames (and Vecs) with many billions of rows are used
 * routinely.
 *
 * <p>
 ...
 *
 * <p>
 * Multiple Frames can reference the same Vecs, although this sharing can make
 * Vec lifetime management complex. Commonly temporary Frames are used to work
 * with a subset of some other Frame (often during algorithm execution, when
 * some columns are dropped from the modeling process). The temporary Frame can
 * simply be ignored, allowing the normal GC process to reclaim it. Such temp
 * Frames usually have a {@code null} key.
 *
 * <p>
 * All the Vecs in a Frame belong to the same {@link Vec.VectorGroup} which then
 * enforces {@link Chunk} row alignment across Vecs (or at least enforces a
 * low-cost access model). Parallel and distributed execution touching all the
 * data in a Frame relies on this alignment to get good performance.
 *
 * <p>
 ...
 *
 * <p>
 * Example: Find and remove the Vec called "unique_id" from the Frame, since
 * modeling with a unique_id can lead to overfitting:
 * 
 * <pre>
...
public class Frame extends Lockable<Frame> {
	...
	/**
	 * Creates an internal frame composed of the given Vecs and default names. The
	 * frame has no key.
	 */
	...
	/**
	 * Creates an internal frame composed of the given Vecs and names. The frame has
	 * no key.
	 */
	...
	/**
	 * Special constructor for data with unnamed columns (e.g. svmlight) bypassing
	 * *all* checks.
	 */
	...
	public Frame(Key<Frame> key, String names[], Vec vecs[]) {
		...
		if (names == null) { // Make default names, all known to be unique
			...
			for (int i = 0; i < vecs.length; i++)
				_keys[i] = vecs[i]._key;
			for (int i = 0; i < vecs.length; i++)
				checkCompatibility(_names[i], vecs[i]);
			_lastNameBig = true;
		} else {
	...
	public final void setNames(String[] columns) {
		if (_vecs != null && columns.length != _vecs.length) {
			throw new IllegalArgumentException(
					"Number of column names=" + columns.length + " must be the number of vecs=" + _vecs.length);
		}
		_names = columns;
	}

	/**
	 * Deep copy of Vecs and Keys and Names (but not data!) to a new random Key. The
	 * resulting Frame does not share with the original, so the set of Vecs can be
	 * freely hacked without disturbing the original Frame.
	 */
	 ...
	 * 
	 * @param size
	 *            number of elements in the array that will be created.
	 */
	...
	private int pint(String pintname) {
		try {
			return Integer.valueOf(pintname.substring(1));
	 ...
	 * sharded using same number of rows per chunk, and all names are unique. Throw
	 * an IAE if something does not match.
	 */
	private void checkCompatibility(String compname, Vec compvec) {
		...
		if (!v0.isCompatibleWith(compvec)) {
			...
			if (!Arrays.equals(v0.espc(), compvec.espc())) {
				Log.err("Unexpected incompatible espc, " + Arrays.toString(v0.espc()) + " != "
						+ Arrays.toString(vec.espc()));
			}
	...
	/**
	 * Frames are compatible if they have the same layout (number of rows and
	 * chunking) and the same vector group (chunk placement)..
	 */
	...
	/**
	 * The internal array of Vecs. For efficiency Frames contain an array of Vec
	 * Keys - and the Vecs themselves are lazily loaded from the {@link DKV}.
	 * 
	...
	/**
	 * Force a cache-flush and reload, assuming vec mappings were altered remotely,
	 * or that the _vecs array was shared and now needs to be a defensive copy.
	 * 
	 ...
	 * @return a Vec by name, or null if missing
	 */
	public Vec vec(String name_vec) {
		int idx = find(name_vec);
		return idx == -1 ? null : vecs()[idx];
	}
	...
	/**
	 * Number of categorical levels for categorical columns; -1 for non-categorical
	 * columns.
	 * 
	...
	/**
	 * 64-bit checksum of the checksums of the vecs. SHA-265 checksums of the chunks
	 * are XORed together. Since parse always parses the same pieces of files into
	 * the same offsets in some chunk this checksum will be consistent across
	 * reparses.
	 * 
	...
	/**
	 * Append multiple named Vecs to the Frame. Names are forced unique, by
	 * appending a unique number if needed.
	 */
	private void bulkAdd(String[] blk_names, Vec[] vecs) {
		String[] tmpnames = blk_names.clone();
		int N = blk_names.length;
		assert (blk_names.length == vecs.length) : "names = " + Arrays.toString(blk_names) + ", vecs len = "
				+ vecs.length;
		for (int i = 0; i < N; ++i) {
		...
		// make temp arrays and don't assign them back until they are fully filled -
		// otherwise vecs() can cache null's and NPE.
	...
	/**
	 * Append a Frame onto this Frame. Names are forced unique, by appending unique
	 * numbers if needed.
	 * 
	 * @return the expanded Frame, for flow-coding
	 */
	public Frame add(Frame fr) {
		add(fr._names, fr.vecs().clone(), fr.numCols());
		return this;
	}
	...
	public Frame prepend(String p_name, Vec vec) {
		...
		if (_vecs.length != 0) {
			if (!anyVec().group().equals(vec.group()) && !Arrays.equals(anyVec().espc(), vec.espc())) {
				throw new IllegalArgumentException("Vector groups differs - adding vec '" + p_name + "' into the frame "
						+ Arrays.toString(_names));
			}
			if (numRows() != vec.length()) {
				throw new IllegalArgumentException("Vector lengths differ - adding vec '" + p_name + "' into the frame "
						+ Arrays.toString(_names));
			}
		}
	...
	/**
	 * move the provided columns to be first, in-place. For Merge currently since
	 * method='hash' was coded like that
	 */
	 ...
	 * @return a new frame which collects vectors from this frame with desired
	 *         names.
	 * @throws IllegalArgumentException
	 *             if there is no vector with desired name in this frame.
	 */
	public Frame subframe(String[] sub_names) {
		return subframe(sub_names, false, 0)[0];
	}

	/**
	 * Create a subframe from this frame based on desired names. Throws an exception
	 * if desired column is not in this frame and <code>replaceBy</code> is
	 * <code>false</code>. Else replace a missing column by a constant column with
	 * given value.
	 *
	 * @param names
	 *            list of column names to extract
	 * @param replaceBy
	 *            should be missing column replaced by a constant column
	 * @param c
	 *            value for constant column
	 * @return array of 2 frames, the first is containing a desired subframe, the
	 *         second one contains newly created columns or null
	 * @throws IllegalArgumentException
	 *             if <code>replaceBy</code> is false and there is a missing column
	 *             in this frame
	 */
	private Frame[] subframe(String[] s_names, boolean replaceBy, double c) {
		...
		HashMap<String, Integer> map = new HashMap<>((int) ((names.length / 0.75f) + 1)); // avoid rehashing by set up
																							// initial capacity
		for (int i = 0; i < _names.length; ++i)
			map.put(_names[i], i);
		...
		return new Frame[] { new Frame(Key.<Frame>make("subframe" + Key.make().toString()), s_names, vecs),
				ccv > 0 ? new Frame(Key.<Frame>make("subframe" + Key.make().toString()), Arrays.copyOf(cnames, ccv),
						Arrays.copyOf(cvecs, ccv)) : null };
	}

	/**
	 * Allow rollups for all written-into vecs; used by {@link MRTask} once writing
	 * is complete.
	 * 
	...
	/**
	 * Replace one column with another. Caller must perform global update (DKV.put)
	 * on this updated frame.
	 * 
	 ...
	 * @param endIdx
	 *            index of the last column (exclusive)
	...
	/**
	 * Split this Frame; return a subframe created from the given column interval,
	 * and remove those columns from this Frame.
	 * 
	 * @param startIdx
	 *            index of first column (inclusive)
	 * @param endIdx
	 ...
	 */
	public Vec remove(String r_name) {
		return remove(find(r_name));
	}
	 ...
	 * 
	 * @param startIdx
	 *            - start index of column (inclusive)
	 * @param endIdx
	 *            - end index of column (exclusive)
	...
	/**
	 * Restructure a Frame completely, but only for a specified number of columns
	 * (counting up)
	 */
	...
	void finalizePartialFrame(long[] frame_espc, String[][] frame_domains, byte[] frame_types) {
		...
		for (int i = 0; i < _keys.length; i++) {
			// Insert Vec header
			Vec vec = _vecs[i] = new Vec(_keys[i], Vec.ESPC.rowLayout(_keys[i], espc2),
					domains != null ? domains[i] : null, types[i]);
	 ...
	 *
	 * <p>
	 * The numbering is 1-based; zero's are not allowed in the lists, nor are
	 * out-of-range values.
	 * 
	...
	public Frame deepSlice(Object orows, Object ocols) {
		...
		if (ocols == null) {
			...
			if (fr.numCols() != 1) {
				throw new IllegalArgumentException(
						"Columns Frame must have only one column (actually has " + fr.numCols() + " columns)");
			}
			...
			throw new IllegalArgumentException(
					"Columns is specified by an unsupported data type (" + ocols.getClass().getName() + ")");
		...
		for (int aC2 : c2)
			if (aC2 >= numCols()) {
				throw new IllegalArgumentException(
						"Trying to select column " + (aC2 + 1) + " but only " + numCols() + " present.");
			}
		if (c2.length == 0) {
			throw new IllegalArgumentException(
		...
		if (orows == null) {
			...
			if (rows.length == 0 || rows[0] < 0) {
				if (rows.length != 0 && rows[0] < 0) {
					...
					Frame slicedFrame = new DeepSlice(rows, c2, vecs()).doAll(types(c2), this.add("select_vec", v))
							.outputFrame(names(c2), domains(c2));
					Keyed.remove(v._key);
					Keyed.remove(this.remove(this.numCols() - 1)._key);
					return slicedFrame;
				} else {
					return new DeepSlice(rows.length == 0 ? null : rows, c2, vecs()).doAll(types(c2), this)
							.outputFrame(names(c2), domains(c2));
				}
			}
	...
	public String toString(long off, int len) {
		return toTwoDimTable(off, len).toString();
	}

	public String toString(long off, int len, boolean rollups) {
		return toTwoDimTable(off, len, rollups).toString();
	}

	public TwoDimTable toTwoDimTable() {
		return toTwoDimTable(0, 10);
	}

	public TwoDimTable toTwoDimTable(long off, int len) {
		return toTwoDimTable(off, len, true);
	}
	...
	public TwoDimTable toTwoDimTable(long off, int len, boolean rollups) {
		...
		for (int i = 0; i < ncols; i++) {
			if (DKV.get(_keys[i]) == null) { // deleted Vec in Frame
				...
				for (int j = 0; j < len + H; j++)
					dblCells[j][i] = TwoDimTable.emptyDouble;
				for (int j = 0; j < len; j++)
		...
		return new TwoDimTable("Frame " + _key, numRows() + " rows and " + numCols() + " cols", rowHeaders,
				/* clone the names, the TwoDimTable will replace nulls with "" */_names.clone(), coltypes, null, "",
				strCells, dblCells);
	}
		...
		public void map(Chunk chks[], NewChunk nchks[]) {
			...
			while (true) { // Still got rows to include?
				if (_rows != null) { // Got a row selector?
					if (rx >= _rows.length) {
						break; // All done with row selections
					}
	...
	/**
	 * Create a copy of the input Frame and return that copied Frame. All Vecs in
	 * this are copied in parallel. Caller must do the DKV.put
	 * 
	 * @param keyName
	 ...
	 * Note that this can, in the worst case, copy all of {@code this}s' data.
	 * 
	 * @return This Frame's data in an array of Vectors that is compatible with
	 *         {@code f}.
	 */
	...
	/**
	 * Convert this Frame to a CSV (in an {@link InputStream}), that optionally is
	 * compatible with R 3.1's recent change to read.csv()'s behavior.
	 *
	...
	public static class CSVStream extends InputStream {
			...
			for (int i = 0; i < _curChks.length; i++) {
				...
				if (!_curChks[i].isNA(_chkRow)) {
					if (v.isCategorical()) {
						...
						// (Really type.convert()).
						//
						// Numeric values with too much precision now trigger a type conversion in R 3.1
						// into a factor.
						//
		...
		public int available() throws IOException {
			...
			// Case 4: Out of data in the current chunks => fast-forward to the next set of
			// non-empty chunks.
