...
public class InteractionWrappedVec extends WrappedVec {
  ...
  public int expandedLength() {
    if( _v1Domain==null && _v2Domain==null ) {
    	...
    	return domain().length; // 2 cat -> domains (limited) length
    }
    else if( _v1Domain!=null ) {
    	return _v1Enums==null?_v1Domain.length - (_useAllFactorLevels?0:1):_v1Enums.length-(_useAllFactorLevels?0:1);
    }
