...
/**
 * A class to compute the rollup stats. These are computed lazily, thrown away
 * if the Vec is written into, and then recomputed lazily. Error to ask for them
 * if the Vec is actively being written into. It is common for all cores to ask
 * for the same Vec rollup at once, so it is crucial that it be computed once
 * across the cluster.
 *
 * Rollups are kept in the K/V store, which also controls who manages the rollup
 * work and final results. Winner of a DKV CAS/PutIfMatch race gets to manage
 * the M/R job computing the rollups. Losers block for the same rollup. Remote
 * requests *always* forward to the Rollup Key's master.
 */
final class RollupStats extends Iced {
	/**
	 * The count of missing elements.... or -2 if we have active writers and no
	 * rollup info can be computed (because the vector is being rapidly modified!),
	 * or -1 if rollups have not been computed since the last modification.
	 */
	...
	long _rows, // count(X) for non-NA values excluding negative/positive infinities (for
				// numeric Vecs)
	...
	// the choice of MAX_SIZE being a power of 10 (rather than 1024) just
	// aligns-to-the-grid of the common input of fixed decimal
	// precision numbers. It is still an estimate and makes no difference
	// mathematically. It just gives tidier output in some
	...
	private RollupStats(int mode) {
		...
		Arrays.fill(_pctiles, Double.NaN);
		_mean = _sigma = 0;
		_size = 0;
		_naCnt = mode;
	}
	...
	private RollupStats map(Chunk c) {
		...
		if (isUUID) { // UUID columns do not compute min/max/mean/sigma
			...
			// we need to merge with the mean (0) and variance (0) of the zeros count of 0s
			// of the sparse chunk - which were skipped above
			...
			if (c.isSparseZero()) {
				...
				if (zeros > 0) {
					...
					_sigma += zeroM2 + delta * delta * _rows * zeros / (_rows + zeros); // this is the variance*(N-1),
																						// will do sqrt(_sigma/(N-1))
																						// later in postGlobal
					_rows += zeros;
				}
	...
	private static class Roll extends MRTask<Roll> {
		...
		Roll(H2OCountedCompleter cmp, Key rskey) {
			super(cmp);
			_rskey = rskey;
		}
		...
		// Just toooo common to report always. Drowning in multi-megabyte log file
		// writes.
		@Override
		...
		/**
		 * Added to avoid deadlocks when running from idea in debug mode (evaluating
		 * toSgtring on mr task causes rollups to be computed)
		 * 
		 * @return
		 */
		@Override
	...
	static RollupStats get(Vec vec, boolean computeHisto) {
		...
		while (rs == null || (!rs.isReady() || (computeHisto && !rs.hasHisto()))) {
			...
			// 2. fetch - done in two steps to go through standard DKV.get and enable local
			// caching
			rs = DKV.getGet(rskey);
		}
		return rs;
	...
	double h_stride() {
		return h_stride(_bins.length);
	}

	private double h_stride(int nbins) {
		return (_maxs[0] - _mins[0] + (_isInt ? 1 : 0)) / nbins;
	}
	...
	private static class Histo extends MRTask<Histo> {
		...
		Histo(H2OCountedCompleter cmp, RollupStats rs, int nbins) {
			super(cmp);
			_base = rs.h_base();
			_stride = rs.h_stride(nbins);
			_nbins = nbins;
		}
		...
		private int idx(double d) {
			int idx = (int) ((d - _base) / _stride);
			return Math.min(idx, _bins.length - 1);
		}
	...
	// Only computes the rollups, does not fetch them, caller should fetch them via
	// DKV store (to preserve caching).
	// Only comutes the rollups if needed (i.e. are null or do not have histo and
	// histo is required)
	// If rs computation is already in progress, it will wait for it to finish.
	// Throws IAE if the Vec is being modified (or removed) while this task is in
	// progress.
	static final class ComputeRollupsTask extends DTask<ComputeRollupsTask> {
		...
		public ComputeRollupsTask(Vec v, boolean computeHisto) {
			super((byte) (Thread.currentThread() instanceof H2O.FJWThr ? currThrPriority() + 1
					: H2O.MIN_HI_PRIORITY - 3));
			_vecKey = v._key;
				...
				if (rs != null) {
					...
					if (oldv == v) { // got the lock, compute the rollups
						try {
							...
							// computed the stats, now compute histo if needed and install the response and
							// quit
		...
		final void computeHisto(final RollupStats rs, Vec vec, final Value nnn) {
			...
			for (int i = 0; i < Vec.PERCENTILES.length; i++) {
				...
				assert P >= 0 && P <= 1 && P >= lastP; // rely on increasing percentiles here. If P has dup then strange
														// but accept, hence >= not >
				lastP = P;
				...
				while (hsum < pint)
					hsum += rs._bins[j++];
				// j overshot by 1 bin; we added _bins[j-1] and this goes from too low to either
				// exactly right or too big
				// pint now falls in bin j-1 (the ++ happened even when hsum==pint), so grab
				// that bin value now
				...
				} // otherwise either h==0 and we know which bin, or fraction is between two
					// positions that fall in the same bin
					// this guarantees we are within one bin of the exact answer; i.e. within
					// (max-min)/MAX_SIZE
			}
			installResponse(nnn, rs);
		}
