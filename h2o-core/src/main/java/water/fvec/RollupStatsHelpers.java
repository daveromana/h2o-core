...
public class RollupStatsHelpers {
	...
	/**
	 * MASTER TEMPLATE - All methods below are COPY & PASTED from this template, and
	 * some optimizations are performed based on the chunk types
	 *
	 * @param c
	 * @param start
	 * @param checksum
	...
	public long numericChunkRollup(Chunk c, long start, long checksum) {
		...
		long rows = 0; // count of non-NA rows, might be >0 for sparse chunks (all 0s are already
						// processed outside)
		...
		// loop over all values for dense chunks, but only the non-zeros for sparse
		// chunks
