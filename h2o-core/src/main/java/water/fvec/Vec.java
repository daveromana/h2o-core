 ...
 * Java-like type (mostly determines rounding on store and display), and
 * functions to directly load elements without further indirections. The data is
 * compressed, or backed by disk or both.
 *
 * <p>
 * A Vec is a collection of {@link Chunk}s, each of which holds between 1,000
 ...
 * distributed) array - however this is not the fastest way to access the data.
 * Direct access from Chunks is faster, avoiding several layers of indirection.
 * In particular accessing a random row from the Vec will force the containing
 * Chunk data to be cached locally (and network traffic to bring it local);
 * accessing all rows from a single machine will force all the Big Data to be
 * pulled local typically resulting in swapping and very poor performance. The
 * main API is provided for ease of small-data manipulations and is fairly slow
 * for writing; when touching ALL the data you are much better off using e.g.
 * {@link MRTask}.
 *
 * <p>
 * The main API is {@link #at}, {@link #set}, and {@link #isNA}:<br>
 ...
 * <td>{@link #at16l}</td>
 * <td>throws</td>
 * <td>Low half of 128-bit UUID</td>
 * <tr>
 * <td>{@code long}</td>
 * <td>{@link #at16h}</td>
 * <td>throws</td>
 * <td>High half of 128-bit UUID</td>
 * <tr>
 * <td>{@link BufferedString}</td>
 * <td>{@link #atStr}</td>
 * <td>{@code null}</td>
 * <td>Updates BufferedString in-place and returns it for flow-coding</td>
 * <tr>
 * <td>{@code boolean}</td>
 * <td>{@link #isNA}</td>
 * <td>{@code true}</td>
 ...
 * <tr>
 * <td></td>
 * <td>{@link #set(long,double)}</td>
 * <td>{@code NaN}</td>
 * <td></td>
 * <tr>
 * <td></td>
 * <td>{@link #set(long,float)}</td>
 * <td>{@code NaN}</td>
 * <td>Limited precision takes less memory</td>
 * <tr>
 * <td></td>
 * <td>{@link #set(long,long)}</td>
 * <td>Cannot set</td>
 * <td></td>
 * <tr>
 * <td></td>
 * <td>{@link #set(long,String)}</td>
 * <td>{@code null}</td>
 * <td>Convenience wrapper for String</td>
 * <tr>
 * <td></td<td>{@link #set(long,UUID)}</td>
 * <td>{@code null}</td>
 * </tr>
 * <tr>
 * <td></td>
 * <td>{@link #setNA(long)}</td>
 * <td></td>
 * <td></td>
 * </table>
 *
 ...
 *
 * <p>
 * Vecs have a loosely enforced <em>type</em>: one of numeric, {@link UUID} or
 * {@link String}. Numeric types are further broken down into integral
 * ({@code long}) and real ({@code double}) types. The {@code categorical} type
 * is an integral type, with a String mapping side-array. Most of the math
 * algorithms will treat categoricals as small dense integers, and most
 * categorical printouts will use the String mapping. Time is another special
 * integral type: it is represented as milliseconds since the unix epoch, and is
 * mostly treated as an integral type when doing math but it has special
 * time-based printout formatting. All types support the notion of a missing
 * element; for real types this is always NaN. It is an error to attempt to
 * fetch a missing integral type, and {@link #isNA} must be called first.
 * Integral types are losslessly compressed. Real types may lose 1 or 2 ULPS due
 * to compression.
 *
 * <p>
 * Reading elements as doubles, or checking for an element missing is always
 * safe. Reading a missing integral type throws an exception, since there is no
 * NaN equivalent in the integer domain. <em>Writing</em> to elements may throw
 * if the backing data is read-only (file backed), and otherwise is fully
 * supported.
 *
 * <p>
 * Note this dangerous scenario: loading a missing value as a double, and
 * setting it as a long:
 * 
 * <pre>
 * set(row, (long) at(row)); // Danger!
 * </pre>
 * 
 * The cast from a Double.NaN to a long produces a zero! This code will silently
 * replace a missing value with a zero.
 *
 * <p>
 * Vecs have a lazily computed {@link RollupStats} object and Key. The
 * RollupStats give fast access to the common metrics: {@link #min},
 * {@link #max}, {@link #mean}, {@link #sigma}, the count of missing elements
 * ({@link #naCnt}) and non-zeros ({@link #nzCnt}), amongst other stats. They
 * are cleared if the Vec is modified and lazily recomputed after the modified
 * Vec is closed. Clearing the RollupStats cache is fairly expensive for
 ...
 *
 * <p>
 * Example: Impute (replace) missing values with the mean. Note that the use of
 * {@code vec.mean()} in the constructor uses (and computes) the general
 * RollupStats before the MRTask starts. Setting a value in the Chunk clears the
 * RollupStats (since setting any value but the mean will change the mean); they
 * will be recomputed at the next use after the MRTask.
 * 
 * <pre>
 ...
 *
 * <p>
 * Vecs have a {@link Vec.VectorGroup}. Vecs in the same VectorGroup have the
 * same Chunk and row alignment - that is, Chunks with the same index are homed
 * to the same Node and have the same count of rows-per-Chunk. {@link Frame}s
 * are only composed of Vecs of the same VectorGroup (or very small Vecs)
 * guaranteeing that all elements of each row are homed to the same Node and set
 * of Chunks - such that a simple {@code for} loop over a set of Chunks all
 * operates locally. See the example in the {@link Chunk} class.
 *
 * <p>
 * It is common and cheap to make new Vecs in the same VectorGroup as an
 * existing Vec and initialized to e.g. zero. Such Vecs are often used as temps,
 * and usually immediately set to interest values in a later MRTask pass.
 * 
 * <p>
 ...
 *
 * <p>
 * Vec {@link Key}s have a special layout (enforced by the various Vec
 * constructors) so there is a direct Key mapping from a Vec to a numbered Chunk
 * and vice-versa. This mapping is crucially used in all sorts of places,
 * basically representing a global naming scheme across a Vec and the Chunks
 * that make it up. The basic layout created by {@link #newKey}:
 * 
 * <pre>
...
public class Vec extends Keyed<Vec> {
	...
	/**
	 * Element-start per chunk, i.e. the row layout. Defined in the VectorGroup.
	 * This field is dead/ignored in subclasses that are guaranteed to have
	 * fixed-sized chunks such as file-backed Vecs.
	 */
	...
	/**
	 * Returns the categorical toString mapping array, or null if not an categorical
	 * column. Not a defensive clone (to expensive to clone; coding error to change
	 * the contents).
	 * 
	 * @return the categorical / factor / categorical mapping array, or null if not
	 *         a categorical column
	 */
	public String[] domain() {
		return _domain;
	} // made no longer final so that InteractionWrappedVec which are _type==T_NUM but
		// have a categorical interaction
	...
	public final String factor(long i) {
		return _domain[(int) i];
	}

	/**
	 * Set the categorical/factor names. No range-checking on the actual underlying
	 * numeric domain; user is responsible for maintaining a mapping which is
	 * coherent with the Vec contents.
	 */
	...
	public final int cardinality() {
		return isCategorical() ? _domain.length : -1;
	}
	...
	public static final String[] TYPE_STR = new String[] { "BAD", "UUID", "String", "Numeric", "Enum", "Time", "Time",
			"Time" };
	...
	public final boolean isCategorical() {
		assert (_type == T_CAT && _domain != null) || (_type != T_CAT && _domain == null)
				|| (_type == T_NUM && this instanceof InteractionWrappedVec && _domain != null);
		return _type == T_CAT;
	}
	 ...
	 * 
	 * @return true if this is a numeric column, excluding categorical and time
	 *         types
	 */
	...
	Vec(Key<Vec> key, int rowLayout, String[] domain) {
		this(key, rowLayout, domain, (domain == null ? T_NUM : T_CAT));
	}
	...
	/**
	 * Number of elements in the vector; returned as a {@code long} instead of an
	 * {@code int} because Vecs support more than 2^32 elements. Overridden by
	 * subclasses that compute length in an alternative way, such as file-backed
	 * Vecs.
	 * 
	...
	/**
	 * Number of chunks, returned as an {@code int} - Chunk count is limited by the
	 * max size of a Java {@code long[]}. Overridden by subclasses that compute
	 * chunks in an alternative way, such as file-backed Vecs.
	 * 
	...
	/**
	 * Convert a chunk-index into a starting row #. For constant-sized chunks this
	 * is a little shift-and-add math. For variable-sized chunks this is a table
	 * lookup.
	 */
	...
	private int chunkLen(int cidx) {
		espc();
		return (int) (_espc[cidx + 1] - _espc[cidx]);
	}

	/** Check that row-layouts are compatible. */
	public boolean isCompatibleWith(Vec v) {
		// Vecs are compatible iff they have same group and same espc (i.e. same length
		// and same chunk-distribution)
	...
	private void setMeta(byte type, String[] domain) {
		if (domain == null && type == T_CAT) {
			type = T_NUM;
		} // Until you have some strings, you are just a numeric column
		_domain = domain;
		_type = type;
	}

	public void copyMeta(Vec src, Futures fs) {
		setMeta(src._type, src._domain);
		DKV.put(this, fs);
	}
	...
	/**
	 * Make a new constant vector with the given row count, and redistribute the
	 * data evenly around the cluster.
	 * 
	 * @param x
	 *            The value with which to fill the Vec.
	 * @param len
	 *            Number of rows.
	 ...
	 *            The value with which to fill the Vec.
	 * @param len
	 *            The number of rows in the produced Vec.
	 * @param type
	 *            Type of the Vec to construct.
	 */
	...
	/**
	 * Make a new constant vector with minimal number of chunks. Used for importing
	 * SQL tables.
	 * 
	 * @return New constant vector with the given row count.
	 */
	...
	/**
	 * Make a new vector with the same size and data layout as the current one, and
	 * initialized to zero.
	 * 
	 ...
	 */
	public Vec makeZero() {
		return makeCon(0, null, group(), _rowLayout);
	}
	 ...
	 * 
	 * @return A new vector with the same size and data layout as the current one,
	 *         and initialized to zero, with the given categorical domain.
	 */
	public Vec makeZero(String[] domain) {
		return makeCon(0, domain, group(), _rowLayout);
	}
		...
		new MRTask() { // Body of all zero chunks
			...
			protected void setupLocal() {
				for (int i = 0; i < nchunks; i++) {
					Key k = v0.chunkKey(i);
					if (k.home()) {
						DKV.put(k, new C0LChunk(l, v0.chunkLen(i)), _fs);
					}
				}
			}
		}.doAllNodes();
	...
	/**
	 * Make a new vector with the same size and data layout as the current one, and
	 * initialized to the given constant value.
	 * 
	 * @return A new vector with the same size and data layout as the current one,
	 *         and initialized to the given constant value.
	 */
	public Vec makeCon(final double d) {
		return makeCon(d, group(), _rowLayout, T_NUM);
	}

	public Vec makeCon(final double d, byte type) {
		return makeCon(d, group(), _rowLayout, type);
	}
	...
	public Vec makeCon(final String s) {
		return makeCon(s, group(), _rowLayout, T_STR);
	}
	...
	public Vec[] makeZeros(int n) {
		return makeZeros(n, null, null);
	}

	/**
	 * Make a temporary work vec of double [] . Volatile vecs can only be used
	 * locally (chunks do not serialize) and are assumed to change frequently(MRTask
	 * call preWiting() by default). Chunks stores as C8DVolatileChunk - expose data
	 * directly as double [].
	 *
	 * @param n
	 *            number of columns
	...
	/**
	 * Make a temporary work vec of int [] . Volatile vecs can only be used locally
	 * (chunks do not serialize) and are assumed to change frequently(MRTask call
	 * preWiting() by default). Chunks stores as C4VolatileChunk - expose data
	 * directly as int [].
	 *
	 * @param cons
	 *            integer array with constant used to fill each column.
	 * @return
	 */
	...
	public Vec[] makeZeros(int n, String[][] domain, byte[] types) {
		return makeCons(n, 0, domain, types);
	}
	...
	public Vec[] makeCons(int n, final long l, String[][] domains, byte[] types) {
		...
		for (int i = 0; i < vs.length; ++i)
			vs[i] = new Vec(keys[i], _rowLayout, domains == null ? null : domains[i], types == null ? T_NUM : types[i]);
		new MRTask() {
			@Override
			protected void setupLocal() {
				for (Vec v1 : vs) {
	...
	/**
	 * <b>isInt</b> is a property of numeric Vecs and not a type; this property can
	 * be changed by assigning non-integer values into the Vec (or restored by
	 * overwriting non-integer values with integers). This is a strong type for
	 * {@link #isCategorical} and {@link #isTime} Vecs.
	 * 
	...
	/**
	 * Default percentiles for approximate (single-pass) quantile computation
	 * (histogram-based).
	 */
	public static final double PERCENTILES[] = { 0.001, 0.01, 0.1, 0.2, 0.25, 0.3, 1.0 / 3.0, 0.4, 0.5, 0.6, 2.0 / 3.0,
			0.7, 0.75, 0.8, 0.9, 0.99, 0.999 };

	/**
	 * A simple and cheap histogram of the Vec, useful for getting a broad overview
	 * of the data. Each bin is row-counts for the bin's range. The bin's range is
	...
	/**
	 * The {@code base} for a simple and cheap histogram of the Vec, useful for
	 * getting a broad overview of the data. This returns the base of
	 * {@code bins()[0]}.
	 * 
	...
	/**
	 * The {@code stride} for a a simple and cheap histogram of the Vec, useful for
	 * getting a broad overview of the data. This returns the stride between any two
	 * bins.
	 * 
	 ...
	 * overview of the data. The specific percentiles are take from
	 * {@link #PERCENTILES}.
	 * 
	...
	public void startRollupStats(Futures fs) {
		startRollupStats(fs, false);
	}

	/**
	 * Check if we have local cached copy of basic Vec stats (including histogram if
	 * requested) and if not start task to compute and fetch them; useful when
	 * launching a bunch of them in parallel to avoid single threaded execution
	 * later (e.g. for-loop accessing min/max of every vec in a frame).
	 *
	 * Does *not* guarantee rollup stats will be cached locally when done since
	 * there may be racy changes to vec which will flush local caches.
	 *
	 * @param fs
	 *            Futures allow to wait for this task to finish.
	 * @param doHisto
	 *            Also compute histogram, requires second pass over data amd is not
	 *            computed by default.
	 *
	 */
	...
	/**
	 * A high-quality 64-bit checksum of the Vec's content, useful for establishing
	 * dataset identity.
	 * 
	 ...
	 * ({@link #min}, {@link #max}, {@link #mean}, etc) since such values are not
	 * meaningful while the Vec is being actively modified. Can be called
	 * repeatedly. Per-chunk row-counts will not be changing, just row contents.
	 */
	...
	/**
	 * Marks the Vec as mutating. Vec needs to be marked as mutating whenever it is
	 * modified ({@link #preWriting()}) or removed ({@link #remove_impl(Futures)}).
	 */
	 ...
	 * shift-and-add math. For variable-sized chunks this is a binary search, with a
	 * sane API (JDK has an insane API). Overridden by subclasses that compute
	 * chunks in an alternative way, such as file-backed Vecs.
	 */
	...
	/**
	 * Get a Chunk Key from a chunk-index and a Vec Key, without needing the actual
	 * Vec object. Basically the index-to-key map.
	 * 
	 ...
	 * {@code DKV.get()}. Warning: this pulls the data locally; using this call on
	 * every Chunk index on the same node will probably trigger an OOM!
	 */
	 ...
	 * optimized stencil calculations that want to "roll off the end" of a Chunk,
	 * but in a highly optimized way.
	 */
	...
	/**
	 * Internally used to help build Vec and Chunk Keys; public to help PersistNFS
	 * build file mappings. Not intended as a public field.
	 */
	...
	/**
	 * The Chunk for a chunk#. Warning: this pulls the data locally; using this call
	...
	public Chunk chunkForChunkIdx(int cidx) {
		...
		if (!(cstart == -1 || v == null || tcidx == -1)) {
			throw new RuntimeException("Was not filled in (everybody racily writes the same start value:  cstart = "
					+ cstart + " v == null? " + (v == null) + " cidx = " + tcidx + ", chunk = "
					+ c.getClass().getName());
		}
		assert cstart == -1 || v == null || tcidx == -1 : " cstart = " + cstart + " v == null? " + (v == null)
				+ " cidx = " + tcidx + ", chunk = " + c.getClass().getName(); // Was not filled in (everybody racily
																				// writes the same start value)
	...
	/**
	 * The Chunk for a row#. Warning: this pulls the data locally; using this call
	 * on every Chunk index on the same node will probably trigger an OOM!
	 * 
	...
	/**
	 * Fetch element the slow way, as a long. Floating point values are silently
	 * rounded to an integer. Throws if the value is missing.
	 * 
	 * @return {@code i}th element as a long, or throw if missing
	 */
	public final long at8(long i) {
		return chunkForRow(i).at8_abs(i);
	}
	...
	public final boolean isNA(long row) {
		return chunkForRow(row).isNA_abs(row);
	}

	/**
	 * Fetch element the slow way, as the low half of a UUID. Throws if the value is
	 * missing or not a UUID.
	 * 
	 * @return {@code i}th element as a UUID low half, or throw if missing
	 */
	public final long at16l(long i) {
		return chunkForRow(i).at16l_abs(i);
	}

	/**
	 * Fetch element the slow way, as the high half of a UUID. Throws if the value
	 * is missing or not a UUID.
	 * 
	 * @return {@code i}th element as a UUID high half, or throw if missing
	 */
	public final long at16h(long i) {
		return chunkForRow(i).at16h_abs(i);
	}

	/**
	 * Fetch element the slow way, as a {@link BufferedString} or null if missing.
	 * Throws if the value is not a String. BufferedStrings are String-like objects
	 * than can be reused in-place, which is much more efficient than constructing
	 * Strings.
	 * 
	...
	public final BufferedString atStr(BufferedString bStr, long i) {
		if (isCategorical()) { // for categorical vecs, return the factor level
			...
			return chunkForRow(i).atStr_abs(bStr, i);
	}
	...
	/**
	 * A more efficient way to read randomly to a Vec - still single-threaded, but
	 * much faster than Vec.at(i). Limited to single-threaded single-machine reads.
	 *
	 * Usage: Vec.Reader vr = vec.new Reader(); x = vr.at(0); y = vr.at(1); z =
	 * vr.at(2);
	 */
	public final class Reader {
		...
		private Chunk chk(long i) {
			Chunk c = _cache;
			return (c != null && c.chk2() == null && c._start <= i && i < c._start + c._len) ? c
					: (_cache = chunkForRow(i));
		}

		public final long at8(long i) {
			return chk(i).at8_abs(i);
		}

		public final double at(long i) {
			return chk(i).at_abs(i);
		}

		public final boolean isNA(long i) {
			return chk(i).isNA_abs(i);
		}

		public final BufferedString atStr(BufferedString sb, long i) {
			return chk(i).atStr_abs(sb, i);
		}
	...
	/**
	 * Write element the slow way, as a long. There is no way to write a missing
	 * value with this call. Under rare circumstances this can throw: if the long
	 * does not fit in a double (value is larger magnitude than 2^52), AND float
	 * values are stored in Vec. In this case, there is no common compatible data
	 * representation.
	 */
	...
	/**
	 * Write element the slow way, as a double. Double.NaN will be treated as a set
	...
	/**
	 * Write element the slow way, as a float. Float.NaN will be treated as a set of
	 * a missing element.
	 */
	 ...
	 *
	 * Usage: try( Vec.Writer vw = vec.open() ) { vw.set(0, 3.32); vw.set(1, 4.32);
	 * vw.set(2, 5.32); }
	 */
	public final class Writer implements java.io.Closeable {
		...
		public Futures close(Futures fs) {
			return postWrite(closeLocal(fs));
		}
	...
	/**
	 * Close all chunks that are local (not just the ones that are homed) This
	 * should only be called from a Writer object
	 */
	...
	public String toString() {
		...
		String s = "[" + length() + (rs == null ? ", {"
				: "," + rs._mins[0] + "/" + rs._mean + "/" + rs._maxs[0] + ", " + PrettyPrint.bytes(rs._size) + ", {");
		int nc = nChunks();
		for (int i = 0; i < nc; i++) {
			s += chunkKey(i).home_node() + ":" + chunk2StartElem(i) + ":";
			// CNC: Bad plan to load remote data during a toString... messes up debug
			// printing
	 ...
	 * 
	 * @param off
	 *            index of first element to include
	 * @param len
	...
	/**
	 * True if two Vecs are equal. Checks for equal-Keys only (so it is fast) and
	 * not equal-contents.
	 * 
	...
	/**
	 * Remove associated Keys when this guy removes. For Vecs, remove all associated
	 * Chunks.
	 * 
	...
	static void bulk_remove(final Key[] keys, final int ncs) {
		// Need to mark the Vec as mutating to make sure that no running computations of
		// RollupStats will
		// re-insert the rollups into DKV after they are deleted in bulk_remove(Key,
		// int).
		Futures fs = new Futures();
		for (Key key : keys)
			fs.add(new SetMutating().fork(chunkKey(key, -2)));
		fs.blockForPending();
		// Bulk dumb local remove - no JMM, no ordering, no safety.
		// Remove Vecs everywhere first - important! (this should make simultaneously
		// running Rollups to fail)
		new MRTask() {
			@Override
	...
	/**
	 * Always makes a copy of the given vector which shares the same group as this
	 * Vec. This can be expensive operation since it can force copy of data among
	 * nodes.
	 *
	 * @param vec
	 *            vector which is intended to be copied
	 * @return a copy of vec which shared the same {@link VectorGroup} with this
	 *         vector
	 */
	 ...
	 * CategoricalWrappedVec has 'this' as it's masterVec, but returns results in
	 * the 'to' newDomain (or just past it, if 'this' has elements not appearing in
	 * the 'to' newDomain).
	 */
	public Vec adaptTo(String[] domain) {
		if (!isBad() && isNumeric() && !ArrayUtils.isInt(domain)) { // try to adapt double domain
			// treat double domain here - return full vector copy instead of mapping double
			// to ints on the fly in a WrappedVec
			...
			if (j == double_domain.length) { // only atempt to adapt if we have fully double domain, (to preserve
												// current behavior for ints, could relax this later)
				if (j < double_domain.length) {
					double_domain = Arrays.copyOf(double_domain, j);
				}
				double[] new_double_domain = new VecUtils.CollectDoubleDomain(double_domain, 100000).doAll(this)
						.domain();
	 ...
	 * Each vector is member of exactly one group. Default group of one vector is
	 * created for each vector. Group of each vector can be retrieved by calling
	 * group() method;
	 * 
	 * The expected mode of operation is that user wants to add new vectors matching
	 * the source. E.g. parse creates several vectors (one for each column) which
	 * are all colocated and are colocated with the original bytevector.
	 * 
	 * To do this, user should first ask for the set of keys for the new vectors by
	 * calling addVecs method on the target group.
	 * 
	 ...
	 * group object carries is its own key and the number of vectors it contains
	 * (deleted vectors still count).
	 * 
	 * Because vectors (and chunks) share the same key-pattern with the group,
	 * default group with only one vector does not have to be actually created, it
	 * is implicit.
	 * 
	 * @author tomasnykodym
	 */
	...
	public static class ESPC extends Keyed<ESPC> {
		...
		public static long[] espc(Vec v) {
			...
			throw H2O.fail("Vec " + v._key + " asked for layout " + r + ", but only " + remote._espcs.length
					+ " layouts defined");
		}
		...
		/**
		 * Get the shared ESPC index for this layout. Will return an old layout if one
		 * matches, otherwise will atomically update the ESPC to set a new layout. The
		 * expectation is that new layouts are rare: once per parse, and perhaps from
		 * filtering MRTasks, or data-shuffling.
		 */
