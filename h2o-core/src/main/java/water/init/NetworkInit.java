...
/**
 * Data structure for holding network info specified by the user on the command
 * line.
 */
public class NetworkInit {
	...
	public static void initializeNetworkSockets() {
		...
		while (true) {
			...
			try {
				...
				// if there is a connection in the timeout state involving the socket address or
				// port.
				// Enabling SO_REUSEADDR prior to binding the socket using bind(SocketAddress)
				// allows the socket to be bound even though a previous connection is in a
				// timeout state.
				...
				_tcpSocket = null;
				if (H2O.ARGS.port != 0) {
					H2O.die("On " + H2O.SELF_ADDRESS + " some of the required ports " + H2O.ARGS.port + ", "
							+ (H2O.ARGS.port + 1) + " are not available, change -port PORT and try again.");
				}
			}
		...
		if (ec != null) {
			...
			// ec.notifyAboutH2oCommunicationChannel(H2O.SELF_ADDRESS, H2O.H2O_PORT)
			// As of right now, the function notifies about the H2O.API_PORT, and then the
			// listener adds +1
			// to that in order to determine the H2O_PORT (which what it really cares
			// about). Such
			// assumption is dangerous: we should be free of using independent API_PORT and
			// H2O_PORT,
		...
		// All the machines has to agree on the same multicast address (i.e., multicast
		// group)
		// Hence use the cloud name to generate multicast address
		// Note: if necessary we should permit configuration of multicast address
		// manually
		// Note:
		...
		try {
			H2O.CLOUD_MULTICAST_GROUP = isIPv6
					? NetworkUtils.getIPv6MulticastGroup(hash, NetworkUtils.getIPv6Scope(H2O.SELF_ADDRESS))
	 ...
	 * 
	 * @param ip
	 *            textual representation of IP (host)
		...
		if (!H2O.isFlatfileEnabled()) {
			...
			synchronized (H2O.class) { // Sync'd so single-thread socket create/destroy
				...
				try {
					...
					H2O.CLOUD_MULTICAST_SOCKET.send(
							new DatagramPacket(buf, buf.length, H2O.CLOUD_MULTICAST_GROUP, H2O.CLOUD_MULTICAST_PORT));
	 ...
	 *
	 * name/ip_address:port - name is unused and optional - port is optional -
	 * leading '#' indicates a comment
	 *
	 * For example:
	 *
	 * 10.10.65.105:54322 # disabled for testing # 10.10.65.106 /10.10.65.107 # run
	 * two nodes on 108 10.10.65.108:54322 10.10.65.108:54325
	 */
