 ...
 * US2 box, 32-way Sun Niagara box, 8-way Intel box and a 4-way Power box.
 *
 ...
 *
 * <p>
 * Very full tables, or tables with high re-probe rates may trigger an internal
...
public class NonBlockingHashMap<TypeK, TypeV> extends AbstractMap<TypeK, TypeV>
	...
	private static final CHM chm(Object[] chm_kvs) {
		return (CHM) chm_kvs[0];
	}

	private static final int[] hashes(Object[] hes_kvs) {
		return (int[]) hes_kvs[1];
	}

	// Number of K,V pairs in the table
	private static final int len(Object[] len_kvs) {
		return (len_kvs.length - 2) >> 1;
	}
	...
	private static final Object key(Object[] kvs_ch, int idx) {
		return kvs_ch[(idx << 1) + 2];
	}

	private static final Object val(Object[] kvs_ch_v, int idx_ch) {
		return kvs_ch_v[(idx_ch << 1) + 3];
	}
	 ...
	 * K/V pairs or roughly 84 bytes on a standard 32-bit JVM).
	 */
	 ...
	 * elements, thus avoiding internal resizing operations to reach an appropriate
	 * size. Large numbers here when used with a small count of elements will
	 * sacrifice space for a small amount of time gained. The initial size will be
	 * rounded up internally to the next larger power of 2.
	 */
	 ...
	 * 
	 * @return <tt>true</tt> if the key is in the table using the <tt>equals</tt>
	 *         method
	 * @throws NullPointerException
	 *             if the specified key is null
	 */
	@Override
	public boolean containsKey(Object contenedkey) {
		return get(contenedkey) != null;
	}
	 ...
	 */
	public boolean contains(Object cont_val) {
		return containsValue(cont_val);
	}
	 ...
	 * value can be null.
	 * <p>
	 * The value can be retrieved by calling {@link #get} with a key that is equal
	 * to the original key.
	 * 
	 * @param key
	 ...
	 */
	@Override
	public TypeV put(TypeK putkey, TypeV val) {
		return putIfMatch(putkey, val, NO_MATCH_OLD);
	}

	/**
	 * Atomically, do a {@link #put} if-and-only-if the key is not mapped. Useful to
	 * ensure that only a single mapping for the key exists, even if many threads
	 * are trying to create the mapping in parallel.
	 * 
	 ...
	 *             if the specified key or value is null
	 */
	public TypeV putIfAbsent(TypeK abs_key, TypeV val) {
		return putIfMatch(abs_key, val, TOMBSTONE);
	}
	 ...
	 */
	@Override
	public TypeV remove(Object rmkey) {
		return putIfMatch(rmkey, TOMBSTONE, NO_MATCH_OLD);
	}

	/**
	 * Atomically do a {@link #remove(Object)} if-and-only-if the key is mapped to a
	 * value which is <code>equals</code> to the given value.
	 * 
	 * @throws NullPointerException
	 *             if the specified key or value is null
	 */
	public boolean remove(Object rem_key, Object val) {
		return putIfMatch(rem_key, TOMBSTONE, val) == val;
	}

	/**
	 * Atomically do a <code>put(key,val)</code> if-and-only-if the key is mapped to
	 * some value already.
	 * 
	 * @throws NullPointerException
	 *             if the specified key or value is null
	 */
	public TypeV replace(TypeK ke_y, TypeV val) {
		return putIfMatch(ke_y, val, MATCH_ANY);
	}
	...
	private static boolean keyeq(Object K, Object k_e_y, int[] hashes, int hash, int fullhash) {
		return K == k_e_y || // Either keys match exactly OR
	...
	private static final Object get_impl(final NonBlockingHashMap topmap, final Object[] kvs, final Object key) {
		...
		while (true) {
			...
			if (++reprobe_cnt >= reprobe_limit(len) || // too many probes
				...
				if (newkvs == READONLY) {
					return null; // Missed in a locked table
				}
				return newkvs == null ? null : get_impl(topmap, topmap.help_copy(newkvs), key); // Retry in the new
																								// table
			}
	...
	// --- getk -----------------------------------------------------------------
	/**
	 * Returns the Key to which the specified key is mapped, or {@code null} if this
	...
	private static final Object getk_impl(final NonBlockingHashMap top_map, final Object[] k_vs, final Object k_ey) {
		...
		while (true) {
			...
			if (++reprobe_cnt >= reprobe_limit(len) || // too many probes
				...
				return newkvs == null ? null : getk_impl(top_map, top_map.help_copy(newkvs), k_ey); // Retry in the new
																									// table
			}
	...
	private static final Object putIfMatch(final NonBlockingHashMap topmap, final Object[] kvs, final Object key,
		...
		if (expVal != NO_MATCH_OLD && // Do we care about expected-Value at all?
				...
				(expVal != MATCH_ANY || V == TOMBSTONE || V == null) && !(V == null && expVal == TOMBSTONE) && // Match
																												// on
																												// null/TOMBSTONE
																												// combo
	...
	private static final class CHM<TypeK, TypeV> {
		...
		private final Object[] resize(NonBlockingHashMap topmap, Object[] kvs) {
			...
			if (sz >= (oldlen >> 2)) { // If we are >25% full of keys then...
				...
				if (4L * sz >= ((oldlen >> 20) != 0 ? 3L : 2L) * oldlen) {
					newsz = oldlen << 2; // Double double size, so new table will be between %12.5 (18.75%) and 25%
											// (25%)
				}
			}
		...
		static private final AtomicLongFieldUpdater<CHM> _copyIdxUpdater = AtomicLongFieldUpdater.newUpdater(CHM.class,
				"_copyIdx");
		...
		static private final AtomicLongFieldUpdater<CHM> _copyDoneUpdater = AtomicLongFieldUpdater.newUpdater(CHM.class,
				"_copyDone");
		...
		private final void help_copy_impl(NonBlockingHashMap topmap, Object[] oldkvs, boolean copy_all) {
			...
			while (_copyDone < oldlen) { // Still needing to copy?
				...
				for (int i = 0; i < MIN_COPY_WORK; i++)
					if (copy_slot(topmap, (copyidx + i) & (oldlen - 1), oldkvs, newkvs)) {// Made an oldtable slot go
																							// dead?
						workdone++; // Yes!
					}
				...
				// for( int i=0; i<MIN_COPY_WORK; i++ )
				// if( copy_slot(topmap,(copyidx+i)&(oldlen-1),oldkvs,newkvs) ) // Made an
				// oldtable slot go dead?
		...
		// read it yet.
		private final Object[] copy_slot_and_check(NonBlockingHashMap topmap, Object[] oldkvs, int idx,
				Object should_help) {
			assert chm(oldkvs) == this;
			Object[] newkvs = _newkvs; // VOLATILE READ
		...
		private final void copy_check_and_promote(NonBlockingHashMap topmap, Object[] oldkvs, int workdone) {
			...
			if (workdone > 0) {
				...
				// System.out.print("
				// "+(copyDone+workdone)*100/oldlen+"%"+"_"+(_copyIdx*100/oldlen)+"%");
			}
		...
		return new AbstractSet<TypeK>() {
			...
			public <T> T[] toArray(T[] a) {
				...
				if (j <= a.length) { // Fit in the original array?
					...
					if (j < a.length) {
						r[j++] = null; // One final null not in the spec but in the default impl
					}
	 ...
	 *
	 * <p>
	 * <strong>Warning:</strong> the iterator associated with this Set requires the
	 * creation of {@link java.util.Map.Entry} objects with each iteration. The
	 * {@link NonBlockingHashMap} does not normally create or using
	 * {@link java.util.Map.Entry} objects so they will be created soley to support
	 * this iteration. Iterating using {@link #keySet} or {@link #values} will be
	 * more efficient.
	 */
	@Override
	 ...
	 * remove and replace, but allows putIfAbsent and putIfMatch. Existing mutator
	 * calls in other threads racing with this thread and will either throw
	 * IllegalStateException or their update will be visible to this thread. This
	 * implies that a simple flag cannot make the Set immutable, because a
	 * late-arriving update in another thread might see immutable flag not set yet,
	 * then mutate the Set after the {@link #readOnly} call returns. This call can
	 * be called concurrently (and indeed until the operation completes, all calls
	 * on the Set from any thread either complete normally or end up calling
	 * {@link #readOnly} internally).
	 *
	 * <p>
	 * This call is useful in debugging multi-threaded programs where the Set is
	 * constructed in parallel, but construction completes after some time; and
	 * after construction the Set is only read. Making the Set read-only will cause
	 * updates arriving after construction is supposedly complete to throw an
