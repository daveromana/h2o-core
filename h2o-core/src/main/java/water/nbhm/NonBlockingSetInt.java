 ...
 *
 * <p>
 * <em>General note of caution:</em> The Set API allows the use of
 * {@link Integer} with silent autoboxing - which can be very expensive if many
 * calls are being made. Since autoboxing is silent you may not be aware that
 * this is going on. The built-in API takes lower-case {@code ints} and is much
 * more efficient.
 *
 * <p>
 * Space: space is used in proportion to the largest element, as opposed to the
 * number of elements (as is the case with hash-table based Set
...
public class NonBlockingSetInt extends AbstractSet<Integer> implements Serializable {
	...
	/**
	 * Add {@code i} to the set. Uppercase {@link Integer} version of add, requires
	 * auto-unboxing. When possible use the {@code int} version of {@link #add(int)}
	 * for efficiency.
	 * 
	 ...
	 * version of contains, requires a type-check and auto-unboxing. When possible
	 * use the {@code int} version of {@link #contains(int)} for efficiency.
	 * 
	...
	/**
	 * Remove {@code o} from the set. This is the uppercase {@link Integer} version
	 * of remove, requires a type-check and auto-unboxing. When possible use the
	 * {@code int} version of {@link #remove(int)} for efficiency.
	 * 
	...
	/**
	 * Add {@code i} to the set. This is the lower-case '{@code int}' version of
	 * {@link #add} - no autoboxing. Negative values throw IllegalArgumentException.
	 * 
	 * @throws IllegalArgumentException
	 *             if i is negative.
	 * @return <tt>true</tt> if i was added to the set.
	...
	/**
	 * Test if {@code i} is in the set. This is the lower-case '{@code int}' version
	 * of {@link #contains} - no autoboxing.
	 * 
	...
	/**
	 * Current count of elements in the set. Due to concurrent racing updates, the
	 * size is only ever approximate. Updates due to the calling thread are
	...
	/**
	 * Standard Java {@link Iterator}. Not very efficient because it auto-boxes the
	 * returned values.
	 */
	...
	private static final class NBSI {
		...
		private NBSI install_larger_new_bits(final int i) {
			if (_new == null) {
				...
				// Note: must grow by exact powers-of-2 or the by-64-bit trick doesn't work
				// right
		...
		private NBSI help_copy() {
			...
			if (top_nbsi._copyDone.get() == top_nbsi._sum_bits_length) {
				...
				if (_non_blocking_set_int.CAS_nbsi(top_nbsi, top_nbsi._new)) {
					// System.out.println("Promote at top level to size
					// "+(_non_blocking_set_int._nbsi._bits.length<<6));
				}
		...
		private void print(int d) {
			...
			if (_copyIdx.get() != 0 || _copyDone.get() != 0) {
				print(d, "_copyIdx=" + _copyIdx.get() + " _copyDone=" + _copyDone.get() + " _words_to_cpy="
						+ _sum_bits_length);
			}
			if (_new != null) {
				print(d, "__has_new - ");
