...
class CsvParser extends Parser {
	...
	@Override
	public ParseWriter parseChunk(int cidx, final ParseReader din, final ParseWriter dout) {
		...
		if (offset >= 0) {
			...
			if (_setup._check_header == ParseSetup.HAS_HEADER || cidx > 0) {
				state = SKIP_LINE;
			} else
				state = WHITESPACE_BEFORE_TOKEN;
		}
		...
		if (cidx == 0) {
			while (c == '#' || isEOL(c) || c == '@' /* also treat as comments leading '@' from ARFF format */
		...
		final boolean forceable = dout instanceof FVecParseWriter && ((FVecParseWriter) dout)._ctypes != null
				&& _setup._column_types != null;
		boolean main_loop = true;

		while (main_loop) {
			boolean forcedCategorical = forceable && colIdx < _setup._column_types.length
					&& _setup._column_types[colIdx] == Vec.T_CAT;
			boolean forcedString = forceable && colIdx < _setup._column_types.length
					&& _setup._column_types[colIdx] == Vec.T_STR;
			...
			case STRING_END:
				...
				if (!isNa && _setup.isNA(colIdx, str)) {
					isNa = true;
				}
				if (!isNa) {
			...
			case EOL:
				if (quotes != 0) {
					// System.err.println("Unmatched quote char " + ((char)quotes) + " " +
					// (((str.length()+1) < offset && str.getOffset() > 0)?new
					// String(Arrays.copyOfRange(bits,str.getOffset()-1,offset)):""));
					String err = "Unmatched quote char " + ((char) quotes);
					dout.invalidLine(
							new ParseWriter.ParseErr(err, cidx, dout.lineNum(), offset + din.getGlobalByteOffset()));
					colIdx = 0;
					quotes = 0;
				} else if (colIdx != 0) {
					dout.newLine();
			...
			c = bits[offset];
			if (isEOL(c) && state != COND_QUOTE && quotes != 0) { // quoted string having newline character => fail the
																	// line!
				state = EOL;
			}
		...
		if (lines != null && lines.length > 0) {
			...
			if (_setup._column_names != null) {
				for (int i = 0; hasHdr && i < firstLine.length; ++i)
					hasHdr = (_setup._column_names[i] == firstLine[i]) || (_setup._column_names[i] != null
							&& _setup._column_names[i].equalsIgnoreCase(firstLine[i]));
			} else { // declared to have header, but no column names provided, assume header exist in
						// all files
	...
	// ==========================================================================
	/**
	 * Separators recognized by the CSV parser. You can add new separators to this
	 * list and the parser will automatically attempt to recognize them. In case of
	 * doubt the separators are listed in descending order of probability, with
	 * space being the last one - space must always be the last one as it is used if
	 * all other fails because multiple spaces can be used as a single separator.
	 */
	static final byte HIVE_SEP = 0x1; // '^A', Hive table column separator
	private static byte[] separators = new byte[] { HIVE_SEP, ',', ';', '|', '\t',
			' '/* space is last in this list, because we allow multiple spaces */ };
	...
	/**
	 * Determines the tokens that are inside a line and returns them as strings in
	 * an array. Assumes the given separator.
	 */
	...
	public static String[] determineTokens(String from, byte separator, byte singleQuote) {
		...
		while (offset < bits.length) {
			while ((offset < bits.length) && (bits[offset] == CsvParser.CHAR_SPACE))
				++offset; // skip first whitespace
			if (offset == bits.length) {
				break;
			}
	...
	public static byte guessSeparator(String l1, String l2, boolean singleQuotes) {
		...
		for (int i = 0; i < s1.length; ++i) {
			...
			if (s1[i] == s2[i] && s1[i] >= s1[max] >> 1) { // Sep counts are equal? And at nearly as large as the larger
															// header sep?
				try {
	...
	/**
	 * Determines the CSV parser setup from the first few lines. Also parses the
	 * next few lines, tossing out comments and blank lines.
	 *
	...
	static ParseSetup guessSetup(byte[] bits, byte sep, int ncols, boolean singleQuotes, int checkHeader,
			String[] columnNames, byte[] columnTypes, String[][] naStrings) {
		...
		if (lines.length == 1) { // Ummm??? Only 1 line?
			if (sep == GUESS_SEP) {
				if (lines[0].split(",").length > 1) {
					...
					return new ParseSetup(CSV_INFO, GUESS_SEP, singleQuotes, checkHeader, 1, null, ctypes, domains,
			...
			} else if (checkHeader == HAS_HEADER) {
				labels = data[0];
			} else
				labels = null;
			...
			if (columnNames != null && labels != null) {
				if (labels.length != columnNames.length) {
					throw new ParseDataset.H2OParseException("Already have " + columnNames.length
							+ " column labels, but found " + labels.length + " in this file");
				} else {
					for (int i = 0; i < labels.length; ++i)
						if (!labels[i].equalsIgnoreCase(columnNames[i])) {
							throw new ParseDataset.H2OParseException("Column " + (i + 1) + " label '" + labels[i]
									+ "' does not match '" + columnNames[i] + "'");
						}
		...
		ParseSetup resSetup = new ParseSetup(CSV_INFO, sep, singleQuotes, checkHeader, ncols, labels, null,
				null /* domains */, naStrings, data);
		...
		if (columnTypes == null || ncols != columnTypes.length) {
			...
			for (int i = 0; i < columnTypes.length; i++)
				if (columnTypes[i] == Vec.T_BAD) {
					columnTypes[i] = Vec.T_NUM;
				}
