...
public class FVecParseWriter extends Iced implements StreamParseWriter {
	...
	public FVecParseWriter close(Futures fs) {
		if (_nvs == null) {
			return this; // Might call close twice
		}
	...
	public void addNumCol(int colIdx, long number, int exp) {
		if (colIdx < _nCols) {
			...
			if (_ctypes != null && _ctypes[colIdx] == Vec.T_BAD) {
				_ctypes[colIdx] = Vec.T_NUM;
			}
		}
	}
	...
	public boolean isString(int colIdx) {
		return (colIdx < _nCols) && (_ctypes[colIdx] == Vec.T_CAT || _ctypes[colIdx] == Vec.T_STR);
	}
		...
		if (colIdx < _nvs.length) {
			...
			if (_ctypes[colIdx] == Vec.T_TIME) {
				...
				if (!_categoricals[colIdx].isMapFull()) {
					...
					if (_ctypes[colIdx] == Vec.T_BAD && id > 1) {
						_ctypes[colIdx] = Vec.T_CAT;
					}
					if (_ctypes[colIdx] == Vec.T_CAT) {
						_nvs[colIdx].addNum(id, 0); // if we are sure we have a categorical column, we can only store
													// the integer (more efficient than remembering this value was
													// categorical)
					} else
						_nvs[colIdx].addCategorical(id);
				} else { // maxed out categorical map
					throw new ParseDataset.H2OParseException("Exceeded categorical limit on column #" + (colIdx + 1)
							+ " (using 1-based indexing).  Consider reparsing this column as a string.");
				}
			}
		}
	}
