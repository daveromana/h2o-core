...
public final class ParseDataset {
	...
	public static Frame parse(Key okey, Key... keys) {
		return parse(okey, keys, true, false, ParseSetup.GUESS_HEADER);
	}
	...
	public static ParseDataset parse(Key okey, Key[] keys, boolean deleteOnDone, ParseSetup globalSetup,
			boolean blocking) {
	 ...
	 *
	 * @param dest
	 *            name for destination key
	 * @param keys
	 *            input keys
	 * @param parseSetup
	 *            a generic parser setup
	 * @param deleteOnDone
	 *            delete input data when finished
	 * @return a new parse job
	 */
	public static ParseDataset forkParseDataset(final Key<Frame> dest, final Key[] keys, final ParseSetup parseSetup,
			boolean deleteOnDone) {
		// Get a parser specific setup
		// FIXME: ParseSetup should be separated into two classes - one for using via
		// Rest API as user setup
		...
		// Some quick sanity checks: no overwriting your input key, and a resource
		// check.
				...
				for (int j = i + 1; j < keys.length; j++)
					if (k == keys[j]) {
						throw new IllegalArgumentException(
								"Source key " + k + " appears twice, deleteOnDone must be false");
					}
			}
		...
		if (totalParseSize > memsz * 4) {
			throw new IllegalArgumentException("Total input file size of " + PrettyPrint.bytes(totalParseSize)
					+ " is much larger than total cluster memory of " + PrettyPrint.bytes(memsz)
		...
		new Frame(pds._job._result, new String[0], new Vec[0]).delete_and_lock(pds._job); // Write-Lock BEFORE returning
		for (Key k : keys)
			Lockable.read_lock(k, pds._job); // Read-Lock BEFORE returning
	...
	public static class ParserFJTask extends water.H2O.H2OCountedCompleter {
		...
		private void parseCleanup() {
			...
			// Since this is racily called, perhaps multiple times, read _mfpt only exactly
			// once.
	...
	private static ParseDataset parseAllKeys(ParseDataset pds, Key[] fkeys, ParseSetup setup, boolean deleteOnDone) {
		...
		if (setup._column_names != null && ((setup._column_names.length == 0)
				|| (setup._column_names.length == 1 && setup._column_names[0].isEmpty()))) {
			setup._column_names = null; // // FIXME: annoyingly front end sends column names as String[] {""} even if
										// setup returned null
		}
				...
				job.update(0, "Collecting categorical domains across nodes.");
				{
					...
					if (offendingColNames.size() > 0) {
						throw new H2OParseException("Exceeded categorical limit on columns " + offendingColNames
								+ ".   Consider reparsing these columns as a string.");
					}
				}
		...
		// SVMLight is sparse format, there may be missing chunks with all 0s, fill them
		// in
	...
	private static class CreateParse2GlobalCategoricalMaps extends DTask<CreateParse2GlobalCategoricalMaps> {
		...
		public void compute2() {
			...
			// get the node local category->ordinal maps for each column from initial parse
			// pass
			...
			for (int eColIdx = 0; eColIdx < _ecol.length; eColIdx++) {
				...
				if (parseCatMaps[colIdx].size() != 0) {
					...
					// Bulk String->BufferedString conversion is slightly faster, but consumes
					// memory
			...
			// Store the local->global ordinal maps in DKV by node parse categorical key and
			// node index
			DKV.put(Key.make(_parseCatMapsKey.toString() + "parseCatMapNode" + H2O.SELF.index()),
					new CategoricalUpdateMap(_nodeOrdMaps));
			tryComplete();
		}
	}
	...
	// --------------------------------------------------------------------------
	/**
	 * Task to update categorical (categorical) values to match the global numbering
	 * scheme. Performs update in place so that values originally numbered using
	...
	private static class UpdateCategoricalChunksTask extends MRTask<UpdateCategoricalChunksTask> {
		...
		public void map(Chunk[] chks) {
			CategoricalUpdateMap temp = DKV.getGet(
					Key.make(_parseCatMapsKey.toString() + "parseCatMapNode" + _chunk2ParseNodeMap[chks[0].cidx()]));
			...
			for (int i = 0; i < chks.length; ++i) {
				...
				if (!(chk instanceof CStrChunk)) {
					for (int j = 0; j < chk._len; ++j) {
						...
						if (_parse2GlobalCatMaps[i] != null && _parse2GlobalCatMaps[i][old] < 0) {
							throw new H2OParseException("Error in unifying categorical values. This is typically "
									...
									+ "occurred in the " + PrettyPrint.withOrdinalIndicator(i + 1)
									+ " categorical col, " + PrettyPrint.withOrdinalIndicator(chk.start() + j)
									+ " row.");
						}
					...
					Log.trace(
							"Updated domains for " + PrettyPrint.withOrdinalIndicator(i + 1) + " categorical column.");
				}
				chk.close(cidx, _fs);
			}
		}
	...
	private static class MultiFileParseTask extends MRTask<MultiFileParseTask> {
		...
		// It is either self for all the non-parallel parses, or the Chunk-home for
		// parallel parses.
		...
		MultiFileParseTask(VectorGroup vg, ParseSetup setup, Key<Job> jobKey, Key[] fkeys, boolean deleteOnDone) {
			...
			_vecIdStart = _vg.reserveKeys(
					_reservedKeys = _parseSetup._parse_type.equals(SVMLight_INFO) ? 100000000 : setup._number_columns);
			_deleteOnDone = deleteOnDone;
			_jobKey = jobKey;
			// A mapping of Key+ByteVec to rolling total Chunk counts.
		...
		public void postGlobal() {
			...
			for (FVecParseWriter fvpw : _dout) {
				...
				// Quick assert that all partial AVs in each DOUT are sharing a common chunkOff,
				// and common Vec Keys
		...
		// Fetch out the node-local Categorical[] using _cKey and _categoricals
		// hashtable
		...
		private FVecParseWriter makeDout(ParseSetup localSetup, int chunkOff, int nchunks) {
			...
			final byte[] ctypes = localSetup._column_types; // SVMLight only uses numeric types, sparsely represented as
															// a null
			for (int i = 0; i < avs.length; ++i)
				avs[i] = new AppendableVec(_vg.vecKey(i + _vecIdStart), espc,
						ctypes == null ? /* SVMLight */Vec.T_NUM : ctypes[i], chunkOff);
			return localSetup._parse_type.equals(SVMLight_INFO)
					...
					: new FVecParseWriter(_vg, chunkOff, categoricals(_cKey, localSetup._number_columns),
							localSetup._column_types, _parseSetup._chunk_size, avs);
		}
		...
		public void map(Key key) {
			...
			if (localSetup._check_header == ParseSetup.HAS_HEADER) {// check for header on local file
				localSetup._check_header = localSetup.parser(_jobKey)
						.fileHasHeader(ZipUtil.unzipBytes(zips, cpr, localSetup._chunk_size), localSetup);
			}
			// Parse the file
			try {
				switch (cpr) {
				case NONE:
					...
					if (pm == ParserInfo.ParseMethod.DistributedParse) {
						new DistributedParse(_vg, localSetup, _vecIdStart, chunkStartIdx, this, key, vec.nChunks())
						...
						localSetup = ParserService.INSTANCE.getByInfo(localSetup._parse_type).setupLocal(vec,
								localSetup);
						InputStream bvs = vec.openStream(_jobKey);
						Parser p = localSetup.parser(_jobKey);
						_dout[_lo] = ((FVecParseWriter) p.streamParse(bvs,
				...
				case ZIP: {
					...
					if (ze != null && !ze.isDirectory()) {
						_dout[_lo] = streamParse(zis, localSetup, makeDout(localSetup, chunkStartIdx, vec.nChunks()),
								bvs);
						_errors = _dout[_lo].removeErrors();
					}
				...
				case GZIP: {
					...
					InputStream bvs = vec.openStream(_jobKey);
					// Zipped file; no parallel decompression;
					_dout[_lo] = streamParse(new GZIPInputStream(bvs), localSetup,
							makeDout(localSetup, chunkStartIdx, vec.nChunks()), bvs);
					_errors = _dout[_lo].removeErrors();
		...
		public void reduce(MultiFileParseTask mfpt) {
			...
			if (_dout == null) {
				_dout = mfpt._dout;
			} else if (_dout != mfpt._dout) {
				_dout = ArrayUtils.append(_dout, mfpt._dout);
			}
			if (_chunk2ParseNodeMap == null) {
				...
				for (int i = 0; i < _chunk2ParseNodeMap.length; ++i) {
					if (_chunk2ParseNodeMap[i] == -1) {
						...
						assert mfpt._chunk2ParseNodeMap[i] == -1 : Arrays.toString(_chunk2ParseNodeMap) + " :: "
								+ Arrays.toString(mfpt._chunk2ParseNodeMap);
				}
			}
			if (_errors == null) {
		...
		private FVecParseWriter streamParse(final InputStream is, final ParseSetup localSetup, FVecParseWriter dout,
				InputStream bvs) throws IOException {
		...
		private static class DistributedParse extends MRTask<DistributedParse> {
			...
			DistributedParse(VectorGroup vg, ParseSetup setup, int vecIdstart, int startChunkIdx,
					MultiFileParseTask mfpt, Key srckey, int nchunks) {
				super(null);
				_vg = vg;
				_setup = setup;
			...
			public void map(Chunk in) {
				...
				for (int i = 0; i < avs.length; ++i)
					if (_setup._column_types == null) {// SVMLight
						...
						avs[i] = new AppendableVec(_vg.vecKey(_vecIdStart + i), _espc, _setup._column_types[i],
								_startChunkIdx);
				...
				case "ORC": // setup special case for ORC
					...
					break;
				default: // FIXME: should not be default and creation strategy should be forwarded to
							// ParserProvider
			...
			public void postGlobal() {
				...
				if (_dout.hasErrors()) {
					...
					for (ParseWriter.ParseErr err : errs)
						err._file = FileVec.getPathForKey(_srckey).toString();
	...
	public static void logParseResults(Frame fr) {
		...
		Log.info(String.format(format, "ColV2", "type", "min", "max", "mean", "sigma", "NAs", "constant",
				"cardinality"));
		...
		for (int i = 0; i < vecArr.length; i++) {
			...
			String s = String.format(format, CStr, typeStr, minStr, maxStr, meanStr, sigmaStr, naStr, isConstantStr,
					numLevelsStr);
