...
public class ParseSetup extends Iced {
	...
	public ParseSetup(ParseSetup ps) {
		this(ps._parse_type, ps._separator, ps._single_quotes, ps._check_header, ps._number_columns, ps._column_names,
				ps._column_types, ps._domains, ps._na_strings, ps._data, new ParseWriter.ParseErr[0], ps._chunk_size);
	}

	public static ParseSetup makeSVMLightSetup() {
		return new ParseSetup(SVMLight_INFO, ParseSetup.GUESS_SEP, false, ParseSetup.NO_HEADER, 1, null,
				new byte[] { Vec.T_NUM }, null, null, null, new ParseWriter.ParseErr[0]);
	}

	// This method was called during guess setup, lot of things are null, like
	// ctypes.
	// when it is called again, it either contains the guess column types or it will
	// have user defined column types
	public ParseSetup(ParserInfo parse_type, byte sep, boolean singleQuotes, int checkHeader, int ncols,
			String[] columnNames, byte[] ctypes, String[][] domains, String[][] naStrings, String[][] data,
			ParseWriter.ParseErr[] errs, int chunkSize) {
	 ...
	 *
	 * Typically used to guide sampling in the data to verify chosen settings, and
	 * fill in missing settings.
	 *
	 * @param ps
	 *            Parse setup settings from client
	 */
	public ParseSetup(ParseSetupV3 ps) {
		this(ps.parse_type != null ? ParserService.INSTANCE.getByName(ps.parse_type).info() : GUESS_INFO,
				ps.separator != 0 ? ps.separator : GUESS_SEP, ps.single_quotes, ps.check_header, GUESS_COL_CNT,
				ps.column_names, strToColumnTypes(ps.column_types), null, ps.na_strings, null,
				new ParseWriter.ParseErr[0], ps.chunk_size);
	}
	 ...
	 */
	public ParseSetup(ParserInfo parseType, byte sep, boolean singleQuotes, int checkHeader, int ncols,
			String[] columnNames, byte[] ctypes, String[][] domains, String[][] naStrings, String[][] data) {
		this(parseType, sep, singleQuotes, checkHeader, ncols, columnNames, ctypes, domains, naStrings, data,
				new ParseWriter.ParseErr[0], FileVec.DFLT_CHUNK_SIZE);
	}

	public ParseSetup(ParserInfo parseType, byte sep, boolean singleQuotes, int checkHeader, int ncols,
			String[] columnNames, byte[] ctypes, String[][] domains, String[][] naStrings, String[][] data,
			ParseWriter.ParseErr[] errs) {
		this(parseType, sep, singleQuotes, checkHeader, ncols, columnNames, ctypes, domains, naStrings, data, errs,
	 ...
	 */
	public ParseSetup(ParserInfo parseType, byte sep, boolean singleQuotes, int checkHeader, int ncols, String[][] data,
			ParseWriter.ParseErr[] errs) {
		this(parseType, sep, singleQuotes, checkHeader, ncols, null, null, null, null, data, errs,
				FileVec.DFLT_CHUNK_SIZE);
	}
	...
	public static byte[] strToColumnTypes(String[] strs) {
		...
		for (int i = 0; i < types.length; i++) {
			...
			default:
				types[i] = Vec.T_BAD;
				throw new H2OIllegalArgumentException("Provided column type " + strs[i]
						+ " is unknown.  Cannot proceed with parse due to invalid argument.");
			}
		}
		return types;
	}
	...
	// return true iff the first line is all strings and second line has at least
	// one number
	 ...
	 *
	 * @param fkeys
	 *            Keys to input vectors to be parsed
	 * @param singleQuote
	...
	public static ParseSetup guessSetup(Key[] fkeys, boolean singleQuote, int checkHeader) {
		return guessSetup(fkeys, new ParseSetup(GUESS_INFO, GUESS_SEP, singleQuote, checkHeader, GUESS_COL_CNT, null,
				new ParseWriter.ParseErr[0]));
	}

	/**
	 * Discover the parse setup needed to correctly parse all files. This takes a
	 * ParseSetup as guidance. Each file is examined individually and then results
	 * merged. If a conflict exists between any results all files are re-examined
	 * using the best guess from the first examination.
	 *
	 * @param fkeys
	 *            Keys to input vectors to be parsed
	 * @param userSetup
	 *            Setup guidance from user
	 * @return ParseSetup settings from looking at all files
	 */
	public static ParseSetup guessSetup(Key[] fkeys, ParseSetup userSetup) {
		...
		// Calc chunk-size
		// FIXME: should be a parser specific - or at least parser should be able to
		// override defaults
		...
		if (ice instanceof Frame && ((Frame) ice).vec(0) instanceof UploadFileVec) {
			...
			t._gblSetup._chunk_size = FileVec.calcOptimalChunkSize(t._totalParseSize, t._gblSetup._number_columns,
					t._maxLineLength, Runtime.getRuntime().availableProcessors(), H2O.getCloudSize(),
					false /* use new heuristic */, true);
		}
	...
	public static class GuessSetupTsk extends MRTask<GuessSetupTsk> {
		 ...
		 *
		 * @param userSetup
		 *            ParseSetup to guide examination of files
		 */
		 ...
		 *
		 * For ByteVecs, UploadFileVecs, compressed files and small files, the
		 * ParseSetup is guessed from a single DFLT_CHUNK_SIZE chunk from the start of
		 * the file. This is because UploadFileVecs and compressed files don't allow
		 * random sampling, small files don't need it, and ByteVecs tend to be small.
		 *
		 * For larger NSFFileVecs and HDFSFileVecs 1M samples are taken at the beginning
		 * of every 100M, and an additional sample is taken from the last chunk of the
		 * file. The results of these samples are merged together (and compared for
		 * consistency).
		 *
		 * Sampling more than the first bytes is preferred, since large data sets with
		 * sorted columns may have all the same value in their first bytes, making for
		 * poor type guesses.
		 *
		 */
		@Override
		public void map(Key key) {
			...
			if (bits != null && bits.length > 0) {
				...
				// since later calculation of chunk size and later number of chunks do not
				// consider the
				// compression ratio, we should not do that here either. Quick fix proposed by
				// Tomas. Sleek!
				...
				// Compute the max line length (to help estimate the number of bytes to read per
				// Parse map)
				_maxLineLength = maxLineLength(bits);
				if (_maxLineLength == -1) {
					throw new H2OIllegalArgumentException(
							"The first 4MB of the data don't contain any line breaks. Cannot parse.");
				}

				// only preview 1 DFLT_CHUNK_SIZE for ByteVecs, UploadFileVecs, compressed, and
				// small files
				/*
				 * if (ice instanceof ByteVec || ((Frame)ice).vecs()[0] instanceof UploadFileVec
				 * || bv.length() <= FileVec.DFLT_CHUNK_SIZE || decompRatio > 1.0) {
				 */
				try {
				...
				/*
				 * } else { // file is aun uncompressed NFSFileVec or HDFSFileVec & larger than
				 * the DFLT_CHUNK_SIZE FileVec fv = (FileVec) ((Frame) ice).vecs()[0]; // reset
				 * chunk size to 1M (uncompressed) int chkSize = (int) ((1<<20) /decompRatio);
				 * fv.setChunkSize((Frame) ice, chkSize);
				 * 
				 * // guessSetup from first chunk _gblSetup =
				 * guessSetup(fv.getPreviewChunkBytes(0), _userSetup); _userSetup._check_header
				 * = -1; // remaining chunks shouldn't check for header _userSetup._parse_type =
				 * _gblSetup._parse_type; // or guess parse type
				 * 
				 * //preview 1M data every 100M int numChunks = fv.nChunks(); for (int i=100; i
				 * < numChunks;i += 100) { bits = fv.getPreviewChunkBytes(i); if (bits != null)
				 * _gblSetup = mergeSetups(_gblSetup, guessSetup(bits, _userSetup)); }
				 * 
				 * // grab sample at end of file (if not done by prev loop) if (numChunks % 100
				 * > 1){ bits = fv.getPreviewChunkBytes(numChunks - 1); if (bits != null)
				 * _gblSetup = mergeSetups(_gblSetup, guessSetup(bits, _userSetup)); }
				 * 
				 * // return chunk size to DFLT fv.setChunkSize((Frame) ice,
				 * FileVec.DFLT_CHUNK_SIZE); }
				 */
				// report if multiple files exist in zip archive
				/*
				 * if (ZipUtil.getFileCount(bv) > 1) { if (_gblSetup._errors != null)
				 * _gblSetup._errors = Arrays.copyOf(_gblSetup._errors, _gblSetup._errors.length
				 * + 1); else _gblSetup._errors = new String[1];
				 * 
			...
			if (_gblSetup == null) {
				throw new RuntimeException(
						"This H2O node couldn't find the file(s) to parse. Please check files and/or working directories.");
			}
		...
		private ParseSetup mergeSetups(ParseSetup setupA, ParseSetup setupB, String fileA, String fileB) {
			// FIXME: have a merge function defined on a specific parser setup (each parser
			// setup is responsible for merge)
			if (setupA == null) {
				return setupB;}
			if (setupA._parse_type.equals(DefaultParserProviders.SVMLight_INFO){
					&& setupB._parse_type.equals(DefaultParserProviders.SVMLight_INFO)) {
				// no merging for svm light, all columns are numeric and we take the max of
				// number of columns (it's an estimate anyways)
			...
			if (setupA._parse_type.equals(ARFF_INFO) && setupB._parse_type.equals(CSV_INFO)) {
				...
				mergedSetup._column_previews = PreviewParseWriter.unifyColumnPreviews(setupA._column_previews,
						setupB._column_previews);
			} else
				throw new ParseDataset.H2OParseException("File type mismatch. Cannot parse files " + setupA.file()
						+ " and " + setupB.file() + " of type " + setupA._parse_type.name() + " and "
						+ setupB._parse_type.name() + " as one dataset.");
			mergedSetup._column_names = unifyColumnNames(setupA._column_names, setupB._column_names);
			mergedSetup._number_columns = mergedSetup._parse_type.equals(CSV_INFO)
					? Math.max(setupA._number_columns, setupB._number_columns)
					: unifyColumnCount(setupA._number_columns, setupB._number_columns, mergedSetup, fileA, fileB);
		...
		private static byte unifyColumnSeparators(byte sepA, byte sepB) {
			...
			throw new ParseDataset.H2OParseException("Column separator mismatch. One file seems to use \"" + (char) sepA
					+ "\" and the other uses \"" + (char) sepB + "\".");
		}
	...
	protected boolean isCompatible(ParseSetup setupB) {
		return _parse_type.equals(setupB._parse_type) && _parse_type.equals(DefaultParserProviders.SVMLight_INFO)
				|| _number_columns == setupB._number_columns;
	}

	/**
	 * Guess everything from a single pile-o-bits. Used in tests, or in initial
	 * parser inspections when the user has not told us anything about separators or
	 * headers.
	 *
	 * @param bits
	...
	public static ParseSetup guessSetup(ByteVec bv, byte[] bits, ParseSetup userSetup) {
		return guessSetup(bv, bits, userSetup._parse_type, userSetup._separator, GUESS_COL_CNT,
				userSetup._single_quotes, userSetup._check_header, userSetup._column_names, userSetup._column_types,
				null, null);
	}

	public static ParseSetup guessSetup(ByteVec bv, byte[] bits, ParserInfo parserType, byte sep, int ncols,
			boolean singleQuotes, int checkHeader, String[] columnNames, byte[] columnTypes, String[][] domains,
			String[][] naStrings) {
		ParserProvider pp = ParserService.INSTANCE.getByInfo(parserType);
		if (pp != null) {
			return pp.guessSetup(bv, bits, sep, ncols, singleQuotes, checkHeader, columnNames, columnTypes, domains,
					naStrings);
		}
	 ...
	 * Eliminates common file extensions, and replaces odd characters.
	 *
	 * @param n
	...
	public static String createHexName(String n) {
		...
		while (dot > 0 && (n.endsWith("zip") || n.endsWith("gz") || n.endsWith("csv") || n.endsWith("xls")
				|| n.endsWith("txt") || n.endsWith("svm") || n.endsWith("orc") || n.endsWith("arff"))) {
			n = n.substring(0, dot);
			dot = n.lastIndexOf('.');
		}
	 ...
	 *
	 * For the curious, this is hardly a complete test, it only catches the most
	 * polite UTF-16 cases. Switch to jChardet or guessEncoding libraries for more
	 * robust solutions. WARNING: not all UTF-16 files use BOM to indicate their
	 * encoding. Even worse, some datasets may be made from disparate sources, and
	 * could used a mix that wouldn't be detected by this.
	 *
	 * @param bits
	...
	private static final void checkEncoding(byte[] bits) {
		if (bits.length >= 2) {
			if ((bits[0] == (byte) 0xff && bits[1] == (byte) 0xfe)/* UTF-16, little endian */ || (bits[0] == (byte) 0xfe
					&& bits[1] == (byte) 0xff) /* UTF-16, big endian */) {
	 ...
	 * 
	 * @param bytes
	 *            Array of bytes (containing 0 or more newlines)
	...
	/**
	 * Copies the common setup to another object (that is possibly and extension of
	 * the base setup). Note: this method only copies fields directly declared in
	 * ParseSetup class, it doesn't handle fields that are declared in classes
	 * derived from ParseSetup.
	 * 
	 * @param setup
	 *            target setup object
	 * @param <T>
	 *            class derived from ParseSetup
	...
	/**
	 * Tests whether a given string represents a NA in a given column. Note: NAs are
	 * expected to be made ONLY of ASCII (7-bit) characters, NA constants in unicode
	 * won't be recognized.
	 * 
	 * @param colIdx
	 *            index of the column
	 * @param str
	 *            string to be tested for NA
