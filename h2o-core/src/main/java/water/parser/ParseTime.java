...
public abstract class ParseTime {
	...
	// "sun.util.calendar.Gregorian$Date cannot be cast to
	...
	private static final byte MMS[][][] = new byte[][][] { { bytesOf("jan"), bytesOf("january") },
			{ bytesOf("feb"), bytesOf("february") }, { bytesOf("mar"), bytesOf("march") },
			{ bytesOf("apr"), bytesOf("april") }, { bytesOf("may"), bytesOf("may") },
			{ bytesOf("jun"), bytesOf("june") }, { bytesOf("jul"), bytesOf("july") },
			{ bytesOf("aug"), bytesOf("august") }, { bytesOf("sep"), bytesOf("september") },
			{ bytesOf("oct"), bytesOf("october") }, { bytesOf("nov"), bytesOf("november") },
	...
	public static long attemptTimeParse(BufferedString str) {
		try {
			...
			long t2 = attemptYearMonthTimeParse(str); // "yy-MMM", not ambiguous with dd-MMM-yy because of trailing
														// "-yy"
			...
			long t4 = attemptDayFirstTimeParse2(str); // "dd/MM/yy" and time if present; note that this format is
														// ambiguous
			if (t4 != Long.MIN_VALUE) {
				return t4; // Cant tell which date: 3/2/10 is
			}
		} catch (org.joda.time.IllegalFieldValueException | // Not time at all
				org.joda.time.IllegalInstantException // Parsed as time, but falls into e.g. a daylight-savings hour
														// hole
		ie) {
		} // FIXME should collect errors and report at end of parse
		return Long.MIN_VALUE;
	}

	// Tries to parse "yyyy-MM[-dd] [HH:mm:ss.SSS aa]"
	// Tries to parse "yyyyMMdd-HH:mm:ss.SSS aa". In this form the dash and trailing
	// time is required
	private static long attemptYearFirstTimeParse(BufferedString str) {
		...
		MM = digit(MM, buf[i++]);
		// note: at this point we need guard every increment of "i" to avoid reaching
		...
		if ((end - i) >= 2) {
			...
			if (!dash) {
				return Long.MIN_VALUE; // yyyyMM is ambiguous with plain numbers
			}
			dd = 1; // yyyy-MM; no day
		}
		if (dash) { // yyyy-MM[-dd]
	...
	private static long attemptDayFirstTimeParse1(BufferedString str) {
		...
		if (MM == -1) {
			return Long.MIN_VALUE; // No matching month
		}
	...
	private static long attemptYearMonthTimeParse(BufferedString str) {
		...
		while (i < end && buf[i] == ' ')
			i++;
		if (i == end) {
			return new DateTime(yyyy, MM, 1, 0, 0, 0, getTimezone()).getMillis();
		}
	 ...
	 * 
	 * @param buf
	 *            - byte buffer containing text to parse
	 * @param i
	 *            - index of expected start of time string in buffer
	 * @return -1 if failed parse, or (bytes_parsed<<4)|(month). One-based month is
	 *         returned in the low nybble.
	 */
	...
	/**
	 * Attempts to parse time. Expects at least: HH:mm:ss where : or . are accepted
	 * as delimiters Additionally the time can contain either 1 or 3 or 9 places for
	 * fractions of a second e.g. HH:mm:ss.SSS or HH:mm:ss.SSSnnnnnn Note that only
	 * millisecond accuracy is stored Additionally the time can end with AM|PM. When
	 * AM or PM is present, HH must be 1-12. When absent, HH must be 0-23. If the
	 * text doesn't fit this format it returns Long.MIN_VALUE to indicate failed
	 * parse
	 *
	 * @param buf
	 *            - byte buffer containing text to parse
	 ...
	 *            - index of expected start of time string in buffer
	 * @param end
	 *            - index for end of time in buffer
	 * @param yyyy
	 *            - 4 digit year
	 * @param MM
	 *            - month of year (1-12)
	 ...
	 * @return long representing time in currently timezone as milliseconds since
	 *         UNIX epoch or Long.MIN_VALUE to represent failed time parse
	 */
	...
	public static String listTimezones() {
		DateTimeFormatter offsetFormatter = new DateTimeFormatterBuilder().appendTimeZoneOffset(null, true, 2, 4)
				.toFormatter();
		Set<String> idSet = DateTimeZone.getAvailableIDs();
	...
	/**
	 * Factory to create a formatter from a strptime pattern string. This models the
	 * commonly supported features of strftime from POSIX (where it can).
	 * <p>
	 * The format may contain locale specific output, and this will change as you
	 * change the locale of the formatter. Call DateTimeFormatter.withLocale(Locale)
	 * to switch the locale. For example:
	 * 
	 * <pre>
		...
		for (int i = 0; i < length; i++) {
			...
			if (c == '%' && token.charAt(1) != '%') {
				...
				case 'T':
					...
					break;
				/*
				 * case 'U': //FIXME Joda does not support US week start (Sun), this will be
				 * wrong builder.appendWeekOfYear(2); break; case 'u':
				 * builder.appendDayOfWeek(1); break;
				 */
				case 'V':
					break; // accepted and ignored
				/*
				 * case 'w': //FIXME Joda does not support US week start (Sun), this will be
				 * wrong builder.appendDayOfWeek(1); break; case 'W':
				 * builder.appendWeekOfYear(2); break;
				 */
				case 'x':
					builder.appendTwoDigitYear(2019);
					builder.appendLiteral('/');
					...
					break;
				/*
				 * case 'X': //Results differ between OSX and Linux builder.appendHourOfDay(2);
				 * builder.appendLiteral(':'); builder.appendMinuteOfHour(2);
				 * builder.appendLiteral(':'); builder.appendSecondOfMinute(2); break;
				 */
	 ...
	 *            the pattern string
	 * @param indexRef
	 *            a single element array, where the input is the start location and
	 *            the output is the location after parsing the token
