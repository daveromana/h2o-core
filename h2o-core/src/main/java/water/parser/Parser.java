 ...
 *
 * Interfaces: DataIn - Manage bulk streaming input data to the parser.
 * Sometimes the data comes from parallel raw byte file reads, with speculative
 * line starts. Sometimes the data comes from an InputStream - probably a GZIP
 * stream. DataOut- Interface for writing results of parsing, accumulating
 * numbers and strings or handling invalid lines &amp; parse errors.
 *
 * static classes: StreamData - Class implementing DataIn from a Stream
 * (probably a GZIP stream) InspectDataOut - Class implementing DataOut, on
 * behalf of the GUI, for parsing &amp; previewing the first few lines &amp;
 * columns of a file.
 */
public abstract class Parser extends Iced {
	...
	/**
	 * This method performs guess setup with each file. If will return true only if
	 * the number of columns/separator found in the current file match that of files
	 * parsed earlier. In addition, it will also check for headers within a file.
	 * However, it will only check for headers if the user has included column names
	...
	private boolean checkFileNHeader(final InputStream is, final StreamParseWriter dout, StreamData din, int cidx)
		...
		ParseSetup ps = ParseSetup.guessSetup(null, headerBytes, GUESS_INFO, ParseSetup.GUESS_SEP,
				ParseSetup.GUESS_COL_CNT, this._setup._single_quotes, ParseSetup.GUESS_HEADER, null, null, null, null);
		// check to make sure datasets in file belong to the same dataset
		// just check for number for number of columns/separator here. Ignore the column
		// type, user can force it
		if ((this._setup._number_columns != ps._number_columns) || (this._setup._separator != ps._separator)) {
			...
			// assume column names must appear in the first file. If column names appear in
			// first and other
			// files, they will be recognized. Otherwise, if no column name ever appear in
			// the first file, the other
			...
			if (ps._check_header == ParseSetup.HAS_HEADER) {
				if (this._setup._column_names != null) {
					// found header in later files, only incorporate it if the column names are the
					// same as before
					...
					if (sameColumnNames) { // only recognize current file header if it has the same column names as
											// previous files
	...
	/**
	 * This method will try to get the next file to be parsed. It will skip over
	 * directories if encountered.
	 *
	 * @param is
	 * @throws IOException
	 */
	...
	/**
	 * This method reads in one zip file. Before reading the file, it will check if
	 * the current file has the same number of columns and separator type as the
	 * previous files it has parssed. If they do not match, no file will be parsed
	 * in this case.
	 *
	 * @param is
	 * @param dout
	...
	private StreamInfo readOneFile(final InputStream is, final StreamParseWriter dout, InputStream bvs,
		...
		// only check header for 2nd file onward since guess setup is already done on
		// first file.
	...
	protected ParseWriter streamParseZip(final InputStream is, final StreamParseWriter dout, InputStream bvs)
			throws IOException {
		...
		while (is.available() > 0) { // loop over all files in zip file
			streamInfo = readOneFile(is, dout, bvs, streamInfo._nextChunk, streamInfo._zidx, fileIndex++); // read one
																											// file in
			// streamInfo = readOneFile(is, dout, bvs, nextChunk, streamInfo._zidx,
			// fileIndex++); // read one file in
	...
	/**
	 * Class implementing DataIns from a Stream (probably a GZIP stream) Implements
	 * a classic double-buffer reader.
	 */
