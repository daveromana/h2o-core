...
class BinaryMerge extends DTask<BinaryMerge> {
	...
	static class FFSB extends Iced<FFSB> {
		...
		FFSB(Frame frame, int msb, int shift, int fieldSizes[], BigInteger base[]) {
			...
			if (vec == null) {
				return; // Zero-columns for Sort
			}
			for (int i = 0; i < _chunkNode.length; i++)
				_chunkNode[i] = vec.chunkKey(i).home_node().index();
		}

		// long min() { return (((long)_msb ) << _shift) + _base[0]-1; } // the first
		// key possible in this bucket
		// long max() { return (((long)_msb+1) << _shift) + _base[0]-2; } // the last
		// key possible in this bucket
		...
		long max() {
			return BigInteger.valueOf(((long) _msb + 1) << _shift)
					.add(_base[0].subtract(BigInteger.ONE).subtract(BigInteger.ONE)).longValue();
		}
	}
	...
	BinaryMerge(FFSB leftSB, FFSB riteSB, boolean allLeft) {
		...
		_leftSB = leftSB;
		_riteSB = riteSB;
		// the number of columns in the key i.e. length of _leftFieldSizes and
		// _riteSB._fieldSizes
	...
	public void compute2() {
		...
		SingleThreadRadixOrder.OXHeader leftSortedOXHeader = DKV
				.getGet(getSortedOXHeaderKey(/* left= */true, _leftSB._msb));
		...
		SingleThreadRadixOrder.OXHeader rightSortedOXHeader = DKV
				.getGet(getSortedOXHeaderKey(/* left= */false, _riteSB._msb));
		// if (_riteSB._msb==-1) assert _allLeft && rightSortedOXHeader == null; // i.e.
		// it's known nothing on right can join
		...
		// if _riteSB._msb==-1 then the values in riteMin and riteMax here are redundant
		// and not used
		...
		_leftFrom = (_riteSB._msb == -1 || leftMin >= riteMin || (_allLeft && _riteSB._msb == 0)) ? -1
				: bsearchLeft(riteMin, /* retLow */true, leftN);
		long leftTo = (_riteSB._msb == -1 || leftMax <= riteMax || (_allLeft && _riteSB._msb == 255)) ? leftN
				: bsearchLeft(riteMax, /* retLow */false, leftN);
		...
		if (retSize == 0) {
			tryComplete();
			return;
		} // nothing can match, even when allLeft
	...
	private static class KeyOrder {
		...
		void initKeyOrder(int msb, boolean isLeft) {
			for (int b = 0; b < _key.length; b++) {
				...
				SplitByMSBLocal.OXbatch ox = v.get(); // mem version (obtained from remote) of the Values gets turned
														// into POJO version
		...
		long numRowsToFetch() {
			return ArrayUtils.sum(_perNodeNumRowsToFetch);
		}

		// Do a mod/div long _order array lookup
		long at8order(long idx) {
			return _order[(int) (idx / _batchSize)][(int) (idx % _batchSize)];
		}
		...
		long[][] fillPerNodeRows(int i) {
			...
			int nbatch = (int) ((_perNodeNumRowsToFetch[i] - 1) / batchSizeLong + 1); // TODO: wrap in class to avoid
																						// this boiler plate
			assert nbatch >= 1;
	...
	private int keycmp(byte xss[][], long xi, byte yss[][], long yi) {
		...
		long diff = xval - yval; // could overflow even in long; e.g. joining to a prevailing NA, or very large
									// gaps O(2^62)
	...
	// Optimized special case for the two calling points; see usages in bmerge_r
	// below.
	private boolean leftKeyEqual(byte x[][], long xi, long yi) {
		...
		int i = 0;
		while (i < _leftSB._keySize && xbatch[xoff++] == ybatch[yoff++])
			i++;
	...
	private void bmerge_r(long lLowIn, long lUppIn, long rLowIn, long rUppIn) {
		...
		while (rLow < rUpp - 1) {
			...
			if (cmp < 0) {
				...
				while (tmpLow < rUpp - 1) {
					...
					if (keycmp(_leftKO._key, lr, _riteKO._key, mid) == 0) {
						tmpLow = mid;
					} else
						rUpp = mid;
				}
		...
		// The left table key may (unusually, and not recommended, but sometimes needed)
		// be duplicated.
		...
		tmpLow = lr + 1;
		// TODO: these while's could be rolled up inside leftKeyEqual saving call
		// overhead
		while (tmpLow < lUpp && leftKeyEqual(_leftKO._key, tmpLow, lr))
			tmpLow++;
		lUpp = tmpLow;
		tmpUpp = lr - 1;
		while (tmpUpp > lLow && leftKeyEqual(_leftKO._key, tmpUpp, lr))
			tmpUpp--;
		lLow = tmpUpp;
		// lLow and lUpp now surround the group in the left table. If left key is unique
		// then lLow==lr-1 and lUpp==lr+1.
		assert lUpp - lLow >= 2;

		// if value found, rLow and rUpp surround it, unlike standard binary search
		// where rLow falls on it
		long len = rUpp - rLow - 1;
		// TODO - we don't need loop here :) Why does perNodeNumRightRowsToFetch
		// increase so much?
		if (len > 0 || _allLeft) {
			long t0 = System.nanoTime();
			if (len > 1) {
				_oneToManyMatch = true;
			}
			_numRowsInResult += Math.max(1, len) * (lUpp - lLow - 1); // 1 for NA row when _allLeft
			for (long j = lLow + 1; j < lUpp; j++) { // usually iterates once only for j=lr, but more than once if there
														// are dup keys in left table
				...
				// the key is the same within this left dup range, but still need to fetch left
				// non-join columns
				_leftKO._perNodeNumRowsToFetch[_leftSB._chunkNode[chkIdx]]++;
				if (len == 0) {
					continue; // _allLeft must be true if len==0
				}
				...
				long outLoc = j - (_leftFrom + 1); // outOffset is 0 here in the standard scaling up high cardinality
													// test
				...
				final int jo2 = (int) (outLoc % _retBatchSize); // TODO - take outside the loop. However when we go
																// deep-msb, this'll go away.
		...
		// TODO: check assumption that retFirst and retLength are initialized to 0, for
		// case of no match
	...
	private void createChunksInDKV() {
		...
		// Cannot use a List<Long> as that's restricted to 2Bn items and also isn't an
		// Iced datatype
		...
		// 16 bytes for each UUID (biggest type). Enum will be long (8). TODO: How is
		// non-Enum 'string' handled by H2O?
		final int batchSizeUUID = 256 * 1024 * 1024 / 16; // number of rows per chunk to fit in 256GB DKV limit.
		final int nbatch = (int) ((_numRowsInResult - 1) / batchSizeUUID + 1); // TODO: wrap in class to avoid this
																				// boiler plate
		assert nbatch >= 1;
		...
		for (int col = 0; col < numColsInResult; col++)
			for (int b = 0; b < nbatch; b++) {
				frameLikeChunks[col][b] = MemoryManager
						.malloc8d(_chunkSizes[b] = (b == nbatch - 1 ? lastSize : batchSizeUUID));
		...
		chunksPopulateRetFirst(numColsInResult, numLeftCols, perNodeLeftLoc, grrrsLeft, perNodeRightLoc, grrrsRite,
				frameLikeChunks);
		...
		for (int jb = 0; jb < _ret1st.length; ++jb) { // jb = j batch
			for (int jo = 0; jo < _ret1st[jb].length; ++jo) { // jo = j offset
				...
				if (f == 0) {
					...
					assert l == 0; // doesn't have to be 0 (could be 1 already if allLeft==true) but currently it
									// should be, so check it
					if (!_allLeft) {
						continue;
						
					}
					// now insert the left row once and NA for the right columns i.e. left outer
					// join
				}

				{ // new scope so 'row' can be declared in the for() loop below and registerized
					// (otherwise 'already defined in this scope' in that scope)
					// Fetch the left rows and mark the contiguous from-ranges each left row should
					// be recycled over
					...
					perNodeLeftRows[ni][(int) (pnl / batchSizeLong)][(int) (pnl % batchSizeLong)] = row; // ask that
																											// node for
																											// global
																											// row
																											// number
				...
				assert l > 0;
				if (prevf == f && prevl == l) {
					continue; // don't re-fetch the same matching rows (cartesian). We'll repeat them locally
				}
								// later.
				prevf = f;
				prevl = l;
				for (int r = 0; r < l; r++) {
					long loc = f + r - 1; // -1 because these are 0-based where 0 means no-match and 1 refers to the
											// first row
					// TODO: could take / and % outside loop in cases where it doesn't span a batch
					// boundary
					...
					perNodeRightRows[ni][(int) (pnl / batchSizeLong)][(int) (pnl % batchSizeLong)] = row; // ask that
																											// node for
																											// global
																											// row
																											// number
	...
	private void chunksGetRawRemoteRows(final long perNodeLeftRows[][][], final long perNodeRightRows[][][],
		...
		for (H2ONode node : H2O.CLOUD._memary) {
			...
			for (int b = 0; b < bUppRite; b++) {
				...
				// Arrays.sort(perNodeRightRows[ni][b]); Simple quick test of fetching in
				// monotonic order. Doesn't seem to help so far.
				grrrsRiteRPC[ni][b] = new RPC<>(node, new GetRawRemoteRows(_riteSB._frame, perNodeRightRows[ni][b]))
						.call();
			}
			for (int b = 0; b < bUppLeft; b++) {
				// Arrays.sort(perNodeLeftRows[ni][b]);
				grrrsLeftRPC[ni][b] = new RPC<>(node, new GetRawRemoteRows(_leftSB._frame, perNodeLeftRows[ni][b]))
						.call();
			}
		}
		for (H2ONode node : H2O.CLOUD._memary) {
			// TODO: just send and wait for first batch on each node and then .get() next
			// batch as needed.
			int ni = node.index();
			final int bUppRite = perNodeRightRows[ni] == null ? 0 : perNodeRightRows[ni].length;
			for (int b = 0; b < bUppRite; b++)
	...
	private void chunksPopulateRetFirst(final int numColsInResult, final int numLeftCols, final long perNodeLeftLoc[],
			final GetRawRemoteRows grrrsLeft[][], final long perNodeRightLoc[], final GetRawRemoteRows grrrsRite[][],
		...
		for (int jb = 0; jb < _ret1st.length; ++jb) { // jb = j batch
			for (int jo = 0; jo < _ret1st[jb].length; ++jo) { // jo = j offset
				...
				long l = _retLen[jb][jo];
				if (f == 0 && !_allLeft)
					continue; // f==0 => left row matches to no right row
				// else insert the left row once and NA for the right columns i.e. left outer
				// join

				// Fetch the left rows and recycle it if more than 1 row in the right table is
				// matched to.
				// TODO could loop through batches rather than / and % wastefully
				long row = _leftKO.at8order(leftLoc);
				// TODO should leftOrder and retFirst/retLen have the same batch size to make
				// this easier?
				// TODO Can we not just loop through _leftKO._order only? Why jb and jo too
				// through
				int chkIdx = _leftSB._vec.elem2ChunkIdx(row); // binary search in espc
				int ni = _leftSB._chunkNode[chkIdx];
				...
				if (f == 0) {
					resultLoc++;
					continue;
				} // no match so just one row (NA for right table) to advance over
				assert l > 0;
				if (prevf == f && prevl == l) {
					...
					for (int r = 0; r < l; r++) {
						...
						for (int col = 0; col < numColsInResult - numLeftCols; col++) {
							frameLikeChunks[numLeftCols + col][toChunk][toOffset] = frameLikeChunks[numLeftCols
									+ col][fromChunk][fromOffset];
						}
						resultLoc++;
					}
					continue;
				...
				for (int r = 0; r < l; r++) {
					...
					pnl = perNodeRightLoc[ni]++; // pnl = per node location. // TODO Split to an if() and batch and
													// offset separately
	...
	// compress all chunks and store them
	private void chunksCompressAndStore(final int nbatch, final int numColsInResult,
			final double[][][] frameLikeChunks) {
		// compress all chunks and store them
	...
	static Key getKeyForMSBComboPerCol(/* Frame leftFrame, Frame rightFrame, */ int leftMSB, int rightMSB,
			int col /* final table */, int batch) {
		...
		// + rightFrame._key.toString() + "_joined_with" + leftFrame._key.toString()
				+ "_leftSB._msb" + leftMSB + "_riteSB._msb" + rightMSB, (byte) 1, Key.HIDDEN_USER_KEY, false,
				SplitByMSBLocal.ownerOfMSB(rightMSB == -1 ? leftMSB : rightMSB)); // TODO home locally
	}

	static class GetRawRemoteRows extends DTask<GetRawRemoteRows> {
		Frame _fr;
		long[/* rows */] _rows; // which rows to fetch from remote node, non-null on the way to remote, null on
								// the way back
		...
		public void compute2() {
			...
			assert (_chk == null);
			long t0 = System.nanoTime();
			// System.out.print("Allocating _chk with " + _fr.numCols() +" by " +
			// _rows.length + "...");
			...
			for (int row = 0; row < _rows.length; row++) {
				cidx[row] = anyVec.elem2ChunkIdx(_rows[row]); // binary search of espc array. TODO: sort input row
																// numbers to avoid
			...
			// perNodeRows[node] has perNodeRows[node].length batches of row numbers to
			// fetch
			_rows = null;
			_fr = null;
			assert (_chk != null);
