...
class RadixOrder extends H2O.H2OCountedCompleter<RadixOrder> {
	...
	public void compute2() {
		...
		System.out
				.println("Time to use rollup stats to determine biggestBit: " + ((t1 = System.nanoTime()) - t0) / 1e9);
		t0 = t1;

		if (_whichCols.length > 0) {
			new RadixCount(_isLeft, _base[0], _shift[0], _whichCols[0], _isLeft ? _id_maps : null)
					.doAll(_DF.vec(_whichCols[0]));
			System.out.println("Time of MSB count MRTask left local on each node (no reduce): "
					+ ((t1 = System.nanoTime()) - t0) / 1e9);
			t0 = t1;
		}

		// NOT TO DO: we do need the full allocation of x[] and o[]. We need o[] anyway.
		// x[] will be compressed and dense.
		// o is the full ordering vector of the right size
		// x is the byte key aligned with o
		// o AND x are what bmerge() needs. Pushing x to each node as well as o avoids
		// inter-node comms.
		...
		// TODO: fix closeLocal() blocking issue and revert to simpler usage of
		// closeLocal()
		...
		if (_whichCols.length > 0) {
			new SplitByMSBLocal(_isLeft, _base, _shift[0], keySize, batchSize, _bytesUsed, _whichCols, linkTwoMRTask,
					_id_maps).doAll(_DF.vecs(_whichCols)); // postLocal needs DKV.put()
			System.out.println("SplitByMSBLocal MRTask (all local per node, no network) took : "
					+ ((t1 = System.nanoTime()) - t0) / 1e9);
			t0 = t1;
		}
		...
		for (int i = 0; i < 256; i++)
			radixOrders[i] = new RPC<>(SplitByMSBLocal.ownerOfMSB(i),
					new SingleThreadRadixOrder(_DF, _isLeft, batchSize, keySize, /* nGroup, */ i)).call();
		...
		// SingleThreadRadixOrder radixOrder = new RPC<>(node, new
		// SingleThreadRadixOrder(DF, batchSize, keySize, nGroup, i)).call().get();
		// _o[i] = radixOrder._o;
		// _x[i] = radixOrder._x;
		// }
	...
	private long computeShift(final BigInteger max, final int i) {
		...
		if (_base[i].mod(msbWidth).compareTo(BigInteger.ZERO) != 0) {
			_base[i] = _isInt[i]
					? msbWidth.multiply(_base[i].divide(msbWidth)
							.add(_base[i].signum() < 0 ? BigInteger.valueOf(-1L) : BigInteger.ZERO))
					: msbWidth.multiply(_base[i].divide(msbWidth));
			; // dealing with unsigned integer here
