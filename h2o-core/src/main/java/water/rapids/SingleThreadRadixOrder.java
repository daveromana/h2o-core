...
class SingleThreadRadixOrder extends DTask<SingleThreadRadixOrder> {
	...
	private final int _MSBvalue; // only needed to be able to return the number of groups back to the caller
									// RadixOrder
	...
	public void compute2() {
		...
		for (int n = 0; n < H2O.CLOUD.size(); n++) {
			// Log.info("Getting MSB " + MSBvalue + " Node Header from node " + n + "/" +
			// H2O.CLOUD.size() + " for Frame " + _fr._key);
			// Log.info("Getting");
			...
			DKV.remove(k);
			numRows += ArrayUtils.sum(MSBnodeHeader[n]._MSBnodeChunkCounts); // This numRows is split into nbatch
																				// batches on that node.
			// This header has the counts of each chunk (the ordered chunk numbers on that
			// node)
		}
		if (numRows == 0) {
			tryComplete();
		...
		int oxBatchNum[/* node */] = new int[H2O.CLOUD.size()]; // which batch of OX are we on from that node?
																// Initialized to 0.
		...
		for (int c = 0; c < vec.nChunks(); c++) {
			...
			// See long comment at the top of SendSplitMSB. One line from there repeated
			// here :
			...
			// at most batchSize remaining. No need to actually put the number of rows left
			// in here
			int sourceBatchRemaining = _batchSize - oxOffset[fromNode];
			while (numRowsToCopy > 0) { // No need for class now, as this is a bit different to the other batch copier.
										// Two isn't too bad.
				...
				System.arraycopy(ox[fromNode]._x, oxOffset[fromNode] * _keySize, _x[targetBatch],
						targetOffset * _keySize, thisCopy * _keySize);
				...
				if (sourceBatchRemaining == 0) {
					...
					DKV.remove(k);
					if (ox[fromNode] == null) {
						// if the last chunksworth fills a batchsize exactly, the getGet above will have
						// returned null.
						// TODO: Check will Cliff that a known fetch of a non-existent key is ok e.g.
						// won't cause a delay/block? If ok, leave as good check.
						int numNonZero = 0;
		...
		// We now have _o and _x collated from all the contributing nodes, in the
		// correct original order.
		// TODO save this allocation and reuse per thread? Or will heap just take care
		// of it. Time this allocation and copy as step 1 anyway.
		_xtmp = new byte[_x.length][];
		_otmp = new long[_o.length][];
		assert _x.length == _o.length; // i.e. aligned batch size between x and o (think 20 bytes keys and 8 bytes of
										// long in o)
		...
		// Just create enough for the 4 threads active at any one time. Not 256
		// allocations and releases.
		// We need o[] and x[] in full for the result. But this way we don't need full
		// size xtmp[] and otmp[] at any single time.
		// Currently Java will allocate and free these xtmp and otmp and maybe it does
		// good enough job reusing heap that we don't need to explicitly optimize this
		// reuse.
		// Perhaps iterating this task through the largest bins first will help java
		// reuse heap.
		assert (_o != null);
		assert (numRows > 0);
	...
	static Key getSortedOXHeaderKey(boolean isLeft, int MSBvalue) {
		...
		return Key.make("__radix_order__SortedOXHeader_MSB" + MSBvalue + (isLeft ? "_LEFT" : "_RIGHT")); // If we don't
																											// say this
																											// it's
																											// random
																											// ...
	...
	public void insert(long start, /* only for small len so len can be type int */int len) {
		...
		if (batch1 != batch0) {
			...
			// copy two halves to contiguous temp memory, do the below, then split it back
			// to the two halves afterwards.
			// Straddles batches very rarely (at most once per batch) so no speed impact at
			// all.
			...
			_xbatch = _x[batch0]; // taking this outside the loop does indeed make quite a big different (hotspot
									// isn't catching this, then)
			_obatch = _o[batch0];
		}
		int offset = (int) (start % _batchSize);
		for (int i = 1; i < len; i++) {
			int cmp = keycmp(_xbatch, offset + i, _xbatch, offset + i - 1); // TO DO: we don't need to compare the whole
																			// key here. Set cmpLen < keySize
			if (cmp < 0) {
	...
	public void run(final long start, final long len, final int Byte) {
		...
		// thisHist reused and carefully set back to 0 below so we don't need to clear
		// it now
		...
		for (int b = 0; b < nbatch; b++) {
			// taking this outside the loop below does indeed make quite a big different
			// (hotspot isn't catching this, then)
			...
			idx = _keySize - Byte - 1;
			thisLen = (b == nbatch - 2/* next iteration will be last batch */ ? (int) ((start + len) % _batchSize)
					: _batchSize);
		...
		// Sigh. Now deal with batches here as well because Java doesn't have 64bit
		// indexing.
		...
		for (int b = 0; b < nbatch; b++) {
			...
			for (int i = 0; i < thisLen; i++) {
				...
				// now always write to the beginning of _otmp and _xtmp just to reuse the first
				// hot pages
				_otmp[(int) (target / _batchSize)][(int) (target % _batchSize)] = _obatch[oidx + i]; // this must be
																										// kept in 8
																										// bytes longs
				System.arraycopy(_xbatch, (oidx + i) * _keySize, _xtmp[(int) (target / _batchSize)],
						(int) (target % _batchSize) * _keySize, _keySize);
				xidx += _keySize;
		...
		// now copy _otmp and _xtmp back over _o and _x from the start position,
	...
	private static void runCopy(final long start, final long len, final int keySize, final int batchSize,
			final long otmp[][], final byte xtmp[][], final long o[][], final byte x[][]) {
		// now copy _otmp and _xtmp back over _o and _x from the start position,
		// allowing for boundaries
		// _o, _x, _otmp and _xtmp all have the same _batchsize
		...
		int targetBatchRemaining = batchSize - targetOffset; // 'remaining' means of the the full batch, not of the
																// numRowsToCopy
		int sourceBatchRemaining = batchSize - sourceOffset; // at most batchSize remaining. No need to actually put the
																// number of rows left in here
		while (numRowsToCopy > 0) { // TO DO: put this into class as well, to ArrayCopy into batched
			...
			System.arraycopy(xtmp[sourceBatch], sourceOffset * keySize, x[targetBatch], targetOffset * keySize,
					thisCopy * keySize);
			numRowsToCopy -= thisCopy;
			...
			// 'source' and 'target' deliberately the same length variable names and long
			// lines deliberately used so we
