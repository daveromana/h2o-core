...
public class AstHist extends AstPrimitive {
	...
	public ValFrame apply(Env env, Env.StackHelp stk, AstRoot asts[]) {
		...
		double x0 = vec.min();
		if (breaks != null) {
			t = new AstHist.HistTask(breaks, -1, -1/* ignored if _h==-1 */).doAll(vec);
		} else if (algo != null) {
			switch (algo) {
			case "sturges":
		...
		for (int i = 1; i < brks.length; ++i)
			mids[i - 1] = .5 * (t._breaks[i - 1] + t._breaks[i]);
		...
		}.doAll(4, Vec.T_NUM, new Frame(layoutVec)).outputFrame(null,
				new String[] { "breaks", "counts", "mids_true", "mids" }, null);
		layoutVec.remove();
	...
	public double[] computeCuts(Vec v, int numBreaks) {
		...
		// just make numBreaks cuts equidistant from each other spanning range of
		// [v.min, v.max]
		double min;
	...
	public static class HistTask extends MRTask<AstHist.HistTask> {
		...
		@Override
		public void map(Chunk c) {
			// if _h==-1, then don't have fixed bin widths... must loop over bins to obtain
			// the correct bin #
			for (int i = 0; i < c._len; ++i) {
				int x = 1;
				if (c.isNA(i)) {
				...
				if (_h == -1) {
					...
					x = Math.min(_counts.length - 1, (int) Math.floor((r - _x0) / _h)); // Pick the bin floor( (x - x0)
																						// / h ) or ceil( (x-x0)/h - 1
																						// ), choose the first since
																						// fewer ops
				bumpCount(x);
