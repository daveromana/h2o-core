...
/**
 * Accepts a frame with a single string column. Returns a new string column
 * containing the rstripped versions of the strings in the target column.
 * Stripping removes all characters in the strings for the target columns that
 * match the user provided set
 */
	...
	public ValFrame apply(Env env, Env.StackHelp stk, AstRoot asts[]) {
		...
		for (Vec v : fr.vecs())
			if (!(v.isCategorical() || v.isString())) {
				throw new IllegalArgumentException("trim() requires a string or categorical column. " + "Received "
						+ fr.anyVec().get_type_str() + ". Please convert column to a string or categorical first.");
			}
			...
			public void map(Chunk chk, NewChunk newChk) {
				if (chk instanceof C0DChunk) { // all NAs
					for (int i = 0; i < chk.len(); i++)
						newChk.addNA();
				} else if (((CStrChunk) chk)._isAllASCII && StringUtils.isAsciiPrintable(charSet)) { // fast-path
																										// operations
