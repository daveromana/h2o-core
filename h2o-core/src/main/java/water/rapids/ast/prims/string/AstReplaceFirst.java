...
/**
 * Accepts a frame with a single string column, a regex pattern string, a
 * replacement substring, and a boolean to indicate whether to ignore the case
 * of the target string. Returns a new string column containing the results of
 * the replaceFirst method on each string in the target column.
 * <p/>
 * replaceAll - Replaces the first substring of this string that matches the
...
public class AstReplaceFirst extends AstPrimitive {
	...
	private Vec replaceFirstCategoricalCol(Vec vec, String pattern, String replacement, boolean ignoreCase) {
		...
		for (int i = 0; i < doms.length; ++i)
			doms[i] = ignoreCase ? doms[i].toLowerCase(Locale.ENGLISH).replaceFirst(pattern, replacement)
