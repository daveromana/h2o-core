...
/**
 * Convert year, month, day, hour, minute, sec, msec to Unix epoch time (in
 * milliseconds).
 *
...
public class AstMoment extends AstBuiltin<AstMoment> {
			...
			if (vi.isFrame()) {
				...
				if (fr.numCols() != 1) {
					throw new IllegalArgumentException(
							"Argument " + i + " is a frame with " + fr.numCols() + " columns");
				}
				...
				if (fr.numRows() == 1) {
					...
					if (fr.numRows() != numRows) {
						throw new IllegalArgumentException("Incompatible vec " + i + " having " + fr.numRows()
								+ " rows, whereas " + "other vecs have " + numRows + " rows.");
					}
	...
	private static class SetTimeTask extends MRTask<SetTimeTask> {
		...
		/**
		 * @param timeparts
		 *            is the array of [year, month, day, hrs, mins, secs, ms] for all
		 *            constant parts of the date;
		 * @param chunksmap
		 *            is a mapping between chunks indices and the timeparts array. For
		 *            example, if {@code chunksmap = [1, 2]}, then the first chunk
		 *            describes the "month" part of the date, and the second chunk the
		 *            "day" part.
		 */
		...
		public void map(Chunk[] chks, NewChunk nc) {
			...
			while (byrow) {
				for (int i = 0; i < nChunkRows; i++) {
					...
					try {
						double millis = chronology.getDateTimeMillis(tpl[0], tpl[1], tpl[2], tpl[3], tpl[4], tpl[5],
								tpl[6]);
						nc.addNum(millis);
