...
/**
 * The iSAX algorithm is a time series indexing strategy that reduces the
 * dimensionality of a time series along the time axis. For example, if a time
 * series had 1000 unique values with data across 500 rows, reduce this data set
 * to a time series that uses 100 unique values, across 10 buckets along the
 * time span.
 *
 * References: http://www.cs.ucr.edu/~eamonn/SAX.pdf
 * http://www.cs.ucr.edu/~eamonn/iSAX_2.0.pdf
 *
 * Note: This approach assumes the frame has the form of TS-i x T where TS-i is
 * a single time series and T is time:
 *
 * T-1, T-2, T-3, T-4, ... , T-N TS-1 ... TS-2 ... TS-3 ... . . . TS-N ...
 *
...
public class AstIsax extends AstPrimitive {
	...
	public int nargs() {
		return 1 + 4;
	} // (ary isax numWords maxCardinality optimize_card)
	...
	public Val apply(Env env, Env.StackHelp stk, AstRoot asts[]) {
		...
		Frame fr2 = new AstIsax.IsaxTask(numWords, maxCardinality).doAll(numWords, Vec.T_NUM, fr).outputFrame(null,
				columns.toArray(new String[numWords]), null);

		int[] maxCards = new int[numWords];

		if (optm_card) {
			_domain_hm = new double[numWords][maxCardinality];
			for (double[] r : _domain_hm)
				Arrays.fill(r, Double.NaN);

			// see if we can reduce the cardinality by checking all unique tokens in all
			// series in a word
			...
			Frame fr3 = new AstIsax.IsaxStringTask(maxCards).doAll(1, Vec.T_STR, fr2_reduced).outputFrame(null,
					new String[] { "iSax_index" }, null);
		...
		Frame fr3 = new AstIsax.IsaxStringTask(maxCards).doAll(1, Vec.T_STR, fr2).outputFrame(null,
				new String[] { "iSax_index" }, null);
		...
		public void map(Chunk cs[], NewChunk[] nc) {
			...
			for (int i = 0; i < cs.length; i += step) {
				...
				for (int j = 0; j < chunkSize; j++) {
					double mySum = 0.0;
					double myCount = 0.0;
					// Loop through all the data in the chunk for the given series in the given
					// subset (word)
