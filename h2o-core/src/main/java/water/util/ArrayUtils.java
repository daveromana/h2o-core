...
public class ArrayUtils {
	...
	// return the sqrt of each element of the array. Will overwrite the original
	// array in this case
	...
	public static float[] div(float[] nums, float n) {
		assert !Float.isInfinite(n) : "Trying to divide " + Arrays.toString(nums) + " by  " + n; // Almost surely not
																									// what you want
		for (int i = 0; i < nums.length; i++)
			nums[i] /= n;
		return nums;
	...
	public static double[] div(double[] nums, double n) {
		assert !Double.isInfinite(n) : "Trying to divide " + Arrays.toString(nums) + " by  " + n; // Almost surely not
																									// what you want
		for (int i = 0; i < nums.length; i++)
			nums[i] /= n;
		return nums;
	...
	public static float[] mult(float[] nums, float n) {
		// assert !Float.isInfinite(n) : "Trying to multiply " + Arrays.toString(nums) +
		// " by " + n; // Almost surely not what you want
		for (int i = 0; i < nums.length; i++)
			nums[i] *= n;
		return nums;
	}
	...
	public static double[] mult(double[] nums, double n) {
		// assert !Double.isInfinite(n) : "Trying to multiply " + Arrays.toString(nums)
		// + " by " + n; // Almost surely not what you want
		for (int i = 0; i < nums.length; i++)
			nums[i] *= n;
		return nums;
	...
	public static double[] multArrVec(double[][] ary, double[] nums, double[] res) {
		...
		assert ary[0].length == nums.length : "Inner dimensions must match: Got " + ary[0].length + " != "
				+ nums.length;
		for (int i = 0; i < ary.length; i++)
	...
	/*
	 * with no memory allocation for results. We assume the memory is already
	 * allocated.
	 */
	public static double[][] multArrArr(double[][] ary1, double[][] ary2, double[][] res) {
		...
		assert ary1[0].length == ary2.length : "Inner dimensions must match: Got " + ary1[0].length + " != "
				+ ary2.length;
	...
	public static <T> T[] cloneOrNull(T[] ary) {
		return ary == null ? null : ary.clone();
	}
	 ...
	 * @param transpose
	 *            If true, compute n by n Gram of rows = XX' If false, compute k by
	 *            k Gram of cols = X'X
	...
	public static double[] permute(double[] vec, int[] idx) {
		...
		assert vec.length == idx.length : "Length of vector must match permutation vector length: Got " + vec.length
				+ " != " + idx.length;
	...
	public static double[][] permuteRows(double[][] ary, int[] idx) {
		...
		assert ary.length == idx.length : "Number of rows must match permutation vector length: Got " + ary.length
				+ " != " + idx.length;
		double[][] res = new double[ary.length][ary[0].length];
		for (int i = 0; i < ary.length; i++)
	...
	/**
	 * Returns the index of the largest value in the array. In case of a tie, an the
	...
	// behaves like Arrays.binarySearch, but is slower -> Just good for tiny arrays
	// (length<20)
	 ...
	 * @param result
	 *            array to store the results into (will be of size n)
	 * @param seed
	 *            random number seed
	 * @param startIndex
	 *            offset into a
	...
	// Generate a n by m array of random numbers drawn from the standard normal
	// distribution
	public static double[][] gaussianArray(int n, int m) {
		return gaussianArray(n, m, System.currentTimeMillis());
	}
	...
	public static double[] gaussianVector(int n) {
		return gaussianVector(n, System.currentTimeMillis());
	}

	public static double[] gaussianVector(int n, long seed) {
		return gaussianVector(n, getRNG(seed));
	}
	 ...
	 *
	 * For union of numeric arrays (strings represent integers) it is expecting
	 * numeric ordering. For pure string domains it is expecting lexicographical
	 * ordering. For mixed domains it always expects lexicographical ordering since
	 * such a domain were produce by a parser which sort string with Array.sort().
	 *
	 * PRECONDITION - string domain was sorted by Array.sort(String[]), integer
	 * domain by Array.sort(int[]) and switched to Strings !!!
	 *
	 * @param a
	 *            a set of strings
	 * @param b
	 *            a set of strings
	 * @return union of arrays // TODO: add tests
	 */
	 ...
	 *
	 * The method expects ordering of domains in given order (lexicographical,
	 * numeric)
	 *
	 * @param a
	 *            first array
	 ...
	 *            second array
	 * @param lexo
	 *            - true if domains are sorted in lexicographical order or false for
	 *            numeric domains
	 * @return union of values in given arrays.
	 *
	 *         precondition lexo ? a,b are lexicographically sorted : a,b are sorted
	 *         numerically precondition a!=null &amp;&amp; b!=null
	 */
	...
	// warning: Non-Symmetric! Returns all elements in a that are not in b (but NOT
	...
	/**
	 * Sort an integer array of indices based on values Updates indices in place,
	 ...
	 * @param <T>
	 *            any type
	 * @return flattened array, if input was null return null, if input was empty
	 *         return null
	 */
	...
	/*
	 * This class is written to a 2-D array to the frame instead of allocating new
	 * memory every time.
	 */
	 ...
	 * @return new frame which contains columns named according given names and
	 *         including given data
	 */
	...
	public static Frame frame(String[] names, double[]... rows) {
		return frame(Key.<Frame>make(), names, rows);
	}
	...
	public static int encodeAsInt(byte[] bs, int at) {
		if (at + 4 > bs.length) {
			throw new IndexOutOfBoundsException(
					"Cannot encode more than 4 bytes into int: len = " + bs.length + ", pos=" + at);
		}
	...
	/**
	 * Transform given long numbers into byte array. Highest 8-bits of the first
	 * long will stored in the first field of returned byte array.
	 *
	 * Example: 0xff18000000000000L -> new byte[] { 0xff, 0x18, 0, 0, 0, 0, 0, 0}
	 */
	...
	/**
	 * Convert an array of primitive types into an array of corresponding boxed
	 * types. Due to quirks of Java language this cannot be done in any generic way
	 * -- there should be a separate function for each use case...
	 * 
	 * @param arr
	 *            input array of `char`s
