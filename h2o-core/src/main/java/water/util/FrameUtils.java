...
public class FrameUtils {
	...
	/**
	 * Parse given file(s) into the form of single frame represented by the given
	 * key.
	 *
	 * @param okey
	 *            destination key for parsed frame
	 * @param files
	 *            files to parse
	 * @return a new frame
	 */
	public static Frame parseFrame(Key okey, File... files) throws IOException {
		...
		for (int i = 0; i < files.length; i++)
			inKeys[i] = NFSFileVec.make(files[i])._key;
	 ...
	 *
	 * @param okey
	 *            key for ouput frame. Can be null
	 * @param uris
	 *            array of URI (file://, hdfs://, s3n://, s3a://, s3://, ...) to
	 *            parse
	...
	public static Frame categoricalEncoder(Frame dataset, String[] skipCols,
			Model.Parameters.CategoricalEncodingScheme scheme, ToEigenVec tev, int maxLevels) {
		switch (scheme) {
		case AUTO:
		case Enum:
		case SortByResponse: // the work is done in ModelBuilder - the domain is all we need to change once,
								// adaptTestTrain takes care of test set adaptation
		case OneHotInternal:
	...
	/**
	 * Compute a chunk summary (how many chunks of each type, relative size, total
	 * size)
	 * 
	 * @param fr
	...
	public static Key[] generateNumKeys(Key mk, int num) {
		return generateNumKeys(mk, num, "_part");
	}
	...
	public static class ExportTaskDriver extends H2O.H2OCountedCompleter<ExportTaskDriver> {
		...
		public void compute2() {
			...
			if (_nParts == 1) {
				// Single file export, the file should be created by the node that was asked to
				// export the data
				// (this is for non-distributed filesystems, we want the file to go to the local
		...
		private int calculateNParts() {
			...
			// the goal is to not to create too small part files (and too many files), ideal
			// part file size is one HDFS block
		...
		/**
		 * Trivial CSV file size estimator. Uses the first line of each non-empty chunk
		 * to estimate the size of the chunk. The total estimated size is the total of
		 * the estimated chunk sizes.
		 */
		...
		private long copyCSVStream(Frame.CSVStream is, OutputStream os, int firstChkIdx, int buffer_size)
				throws IOException {
			long len = 0;
	...
	/**
	 * Helper to convert a categorical variable into a "binary" encoding format. In
	 * this format each categorical value is first assigned an integer value, then
	 * that integer is written in binary, and each bit column is converted into a
	 * separate column. This is intended as an improvement to an existing one-hot
	 * transformation. For each categorical variable we assume that the number of
	 * categories is 1 + domain cardinality, the extra category is reserved for NAs.
	 * See
			...
			public void compute2() {
				...
				for (int i = 0; i < frameVecs.length; ++i)
					if (frameVecs[i].isCategorical()
							&& (_skipCols == null || ArrayUtils.find(_skipCols, _frame._names[i]) == -1)) {
						numCategoricals++;
					}
				...
				Frame binaryCols = mrtask.doAll(numOutputColumns, Vec.T_NUM, categoricalFrame).outputFrame();
				// change names of binaryCols so that they reflect the original names of the
				// categories
	...
	/**
	 * Helper to convert a categorical variable into the first eigenvector of the
	 * dummy-expanded matrix.
	 */
			...
			public void compute2() {
				...
				for (int i = 0; i < frameVecs.length; ++i) {
					...
					if (src.cardinality() > _maxLevels) {
						...
						DKV.put(train);
						Log.info("Reducing the cardinality of a categorical column with " + src.cardinality()
	...
	static public void shrinkDomainsToObservedSubset(Frame frameToModifyInPlace) {
		for (Vec v : frameToModifyInPlace.vecs()) {
			if (v.isCategorical()) {
				long[] uniques = (v.min() >= 0 && v.max() < Integer.MAX_VALUE - 4)
						? new VecUtils.CollectDomainFast((int) v.max()).doAll(v).domain()
