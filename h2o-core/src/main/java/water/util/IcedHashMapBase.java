...
public abstract class IcedHashMapBase<K, V> extends Iced implements Map<K, V>, Cloneable, Serializable {
		...
		try {
			...
			if (key instanceof String) {
				if (val instanceof String) {
					...
					assert (val instanceof Freezable || val instanceof Freezable[]) : "incompatible class "
							+ val.getClass();
	 ...
	 * 
	 * @param ab
	 *            Contains the serialized K-V pairs
	 */
	public final IcedHashMapBase read_impl(AutoBuffer ab) {
		try {
			...
			if (null == t.getCause()) {
				...
				throw H2O.fail("IcedHashMap deserialization failed! + " + t.toString() + ", msg = " + t.getMessage()
						+ ", cause: " + t.getCause().toString() + ", cause msg: " + t.getCause().getMessage()
	...
	public final AutoBuffer writeJSON_impl(AutoBuffer ab) {
		...
		for (Entry<K, V> entry : map().entrySet()) {
			...
			assert value instanceof String || value instanceof String[] || value instanceof Integer
					|| value instanceof Freezable || value instanceof Freezable[];
