 ...
 * message later, and report the number of dropped messages, but this done in a
 * best effort and lossy manner. Basically when an OOME occurs during logging,
...
abstract public class Log {
	...
	public static void info(String s, boolean stdout) {
		if (_level >= INFO) {
			write0(INFO, stdout, new String[] { s });
		}
	}
	...
	private static String header(int lvl) {
		...
		String s = nowString + " " + _preHeader + " " + fixedLength(Thread.currentThread().getName() + " ", 10)
				+ LVLS[lvl] + ": ";
		return s;
	}
	...
	private static void setLog4jProperties(String logDir, java.util.Properties p) throws Exception {
		...
		// H2O-wide logging
		String appenders = new String[] { "TRACE, R6", "TRACE, R5, R6", "TRACE, R4, R5, R6", "TRACE, R3, R4, R5, R6",
				"TRACE, R2, R3, R4, R5, R6", "TRACE, R1, R2, R3, R4, R5, R6", }[_level];
	...
	private static synchronized org.apache.log4j.Logger createLog4j() {
		...
		if (log4jConfiguration != null) {
			...
			// Create some default properties on the fly if we aren't using a provided
			// configuration.
			...
			try {
				...
				if (H2O.ARGS.log_dir != null) {
					...
					} else if (H2O.ICE_ROOT.getScheme() == null
							|| PersistManager.Schemes.FILE.equals(H2O.ICE_ROOT.getScheme())) {
			...
			// For the Hadoop case, force H2O to specify the logging setup since we don't
			// care
			...
			if (!launchedWithHadoopJar && H2O.haveInheritedLog4jConfiguration()) {
				// Use a modified log4j property configurator to append rather than create a new
				// log4j configuration.
	...
	/**
	 * POST stands for "Power on self test". Stamp a POST code to /tmp. This is for
	 * bringup, when no logging or stdout I/O is reliable. (Especially when
	 * embedded, such as in hadoop mapreduce, for example.)
	 *
	 * @param n
	 *            POST code.
	 * @param s
