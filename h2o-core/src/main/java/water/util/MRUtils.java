...
public class MRUtils {
  ...
  public static Frame sampleFrame(Frame fr, final long rows, final long seed) {
	...
	/**
	 * Compute the class distribution from a class label vector (not counting
	 * missing values)
	 *
	 * Usage 1: Label vector is categorical ------------------------------------ Vec
	 * label = ...; assert(label.isCategorical()); double[] dist = new
	 * ClassDist(label).doAll(label).dist();
	 *
	 * Usage 2: Label vector is numerical ---------------------------------- Vec
	 * label = ...; int num_classes = ...; assert(label.isInt()); double[] dist =
	 * new ClassDist(num_classes).doAll(label).dist();
	 *
	 */
	...
	public static class Dist extends MRTask<Dist> {
		...
		public double[] dist() {
			...
			for (IcedAtomicInt v : _dist.values())
				dist[i++] = v.get();
			return dist;
		}
	...
	/**
	 * Stratified sampling for classifiers - FIXME: For weights, this is not
	 * accurate, as the sampling is done with uniform weights
	 * 
	 * @param fr
	 *            Input frame
	 * @param label
	 *            Label vector (must be categorical)
	 * @param weights
	 *            Weights vector, can be null
	 * @param sampling_ratios
	 *            Optional: array containing the requested sampling ratios per class
	 *            (in order of domains), will be overwritten if it contains all 0s
	 * @param maxrows
	 *            Maximum number of rows in the returned frame
	 * @param seed
	 *            RNG seed for sampling
	 * @param allowOversampling
	 *            Allow oversampling of minority classes
	 * @param verbose
	 *            Whether to print verbose info
	 * @return Sampled frame, with approximately the same number of samples from
	 *         each class (or given by the requested sampling ratios)
	 */
	public static Frame sampleFrameStratified(final Frame fr, Vec label, Vec weights, float[] sampling_ratios,
			long maxrows, final long seed, final boolean allowOversampling, final boolean verbose) {
		if (fr == null) {
			return null;
		}
		...
		if (maxrows < label.domain().length) {
			Log.warn(
					"Attempting to do stratified sampling to fewer samples than there are class labels - automatically increasing to #rows == #labels ("
							+ label.domain().length + ").");
		...
		Log.info("Doing stratified sampling for data set containing " + fr.numRows() + " rows from " + dist.length
				+ " classes. Oversampling: " + (allowOversampling ? "on" : "off"));
		if (verbose) {
			for (int i = 0; i < dist.length; ++i) {
				Log.info("Class " + label.factor(i) + ": count: " + dist[i] + " prior: "
		...
		if (ArrayUtils.minValue(sampling_ratios) == 0 && ArrayUtils.maxValue(sampling_ratios) == 0) {
			...
			for (int i = 0; i < dist.length; ++i) {
				sampling_ratios[i] = ((float) fr.numRows() / label.domain().length) / (float) dist[i]; // prior^-1 /
																										// num_classes
			}
			final float inv_scale = ArrayUtils.minValue(sampling_ratios); // majority class has lowest required
																			// oversampling factor to achieve balance
			if (!Float.isNaN(inv_scale) && !Float.isInfinite(inv_scale)) {
				ArrayUtils.div(sampling_ratios, inv_scale); // want sampling_ratio 1.0 for majority class (no
															// downsampling)
			}
		}
		...
		// given these sampling ratios, and the original class distribution, this is the
		// expected number of resulting rows
		float numrows = 0;
		for (int i = 0; i < sampling_ratios.length; ++i) {
		...
		assert (actualnumrows >= 0); // can have no matching rows in case of sparse data where we had to fill in a
										// makeZero() vector
		Log.info("Stratified sampling to a total of " + String.format("%,d", actualnumrows) + " rows"
				+ (actualnumrows < numrows ? " (limited by max_after_balance_size)." : "."));

		if (actualnumrows != numrows) {
			ArrayUtils.mult(sampling_ratios, (float) actualnumrows / numrows); // adjust the sampling_ratios by the
	 ...
	 *            Input frame
	 * @param label
	 *            Label vector (from the input frame)
	 * @param weights
	 *            Weight vector (from the input frame), can be null
	 * @param sampling_ratios
	 *            Given sampling ratios for each class, in order of domains
	 * @param seed
	 *            RNG seed
	 * @param debug
	 ...
	 */
	public static Frame sampleFrameStratified(final Frame fr, Vec label, Vec weights, final float[] sampling_ratios,
			final long seed, final boolean debug) {
		return sampleFrameStratified(fr, label, weights, sampling_ratios, seed, debug, 0);
	}

	// internal version with repeat counter
	// currently hardcoded to do up to 10 tries to get a row from each class, which
	// can be impossible for certain wrong sampling ratios
	private static Frame sampleFrameStratified(final Frame fr, Vec label, Vec weights, final float[] sampling_ratios,
			final long seed, final boolean debug, int count) {
		if (fr == null) {
			return null;
		...
		// if there are no training labels in the test set, then there is no point in
		...
		if (debug) {
			...
			for (int i = 0; i < dist.length; ++i) {
				Log.info("Class " + r.vecs()[labelidx].factor(i) + ": count: " + dist[i] + " sampling ratio: "
						+ sampling_ratios[i] + " actual relative frequency: "
