...
public class ReflectionUtils {
	/**
	 * Reflection helper which returns the actual class for a type parameter, even
	 * if itself is parameterized.
	 */
	public static <T> Class<T> findActualClassParameter(Class clz, int parm) {
		...
		if (clz.getGenericSuperclass() instanceof ParameterizedType) {
			...
			if (handler_type_parms[parm] instanceof Class) {
				...
				parm_class = (Class) handler_type_parms[parm]; // E.g., for a Schema [0] is the impl (Iced) type; [1] is
																// the Schema type
			} else if (handler_type_parms[parm] instanceof TypeVariable) {
				// The handler's Iced class is parameterized, e.g. to handle multiple layers of
				// Schema classes as in ModelsHandler:
				...
				// The handler's Iced class is parameterized, e.g. to handle multiple layers of
				// Schema classes as in ModelsHandler:
				parm_class = (Class) ((ParameterizedType) (handler_type_parms[parm])).getRawType(); // For a Key<Frame>
																									// this returns
																									// Key.class; see
																									// also
																									// getActualTypeArguments()
			} else {
				String msg = "Iced parameter for handler: " + clz
						+ " uses a type parameterization scheme that we don't yet handle: " + handler_type_parms[parm];
				Log.warn(msg);
				throw H2O.fail(msg);
			}
		} else {
			// Superclass is not a ParameterizedType, so we just have Iced.
			parm_class = Iced.class; // If the handler isn't parameterized on the Iced class then this has to be
										// Iced.
		}
	...
	public static Class findMethodParameterClass(Method method, int parm) {
		...
		if (clzes.length <= parm) {
			throw H2O.fail("Asked for the class of parameter number: " + parm + " of method: " + method
					+ ", which only has: " + clzes.length + " parameters.");
		}
		return clzes[parm];
	}
	...
	/**
	 * Reflection helper which returns the actual class for a field which has a
	 * parameterized type. E.g., DeepLearningV2's "parameters" class is in parent
	 * ModelBuilderSchema, and is parameterized by type parameter P.
	 */
	public static Class findActualFieldClass(Class clz, Field f) {
		...
		if (-1 == which_tv) {
			...
			// E.g., this happens when getting the metadata for the parameters field of
			// ModelSchemaV3.
	 ...
	 * <p>
	 * Java reflection will either give you all the public fields all the way up the
	 * class hierarchy (getField()), or will give you all the
	 * private/protected/public only in the single class (getDeclaredField()). This
	 * method uses the latter but walks up the class hierarchy.
	 */
