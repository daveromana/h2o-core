...
/**
 * Simple Co-Occurrence based tabulation of X vs Y, where X and Y are two Vecs
 * in a given dataset Uses histogram of given resolution in X and Y Handles
 * numerical/categorical data and missing values Supports observation weights
 *
 * Fills up two double[][] arrays: _countData[xbin][ybin] contains the sum of
 * observation weights (or 1) for co-occurrences in bins xbin/ybin
 * _responseData[xbin][2] contains the mean value of Y and the sum of
 * observation weights for a given bin for X
 */
public class Tabulate extends Keyed<Tabulate> {
	...
	public Vec cardRid(Vec a) {
		...
		if (a.cardinality() > _nbins_predictor) {
			Interaction in = new Interaction();
			in._source_frame = _dataset._key;
			in._factor_columns = new String[] { _predictor };
			in._max_factors = _nbins_predictor - 1;
			in.execImpl(null);
			a = in._job._result.get().anyVec();
		} else if (a.isInt() && (a.max() - a.min() + 1) <= _nbins_predictor) {
			a = a.toCategoricalVec();
		}
	...
	public Tabulate execImpl() {
		...
		t = null;
		y = ((Tabulate) t).cardRid(y);

		if (y != null && y.cardinality() > 2)
			Log.warn(
					"Response column has more than two factor levels - mean response depends on lexicographic order of factors!");
	...
	private static class CoOccurrence extends MRTask<CoOccurrence> {
		...
		public void map(Chunk x, Chunk y, Chunk w) {
			for (int r = 0; r < x.len(); ++r) {
				...
				if (!y.isNA(r)) {
					AtomicUtils.DoubleArray.add(_sp._response_data[xbin], 0, weight * y.atd(r)); // add to mean response
																									// for x
	...
	public TwoDimTable tabulationTwoDimTable() {
		...
		colTypes[2] = "double";
		colFormats[2] = "%f";
		TwoDimTable table = new TwoDimTable(tableHeader, null/* tableDescription */, rowHeaders, colHeaders, colTypes,
				colFormats, null);
