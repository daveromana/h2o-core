...
/**
 * Serializable 2D Table containing Strings or doubles Table can be named
 * Columns and Rows can be named Fields can be empty
 */
public class TwoDimTable extends Iced {
	...
	// Double.longBitsToDouble(0x7ff8000000000100L); //also a NaN, but not
	 ...
	 * @param rowHeaders
	 *            R-dim array for row headers
	 * @param colHeaders
	 *            C-dim array for column headers
	 * @param colTypes
	 *            C-dim array for column types
	 * @param colFormats
	 *            C-dim array with printf format strings for each column
	 * @param colHeaderForRowHeaders
	 *            column header for row headers
	 */
	public TwoDimTable(String tableHeader, String tableDescription, String[] rowHeaders, String[] colHeaders,
			String[] colTypes, String[] colFormats, String colHeaderForRowHeaders) {
		if (tableHeader == null) {
			tableHeader = "";
		}
			...
			for (int c = 0; c < colDim; ++c) {
				...
				if (!(colTypes[c].equals("double") || colTypes[c].equals("float") || colTypes[c].equals("int")
					...
					throw new IllegalArgumentException(
							"colTypes values must be one of \"double\", \"float\", \"int\", \"long\", or \"string\"");
				}
			}
		}
	 ...
	 *            column header for row headers
	 * @param strCellValues
	 *            String[R][C] array for string cell values, can be null (can
	 *            provide String[R][], for example)
	 * @param dblCellValues
	 *            double[R][C] array for double cell values, can be empty (marked
	 *            with emptyDouble - happens when initialized with double[R][])
	 */
	public TwoDimTable(String tableHeader, String tableDescription, String[] rowHeaders, String[] colHeaders,
			String[] colTypes, String[] colFormats, String colHeaderForRowHeaders, String[][] strCellValues,
			...
			if (colTypes[c].equalsIgnoreCase("string")) {
				for (String[] vec : strCellValues) {
					...
					if (vec.length != colDim) {
						throw new IllegalArgumentException(
								"Each row in strCellValues must have the same length as colHeaders");
					}
				}
				break;
			}
		...
		for (int c = 0; c < colDim; ++c) {
			if (!colTypes[c].equalsIgnoreCase("string")) {
				for (double[] vec : dblCellValues) {
					if (vec.length != colDim) {
						throw new IllegalArgumentException(
								"Each row in dblCellValues must have the same length as colHeaders");
					}
				}
				break;
			}
		...
		for (int r = 0; r < rowDim; ++r) {
			for (int c = 0; c < colDim; ++c) {
				if (strCellValues[r] != null && strCellValues[r][c] != null && dblCellValues[r] != null
						&& !isEmpty(dblCellValues[r][c])) {
					throw new IllegalArgumentException(
							"Cannot provide both a String and a Double at row " + r + " and column " + c + ".");
				}
			}
		}
	 ...
	 *
	 * @param newHeader:
	 *            String containing new table header.
	 */
	 ...
	 * 
	 * @param pad
	 *            number of spaces for padding between columns
	 * @return String containing the ASCII version of the table
	 */
	...
	private boolean skip(int row) {
		...
		if (getRowDim() <= PRINTOUT_ROW_LIMIT) {
			return false;
		}
		if (row <= PRINTOUT_ROW_LIMIT / 2) {
			return false;
		}
		if (row >= getRowDim() - PRINTOUT_ROW_LIMIT / 2) {
			return false;
		}
		return true;
	}
	 ...
	 *            number of spaces for padding between columns
	 * @param full
	 *            whether to print the full table (otherwise top 5 and bottom 5 rows
	 *            only)
	 * @return String containing the ASCII version of the table
	 */
		...
		for (int r = 0; r <= actualRowDim; ++r) {
			...
			if (actualRowDim != rowDim && r - 1 == PRINTOUT_ROW_LIMIT / 2) {
				...
				for (int c = 1; c <= colDim; ++c) {
					len = colLen[c];
					if (len > 0) {
						sb.append(String.format("%" + (len + pad) + "s",
								cellStrings[r][c].equals("null") ? "" : cellStrings[r][c]));
					}
				}
			}
			sb.append("\n");
