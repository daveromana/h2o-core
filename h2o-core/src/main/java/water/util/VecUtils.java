...
public class VecUtils {
	 ...
	 *
	 * This method accepts all {@link Vec} types as input. The original Vec is not
	 * mutated.
	 *
	 * If src is a categorical {@link Vec}, a copy is returned.
	 *
	 * If src is a numeric {@link Vec}, the values are converted to strings used as
	 * domain values.
	 *
	 * For all other types, an exception is currently thrown. These need to be
	 * replaced with appropriate conversions.
	 *
	 * Throws H2OIllegalArgumentException() if the resulting domain exceeds
	 * Categorical.MAX_CATEGORICAL_COUNT.
	 *
	 * @param src
	 *            A {@link Vec} whose values will be used as the basis for a new
	 *            categorical {@link Vec}
	...
	public static Vec toCategoricalVec(Vec src) {
		...
		case Vec.T_TIME: // PUBDEV-2205
			throw new H2OIllegalArgumentException(
					"Changing time/date columns to a categorical" + " column has not been implemented yet.");
		case Vec.T_UUID:
			throw new H2OIllegalArgumentException(
					"Changing UUID columns to a categorical" + " column has not been implemented yet.");
		default:
			throw new H2OIllegalArgumentException(
					"Unrecognized column type " + src.get_type_str() + " given to toCategoricalVec()");
		}
	}
	 ...
	 *
	 * FIXME: implement in more efficient way with Brandon's primitives for
	 * BufferedString manipulation
	 *
	 * @param vec
	 *            a string {@link Vec}
	...
	public static Vec numericToCategorical(Vec src) {
		if (src.isInt()) {
			...
			long dom[] = (min >= 0 && max < Integer.MAX_VALUE - 4) ? new CollectDomainFast(max).doAll(src).domain()
					: new CollectIntegerDomain().doAll(src).domain();
			if (dom.length > Categorical.MAX_CATEGORICAL_COUNT) {
				throw new H2OIllegalArgumentException("Column domain is too large to be represented as an categorical: "
						+ dom.length + " > " + Categorical.MAX_CATEGORICAL_COUNT);
			}
	 ...
	 * If src is a categorical {@link Vec}, a copy is returned.
	 *
	 * If src is a string {@link Vec}, all values that can be are parsed into reals
	 * or integers, and all others become NA. See stringToNumeric for parsing
	 * details.
	 *
	 * If src is a numeric {@link Vec}, a copy is made.
	 *
	 * If src is a time {@link Vec}, the milliseconds since the epoch are used to
	 * populate the new Vec.
	 *
	 * If src is a UUID {@link Vec}, the existing numeric storage is used to
	 * populate the new Vec.
	 *
	 * Throws H2OIllegalArgumentException() if the resulting domain exceeds
	 * Categorical.MAX_CATEGORICAL_COUNT.
	...
	public static Vec toNumericVec(Vec src) {
		...
		default:
			throw new H2OIllegalArgumentException(
					"Unrecognized column type " + src.get_type_str() + " given to toNumericVec()");
		}
	}

	/**
	 * Create a new {@link Vec} of numeric values from a string {@link Vec}. Any
	 * rows that cannot be converted to a number are set to NA.
	 *
	 * Currently only does basic numeric formats. No exponents, or hex values.
	 * Doesn't even like commas or spaces. :( Needs love. Handling more numeric
					...
					for (int i = 0; i < chk._len; i++) {
						if (!chk.isNA(i)) {
							...
							case BufferedString.REAL:
								...
								break;
							default:
								throw new H2OIllegalValueException(
										"Received unexpected type when parsing a string to a number.", this);
							}
						} else
							newChk.addNA();
					}
	 ...
	 *
	 * If the first value in the domain of the src Vec is a stringified ints, then
	 * it will use those ints. Otherwise, it will use the raw enumeration level
	 * mapping. If the domain is stringified ints, then all of the domain must be
	 * able to be parsed as an int. If it cannot be parsed as such, a
	 * NumberFormatException will be caught and rethrown as an
	 * H2OIllegalArgumentException that declares the illegal domain value.
	 * Otherwise, the this pointer is copied to a new Vec whose domain is null.
	 *
	 * The magic of this method should be eliminated. It should just use enumeration
	 * level maps. If the user wants domains to be used, call
	 * categoricalDomainsToNumeric(). PUBDEV-2209
	 *
	 * @param src
	 *            a categorical {@link Vec}
	 * @return a numeric {@link Vec}
	 */
	 ...
	 *
	 * If src is a categorical {@link Vec}, levels are dropped, and the {@link Vec}
	...
	public static Vec toStringVec(Vec src) {
		...
		default:
			throw new H2OIllegalArgumentException(
					"Unrecognized column type " + src.get_type_str() + " given to toStringVec().");
		}
	}
	 ...
	 *
	 * Transformation is done by a {@link Categorical2StrChkTask} which provides a
	 * mapping between values - without copying the underlying data.
	 *
	 * @param src
	 *            a categorical {@link Vec}
	...
	public static Vec categoricalToStringVec(Vec src) {
		if (!src.isCategorical()) {
			throw new H2OIllegalValueException(
					"Can not convert a non-categorical column" + " using categoricalToStringVec().", src);
		}
	 ...
	 *
	 * Currently only uses a default pretty printer. Would be better if it accepted
	 * a format string PUBDEV-2211
	 *
	 * @param src
	 *            a numeric {@link Vec}
	 * @return a string {@link Vec}
	...
	public static Vec numericToStringVec(Vec src) {
		if (src.isCategorical() || src.isUUID()) {
			throw new H2OIllegalValueException("Cannot convert a non-numeric column" + " using numericToStringVec() ",
					src);
		}
		Vec res = new MRTask() {
			@Override
	...
	public static Vec categoricalDomainsToNumeric(final Vec src) {
		if (!src.isCategorical()) {
			throw new H2OIllegalArgumentException(
					"categoricalToNumeric() conversion only works on categorical columns");
		}
		// check if the 1st lvl of the domain can be parsed as int
		return new MRTask() {
			@Override
		...
		private IcedDouble addValue(IcedDouble val) {
			...
			if (!_uniques.containsKey(val)) {
				...
				if (_uniques.size() > _maxDomain) {
					throw new RuntimeException("Too many unique values. Expected |uniques| < " + _maxDomain
							+ ", already got " + _uniques.size());
				}
			}
			return val;
		}
	...
	/**
	 * Collect numeric domain of given {@link Vec} A map-reduce task to collect up
	 * the unique values of an integer {@link Vec} and returned as the domain for
	 * the {@link Vec}.
	 */
	public static class CollectIntegerDomain extends MRTask<CollectIntegerDomain> {
		...
		/**
		 * Returns exact numeric domain of given {@link Vec} computed by this task. The
		 * domain is always sorted. Hence: domain()[0] - minimal domain value
	...
	/**
	 * Create a new categorical {@link Vec} with deduplicated domains from a
	 * categorical {@link Vec}.
	 * 
	 * Categoricals may have the same values after munging, and should have the same
	 * domain index in the numerical chunk representation. Unify categoricals that
	 * are the same by remapping their domain indices.
	 * 
	 * Could be more efficient with a vec copy and replace domain indices as needed.
	 * PUBDEV-2587
	 */
	...
	public static class DomainDedupe extends MRTask<DomainDedupe> {
		...
		public DomainDedupe(HashMap<Integer, Integer> oldToNewDomainIndex) {
			_oldToNewDomainIndex = oldToNewDomainIndex;
		}
		...
		public static Vec domainDeduper(Vec vec, HashMap<String, ArrayList<Integer>> substringToOldDomainIndices) {
			...
			String[][] dom2D = {
					Arrays.copyOf(alphabetizedSubstrings.toArray(), alphabetizedSubstrings.size(), String[].class) };
	...
	/**
	 * (Optimized for positive ints) Collect numeric domain of given {@link Vec} A
	 * map-reduce task to collect up the unique values of an integer {@link Vec} and
	 * returned as the domain for the {@link Vec}.
	 */
	...
	/**
	 * Compute the mean (weighted) response per categorical level Skip NA values
	 * (those are already a separate bucket in the tree building histograms, for
